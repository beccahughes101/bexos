...
pub enum SuspendEvent {
    ...
    Fail {
        time: zx::BootInstant,
        wakeup_sources: Option<Vec<String>>,
    },
}
...
impl SuspendResumeManagerInner {
    ...
    fn signal_wake_events(&mut self) {
        ...
        self.active_lock_writer
            .signal_peer(clear_mask, set_mask)
            .expect("Failed to signal peer");
    }
    ...
    fn record_wakeup_sources(&self, node: &inspect::Node) {
        ...
        for (origin, source) in self.wakeup_sources.iter() {
            ...
            child.record_int(
                "active_duration_mono (ns)",
                source.active_duration().into_nanos(),
            );
...
impl Default for SuspendResumeManager {
    fn default() -> Self {
        ...
        Self {
            message_counters,
            inner,
            ebpf_suspend_lock: Default::default(),
        }
    }
}
...
impl SuspendResumeManager {
    ...
    pub fn suspend(&self, suspend_state: SuspendState) -> Result<(), Errno> {
        ...
        let external_wake_source_abort = state.external_wake_sources.values().find_map(|source| {
            if source
                .handle
                .wait_one(source.signals, zx::MonotonicInstant::INFINITE_PAST)
                .is_ok()
            {
...
impl OwnedMessageCounter {
    ...
    pub fn mark_pending(&self) {
        self.counter
            .as_ref()
            .map(|c| c.add(1).expect("Failed to increment counter"));
    }
...
impl fmt::Display for OwnedMessageCounter {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "Counter({}): {:?}",
            self.name,
            self.counter.as_ref().map(|c| c.read())
        )
    }
}
...
mod test {
    ...
    fn test_external_wake_source_aborts_suspend() {
        ...
        assert!(
            state
                .external_wake_sources
                .contains_key(&event.koid().unwrap())
        );
    }
}
