...
fn send_signal_prio(
    ...
    if signal == Some(SIGKILL) {
        task.write().thaw();
        task.thread_group()
            .set_stopped(StopState::ForceWaking, siginfo, false);
        task.write()
            .set_stopped(StopState::ForceWaking, None, None, None);
    } else if signal == Some(SIGCONT) || force_wake {
        task.thread_group()
            .set_stopped(StopState::Waking, siginfo, false);
        task.write()
            .set_stopped(StopState::Waking, None, None, None);
    }
...
pub fn dequeue_signal(current_task: &mut CurrentTask) {
    ...
    let siginfo = if kernel_signal.is_some() {
        None
    } else {
        task_state.take_any_signal()
    };
    prepare_to_restart_syscall(
        thread_state,
        siginfo
            .as_ref()
            .map(|siginfo| task.thread_group().signal_actions.get(siginfo.signal)),
    );
    ...
    loop {
        ...
        match action {
            ...
            DeliveryAction::CallHandler => {
                ...
                if let Err(err) = dispatch_signal_handler(
                        ...
                        || action == DeliveryAction::Ignore
                    {
                        ...
                        task.thread_group()
                            .signal_actions
                            .set(SIGSEGV, sigaction_t::default());
                    }
            ...
            DeliveryAction::Stop => {
                drop(task_state);
                task.thread_group()
                    .set_stopped(StopState::GroupStopping, Some(siginfo), false);
            }
...
fn dispatch_signal_handler(
    ...
    let main_stack = registers
        .stack_pointer_register()
        .checked_sub(RED_ZONE_SIZE);
    ...
    let stack_pointer = align_stack_pointer(
        stack_bottom
            .checked_sub(SIG_STACK_SIZE as u64)
            .ok_or_else(|| {
                errno!(
                    EINVAL,
                    format!(
...
pub fn prepare_to_restart_syscall(
    ...
    if err.should_restart(sigaction) {
        ...
        thread_state
            .registers
            .set_return_register(EINTR.return_value());
    }
}
