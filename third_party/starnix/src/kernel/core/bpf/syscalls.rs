...
fn read_attr<Attr: FromBytes>(
    ...
    if attr_size > sizeof_attr {
        let tail_addr = attr_addr
            .checked_add(sizeof_attr)
...
fn read_map_key(
    ...
    if map.schema.map_type == bpf_map_type_BPF_MAP_TYPE_SK_STORAGE {
        ...
        let socket = socket
            .downcast_socket::<ZxioBackedSocket>()
...
pub fn sys_bpf(
    ...
    match cmd {
        ...
        bpf_cmd_BPF_MAP_LOOKUP_ELEM => {
            ...
            let _suspend_lock = current_task
                .kernel()
                .suspend_resume_manager
                .acquire_ebpf_suspend_lock();
        ...
        bpf_cmd_BPF_MAP_GET_NEXT_KEY => {
            ...
            let next_key = map
                .get_next_key(key.as_ref().map(|k| &k[..]))
                .map_err(map::map_error_to_errno)?;
        ...
        bpf_cmd_BPF_RAW_TRACEPOINT_OPEN => {
            track_stub!(
                TODO("https://fxbug.dev/322874055"),
