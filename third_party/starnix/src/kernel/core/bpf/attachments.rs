...
pub fn bpf_prog_detach(
    ...
    current_task
        ...
        .ebpf_state
        .attachments
        .detach_prog(current_task, attach_type, target_fd)
}
...
mod internal {
    ...
    impl BpfSockOptWithValue {
        pub fn new(
            ...
            unsafe {
                sockopt.sockopt.0.__bindgen_anon_2.optval = uaddr {
                    addr: sockopt.value_buf.as_mut_ptr() as u64,
                };
            ...
            sockopt.sockopt.0.__bindgen_anon_1.sk = (uaddr {
                addr: BpfValue::from(sock).into(),
            })
            .into();
...
impl<'a> SockOptEbpfRunContextImpl<'a> {
    pub fn new(current_task: &'a CurrentTask, sockopt: EbpfPtr<'a, BpfSockOpt>) -> Self {
        Self {
            ebpf_run_context: EbpfRunContextImpl::new(current_task),
            sockopt,
        }
    }
}
...
impl CgroupEbpfProgramSet {
    ...
    pub fn run_sock_addr_prog(
        ...
        let mut bpf_sockaddr = BpfSockAddr {
        ...
        bpf_sockaddr.__bindgen_anon_1.sk = (uaddr {
            addr: BpfValue::from(&bpf_sock).into(),
        })
        .into();
    ...
    pub fn run_getsockopt_prog(
        ...
        let Some(prog) = prog_guard.as_ref() else {
            return error
                .map(|e| Err(e))
                .unwrap_or_else(|| Ok((value_buf, optlen)));
        };
        ...
        let retval = error
            .as_ref()
            .map(|e| -(e.code.error_code() as i32))
            .unwrap_or(0);
        ...
        let retval = match result {
            ...
            _ => {
                ...
                log_error!(
                    "eBPF getsockopt program returned invalid result: {}",
                    result
                );
                retval
            }
    ...
    fn try_from(attach_type: AttachType) -> Result<Self, Self::Error> {
        match attach_type {
            ...
            | AttachType::TraceKprobeSession => {
                track_stub!(
                    TODO("https://fxbug.dev/322873416"),
                    "BPF_PROG_ATTACH",
                    attach_type
                );
                error!(ENOTSUP)
