...
impl Program {
    pub fn new(
        ...
        let fidl_id = febpf::ProgramId {
            id: fidl_handle.koid().expect("Failed to get koid").raw_koid(),
        };
        let fidl_handle = febpf::ProgramHandle {
            handle: fidl_handle,
        };
    ...
    fn check_load_access(current_task: &CurrentTask, info: &ProgramInfo) -> Result<(), Errno> {
        if matches!(
            info.program_type,
            ProgramType::CgroupSkb | ProgramType::SocketFilter
...
impl TryFrom<&Program> for febpf::VerifiedProgram {
    ...
    fn try_from(program: &Program) -> Result<febpf::VerifiedProgram, Errno> {
        ...
        for v in program.program.struct_access_instructions() {
            let struct_id = StructId::try_from(&v.memory_id)
