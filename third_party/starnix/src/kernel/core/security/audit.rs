        ...
        for arg in cmdline_iter {
            match arg {
                ...
                ArgNameAndValue {
                    ...
                    value: Some(value),
                } => self.backlog_limit.store(
                    value.parse().unwrap_or(DEFAULT_BACKLOG_LIMIT),
                    Ordering::Release,
                ),
                _ => (),
            }
...
impl AuditLogger {
    ...
    pub fn detach_client(&self, client: &Arc<AuditNetlinkClient>) {
            ...
            .is_some_and(|current_client| Arc::ptr_eq(client, &current_client))
        {
            ...
            drop(client_guard);
            self.audit_log(AUDIT_CONFIG_CHANGE as u16, || {
    ...
    pub fn set_status(
        ...
        if status.mask & AUDIT_STATUS_BACKLOG_LIMIT != 0 {
            self.configuration
                .backlog_limit
                .store(status.backlog_limit, Ordering::Release);
        }
        if status.mask & AUDIT_STATUS_FAILURE != 0 {
            self.configuration
                .fail_action
                .store(status.failure as u8, Ordering::Release);
        }
    ...
    fn push_back_audit(
        ...
        client_guard.messages.push_back(AuditMessage {
            audit_type,
            message: audit_message.into(),
        });
    }
