...
pub(super) fn audit_decision(
    ...
    let audit_data_with_bug = [
        Auditable::from_bug(result.todo_bug.map(NonZeroU32::get).unwrap_or(0)),
        audit_data,
    ];
    let audit_data = if result.todo_bug.is_some() {
        (&audit_data_with_bug).into()
    } else {
        audit_data
    };
