...
impl SimpleDirectoryMutator {
    ...
    pub fn entry(&self, name: &str, ops: impl Into<Box<dyn FsNodeOps>>, mode: FileMode) {
        ...
        let node = self
            .fs
            .create_node_and_allocate_node_id(ops, FsNodeInfo::new(mode, FsCred::root()));
        self.node(name, node);
    }
...
impl FsNodeOps for Arc<SimpleDirectory> {
    ...
    fn lookup(
        ...
        entries
            .get(name)
            .cloned()
            .ok_or_else(|| (self.not_found_handler)(name, &entries))
    }
}
...
mod tests {
    ...
    async fn test_default_not_found_handler() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let node = dir
                .clone()
                .into_node(&current_task.fs().root().entry.node.fs(), 0o777);
    ...
    async fn test_simple_directory_lookups() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let sublink = dir
                .lookup("subdir/sublink".into())
                .expect("lookup subdir/sublink");
