...
fn copy_data(
    ...
    let read = |progress: usize, data: &mut VecOutputBuffer| {
        if let Some(offset) = operand_in.maybe_offset {
            operand_in
                .file
                .read_at(current_task, offset + progress, data)
        } else {
    ...
    let write = |progress: usize, data: &mut VecInputBuffer| {
        if let Some(offset) = operand_out.maybe_offset {
            operand_out
                .file
                .write_at(current_task, offset + progress, data)
        } else {
...
pub fn splice(
    ...
    if flags & !KNOWN_FLAGS != 0 {
        track_stub!(
            TODO("https://fxbug.dev/322875389"),
            "splice flags",
            flags & !KNOWN_FLAGS
        );
        return error!(EINVAL);
...
pub fn vmsplice(
    ...
    if flags & !KNOWN_FLAGS != 0 {
        track_stub!(
            TODO("https://fxbug.dev/322875487"),
            "vmsplice flags",
            flags & !KNOWN_FLAGS
        );
        return error!(EINVAL);
...
pub fn tee(
    ...
    if flags & !KNOWN_FLAGS != 0 {
        track_stub!(
            TODO("https://fxbug.dev/322874902"),
            "tee flags",
            flags & !KNOWN_FLAGS
        );
        return error!(EINVAL);
