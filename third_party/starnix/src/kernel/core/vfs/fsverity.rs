...
fn fsverity_descriptor_from_enable_arg(
    ...
    if src.sig_size > 0 {
        ...
        track_stub!(
            TODO("https://fxbug.dev/302620572"),
            "fsverity_enable signature"
        );
        return error!(ENOTSUP);
    }
    let salt =
...
pub mod ioctl {
    ...
    pub fn enable(
        ...
        let _mapping = file.to_mapping(Some(FileWriteGuardMode::ExecMapping))?;
        {
        ...
        *file.node().fsverity.lock() = if result.is_ok() {
        ...
        match &*file.node().fsverity.lock() {
            FsVerityState::FsVerity => {
                ...
                let descriptor = file
                    .node()
                    .ops()
                    .get_fsverity_descriptor(block_size.ilog2() as u8)?;
