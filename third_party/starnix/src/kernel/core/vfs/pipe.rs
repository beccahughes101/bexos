...
impl<'a> OutputBuffer for SpliceOutputBuffer<'a> {
    fn write_each(&mut self, callback: &mut OutputBufferCallback<'_>) -> Result<usize, Errno> {
        ...
        let bytes = unsafe {
            read_to_vec::<u8, _>(self.available, |buf| {
                callback(buf).map(NumberOfElementsRead)
            })
        }?;
        ...
        if bytes_len > 0 {
            ...
            self.pipe
                .messages
                .write_message(PipeMessageData::from(bytes).into());
            if was_empty {
...
impl<'a> InputBuffer for SpliceInputBuffer<'a> {
    fn peek_each(&mut self, callback: &mut InputBufferCallback<'_>) -> Result<usize, Errno> {
        ...
        for message in self.pipe.messages.messages() {
            ...
            let result = message
                .data
                .with_bytes(|bytes| callback(&bytes[0..to_read]))?;
            if result > to_read {
                return error!(EINVAL);
            }
            read += result;
...
impl PipeFileObject {
    ...
    fn lock_pipe_for_reading<'a>(
        ...
        self.lock_pipe_for_reading_with(current_task, file, || Ok(()), non_blocking)
            .map(|(_, l)| l)
    }
    ...
    pub fn splice_from(
        ...
        let mut buffer = SpliceOutputBuffer {
            pipe: &mut pipe,
            len,
            available: len,
        };
    ...
    pub fn splice_to(
        ...
        let mut buffer = SpliceInputBuffer {
            pipe: &mut pipe,
            len,
            available: len,
        };
    ...
    pub fn vmsplice_from(
        ...
        for mut mapping in mappings.into_iter() {
            ...
            pipe.messages
                .write_message(PipeMessageData::Vmspliced(mapping).into());
            remaining -= actual;
    ...
    pub fn lock_pipes<'a, 'b>(
        ...
        let pipe_in = file_in
            .downcast_file::<PipeFileObject>()
            .ok_or_else(|| errno!(EINVAL))?;
        let pipe_out = file_out
            .downcast_file::<PipeFileObject>()
            .ok_or_else(|| errno!(EINVAL))?;
