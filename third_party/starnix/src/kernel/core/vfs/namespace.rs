...
impl Namespace {
    ...
    pub fn new_with_flags(fs: FileSystemHandle, flags: MountpointFlags) -> Arc<Namespace> {
        let kernel = fs
            .kernel
            .upgrade()
            .expect("can't create namespace without a kernel");
    ...
    pub fn translate_node(
        ...
        for mountpoint in mountpoints.iter().rev() {
            let next_mount = {
                ...
                match mount
                    .relations
                    .get_submount(&scope, &PtrKey::from(mountpoint))
                {
                    Some(m) => m,
                    None => {
                        mounts_guard.defer_drop(mount);
    ...
    pub fn pivot_root(
        ...
        let current_root_mountpoint = current_root_mount
            .mountpoint()
        ...
        let new_root_parent = mounts_guard.retain(
            new_root_mountpoint
                .mount
                .as_ref()
                .expect("a mountpoint must be part of a mount"),
        );
        ...
        current_root_mount
            .relations
            .set_mountpoint(&mounts_guard, None);
...
impl Mount {
    ...
    fn new_with_root(root: DirEntryHandle, flags: MountpointFlags) -> MountHandle {
        ...
        let kernel = fs
            .kernel
            .upgrade()
            .expect("can't create mount without kernel");
        Arc::new(Self {
    ...
    fn create_submount(
        ...
        let peers = self
            .peer_group()
            .map(|g| g.copy_propagation_targets())
            .unwrap_or_default();
    ...
    pub fn has_submount(&self, dir_entry: &DirEntryHandle) -> bool {
        let scope = RcuReadScope::new();
        self.relations
            .get_submount(&scope, &PtrKey::from(dir_entry))
            .is_some()
    }
    ...
    pub fn add_submount_internal(
        ...
        let submount = mount
            .kernel()
            .mounts
            .register_mount(dir, Arc::clone(&mount));
        ...
        mount
            .relations
            .set_mountpoint(guard, Some((Arc::downgrade(self), Arc::downgrade(dir))));
        assert!(
            old_mountpoint.is_none(),
            "add_submount can only take a newly created mount"
        );
        ...
        if let Some(old_mount) = &*old_mount {
            old_mount.mount.relations.set_mountpoint(
                guard,
                Some((Arc::downgrade(&mount), Arc::downgrade(&mount.root))),
            );
            let new_old_submount = mount
                .kernel()
                .mounts
                .register_mount(&mount.root, old_mount.mount.clone());
    ...
    fn set_upstream(self: &Arc<Mount>, guard: &MountsWriteToken, group: &Arc<PeerGroup>) {
        ...
        self.relations
            .upstream
            .update(Some((Arc::downgrade(group), Arc::as_ptr(self).into())));
    }
...
impl PeerGroup {
    ...
    fn add(&self, _guard: &MountsWriteToken, mount: &Arc<Mount>) {
        ...
        unsafe {
            self.mounts
                .insert(&starnix_rcu::RcuReadScope::new(), WeakKey::from(mount), ())
        };
    }
...
impl ProcMountsFile {
    pub fn new_node(tid: Pid) -> impl FsNodeOps {
        SimpleFileNode::new(move |_| {
            Ok(Self {
                dynamic_file: DynamicFile::new(ProcMountsFileSource { tid: tid.clone() }),
            })
        })
    }
}
...
impl NamespaceNode {
    ...
    pub fn open_create_node(
        ...
        let create_fn = |dir: &FsNodeHandle, mount: &MountInfo, name: &_| {
            dir.create_node(current_task, mount, name, mode, dev, owner)
        };
        let entry = if flags.contains(OpenFlags::EXCL) {
            self.entry
                .create_entry(current_task, &self.mount, name, create_fn)
        } else {
            self.entry
                .get_or_create_entry(current_task, &self.mount, name, create_fn)
        }?;
    ...
    pub fn create_node(
        ...
        let entry =
            self.entry
                .create_entry(current_task, &self.mount, name, |dir, mount, name| {
                    dir.create_node(current_task, mount, name, mode, dev, owner)
                })?;
        Ok(self.with_new_entry(entry))
    }
    ...
    pub fn unlink(
        ...
        if DirEntry::is_reserved_name(name) {
            ...
            self.entry
                .unlink(current_task, &self.mount, name, kind, directory_mode)
        }
    }

    // Resolve the current node.
    //
    // Depending on context, this will resolve symlink and mount point.
    fn resolve(
        self,
        current_task: &CurrentTask,
        ...
        loop {
            if !node.entry.node.is_lnk() || context.symlink_mode == SymlinkMode::NoFollow {
                break;
            }
            ...
            node = match node.readlink(current_task)? {
                SymlinkTarget::Path(link_target) => {
                    ...
                    current_task.lookup_path(context, link_directory, link_target.as_ref())?
                }
    ...
    pub fn lookup_child(
        ...
        self.lookup_children(current_task, context, &[basename])
    }
        ...
        while basenames.len() > 0 {
            ...
            if basename == ".." {
                ...
                if current_namespace_node != root {
                    current_namespace_node = current_namespace_node
                        .parent()
                        .unwrap_or(current_namespace_node)
                }
            ...
            if basenames.len() == 1
                || !current_namespace_node
                    ...
                    .node
                    .ops()
                    .has_lookup_pipelined()
            {
                current_namespace_node = current_namespace_node.with_new_entry(
                    current_namespace_node.entry.component_lookup(
                        current_task,
                ...
                current_namespace_node = current_namespace_node.resolve(current_task, context)?;
            ...
            let pipelined_basenames = if let Some(pos) = basenames
                .iter()
            ...
            let precomputed_entries = current_namespace_node.entry.get_children_pipelined(
                current_task,
                &current_namespace_node.mount,
                pipelined_basenames,
            );
            for entry in precomputed_entries {
                basenames = &basenames[1..];
                let child = current_namespace_node.with_new_entry(entry?);

                current_namespace_node = child.clone().resolve(current_task, context)?;
                ...
                if current_namespace_node != child {
                    break;
                }
            }
        }
    ...
    fn enter_one_mount(&self) -> Option<NamespaceNode> {
        if let Some(mount) = self.mount.deref() {
            if let Some(submount) = mount
                .relations
                .get_submount(&RcuReadScope::new(), &PtrKey::from(&self.entry))
            {
    ...
    pub fn mount(&self, what: WhatToMount, flags: MountpointFlags) -> Result<(), Errno> {
        ...
        if source_is_dir != target.entry.node.is_dir() {
            return error!(ENOTDIR);
        }
        ...
        let kernel = self
            ...
            .kernel
            .upgrade()
            .expect("can't mount without a kernel");
        let mounts_guard = kernel.mounts_lock();
        let what = mounts_guard.defer_drop(what);
        ...
        let writeable_entry = mounts_guard.retain(&mountpoint.entry);
    ...
    pub fn truncate(&self, current_task: &CurrentTask, length: u64) -> Result<(), Errno> {
        self.entry
            .node
            .truncate(current_task, &self.mount, length)?;
...
impl PartialEq for NamespaceNode {
    fn eq(&self, other: &Self) -> bool {
        self.mount
            .as_ref()
            .map(Arc::as_ptr)
            .eq(&other.mount.as_ref().map(Arc::as_ptr))
...
impl ActiveNamespaceNode {
    pub fn new(name: NamespaceNode) -> Self {
        let marker = name
            .mount
            .as_ref()
            .map(|mount| mount.active_client_counter.clone());
        Self {
            name,
            _marker: marker,
        }
...
impl Drop for Submount {
    fn drop(&mut self) {
        self.mount
            .kernel()
            .mounts
            .unregister_mount(&self.dir, &self.mount)
    }
}
...
mod test {
    ...
    async fn test_namespace() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _dev_node = root_node
                .create_dir(&current_task, "dev".into())
            ...
            let _dev_pts_node = dev_root_node
                .create_dir(&current_task, "pts".into())
            ...
            let pts_parent = pts
                ...
                .expect("failed to get parent of pts");
            assert!(Arc::ptr_eq(&pts_parent.entry, &dev.entry));

            let dev_parent = dev
                .parent()
                .ok_or_else(|| errno!(ENOENT))
                .expect("failed to get parent of dev");
    ...
    async fn test_mount_does_not_upgrade() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let mut context = LookupContext::default();
            assert!(
                dev.lookup_child(&current_task, &mut context, "pts".into())
                    .is_err()
            );
        })
        .await;
    ...
    async fn test_shadowing() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _foo_node = root_fs
                .root()
                .create_dir(&current_task, "foo".into())
                .unwrap();
            let mut context = LookupContext::default();
            let foo_dir = ns
                .root()
            ...
            foo_dir
                .mount(WhatToMount::Fs(foofs1.clone()), MountpointFlags::empty())
                .unwrap();
            let mut context = LookupContext::default();
            assert!(Arc::ptr_eq(
                &ns.root()
            ...
            foo_dir
                .mount(WhatToMount::Fs(foofs2.clone()), MountpointFlags::empty())
                .unwrap();
            let mut context = LookupContext::default();
            assert!(Arc::ptr_eq(
                &ns.root()
    ...
    async fn test_unlink_mounted_directory() {
        spawn_kernel_and_run(async |current_task| {
            ...
            foo_dir
                .mount(WhatToMount::Fs(foofs), MountpointFlags::empty())
                .unwrap();
    ...
    async fn test_rename_mounted_directory() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _foo_node = root_fs
                ...
                .create_dir(&current_task, "foo".into())
                .unwrap();
            let _bar_node = root_fs
                .root()
                .create_dir(&current_task, "bar".into())
                .unwrap();
            let _baz_node = root_fs
                .root()
                .create_dir(&current_task, "baz".into())
                .unwrap();
            let mut context = LookupContext::default();
            let foo_dir = ns1
                .root()
            ...
            assert_eq!(
                ns1.root()
                    .lookup_child(&current_task, &mut context, "baz".into())
                    .unwrap_err(),
                errno!(ENOENT)
            );
        })
    ...
    async fn test_lookup_with_symlink_chain() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _first_subdir_node = root_node
                .create_dir(&current_task, "first_subdir".into())
                .expect("failed to mkdir dev");
            let _second_subdir_node = root_node
                .create_dir(&current_task, "second_subdir".into())
                .expect("failed to mkdir dev");
            ...
            let first_subdir = ns
                .root()
                .lookup_child(&current_task, &mut context, "first_subdir".into())
            ...
            let second_subdir = ns
                .root()
                .lookup_child(&current_task, &mut context, "second_subdir".into())
            ...
            first_subdir
                .create_symlink(&current_task, "path_symlink".into(), "real_file".into())
            ...
            let node_symlink_node = second_subdir
                .entry
                .node
                .fs()
                .create_node_and_allocate_node_id(
            ...
            second_subdir
                ...
                .create_entry(
                    ...
                    &MountInfo::detached(),
                    "node_symlink".into(),
                    move |_dir, _mount, _name| Ok(node_symlink_node),
                )
