...
use starnix_sync::{
    ...
    FuseDirEntryChildrenLevel, LockDepWriteGuard, allow_subclass,
};
...
impl DirEntry {
    ...
    pub fn open_anonymous(
        ...
        let ops = self.node.create_file_ops(current_task, flags)?;
        FileObject::new(
            current_task,
            ops,
    ...
    pub fn set_children(self: &DirEntryHandle, children: BTreeMap<FsString, DirEntryHandle>) {
        ...
        for (name, child) in children.into_iter() {
            ...
            dir_entry_children
                .children
                .insert(name.as_ref(), Arc::downgrade(&child));
        }
    }
            ...
            match current_parent.get_or_create_child(
                ...
                |parent_node, _mount, _name| {
                    ...
                    nodes =
                        parent_node
                            .ops()
                            .lookup_pipelined(parent_node, current_task, &names[i..]);
                    nodes.reverse();
                    nodes.pop().unwrap()
                },
            ) {
    ...
    pub fn create_tmpfile(
        ...
        let node = self
            .node
            .create_tmpfile(current_task, mount, mode, owner, link_behavior)?;
    ...
    pub fn unlink(
        ...
        child_to_unlink = self_children.component_lookup(current_task, mount, name)?;
        ...
        self.node
            .unlink(current_task, mount, name, &child_to_unlink.node)?;
    ...
    fn destroy(self: DirEntryHandle, mounts: &Mounts) {
        let was_already_dead = self
            .raise_flags(DirEntryFlags::IS_DEAD)
            .contains(DirEntryFlags::IS_DEAD);
        if was_already_dead {
            return;
        }
        let unmount = self
            .lower_flags(DirEntryFlags::HAS_MOUNTS)
            .contains(DirEntryFlags::HAS_MOUNTS);
    ...
    pub fn rename(
...

        {
            ...
            renamed =
                state
                    .old_parent_children()
                    .component_lookup(current_task, mount, old_basename)?;
            ...
            let lookup_replaced =
                state
                    .new_parent_children()
                    .component_lookup(current_task, mount, new_basename);
            ...
            let mut state = state.lock_info(
                old_parent,
                new_parent,
                &renamed,
                lookup_replaced.as_ref().ok(),
            );
            ...
            security::check_fs_node_rename_access(
                ...
                maybe_replaced
                    .as_ref()
                    .map(|dir_entry| dir_entry.node.deref().as_ref()),
                old_basename,
                new_basename,
            )?;
            ...
            if let Some(replaced) = maybe_replaced.as_ref() {
                ...
                new_parent.node.check_sticky_bit(
                    ...
                    state
                        .new_parent_info()
                        .unwrap_or_else(|| state.old_parent_info()),
                )?;
            }
            ...
            if flags.contains(RenameFlags::EXCHANGE) {
                ...
                let replaced = maybe_replaced
                    .as_ref()
                    .expect("replaced expected with RENAME_EXCHANGE");
                replaced.set_parent(old_parent.clone());
                replaced.local_name.update(old_basename.to_owned());
                state
                    .old_parent_children()
                    .children
                    .insert(old_basename, Arc::downgrade(replaced));
            } else {
            ...
            state
                .new_parent_children()
                .children
                .insert(new_basename, Arc::downgrade(&renamed));
            ...
            unsafe {
                if let Some(replaced) = maybe_replaced
                    .as_ref()
                    .filter(|_| flags.contains(RenameFlags::EXCHANGE))
                {
        ...
        if let Some(hook) = current_task
            .kernel()
            .expando
            .peek::<Arc<dyn inotify_hook::NotifyHook>>()
        {
            let cookie = hook.get_next_cookie();
            old_parent
                .node
                .notify(InotifyMask::MOVE_FROM, cookie, old_basename, mode, false);
            new_parent
                .node
                .notify(InotifyMask::MOVE_TO, cookie, new_basename, mode, false);
            renamed
                .node
                .notify(InotifyMask::MOVE_SELF, 0, Default::default(), mode, false);
        }
    ...
    fn get_or_create_child(
        ...
        let (child, create_result) = if let Some(child) = child {
            ...
            if self
                .node
                .fail_if_locked(current_task, &self.node.info())
                .is_ok()
            {
            ...
            let (child, create_result) =
                self.lock_children()
                    .get_or_create_child(current_task, mount, name, create_fn)?;
        ...
        let (child, exists) = match create_result {
            ...
            CreationResult::Existed { create_fn } => {
                if child.ops.revalidate(current_task, &child)? {
                    ...
                    (
                        child,
                        matches!(create_result, CreationResult::Existed { .. }),
                    )
                }
            }
        };
    ...
    fn notify_watchers(&self, event_mask: InotifyMask, is_dead: bool) {
        ...
        self.node
            .notify(event_mask, 0, Default::default(), mode, is_dead);
    }
    ...
    fn notify_creation(&self) {
        ...
        if Arc::strong_count(&self.node) > 1 {
            ...
            self.node
                .notify(InotifyMask::ATTRIB, 0, Default::default(), mode, false);
        }
        let scope = RcuReadScope::new();
        if let Some(parent) = self.parent() {
            let local_name = self.local_name.read(&scope);
            parent
                .node
                .notify(InotifyMask::CREATE, 0, local_name, mode, false);
        }
    }
    ...
    fn notify_deletion(&self) {
        ...
        if Arc::strong_count(&self.node) == 1 {
            self.node
                .notify(InotifyMask::DELETE_SELF, 0, Default::default(), mode, false);
        }
        ...
        if let Some(parent) = self.parent() {
            ...
            parent
                .node
                .notify(InotifyMask::DELETE, 0, local_name, mode, false);
        }
    }
...
fn canonicalize_name(name: impl AsRef<[u8]>) -> SmallVec<[u8; NAME_MAX as usize]> {
    ...
    match std::str::from_utf8(name) {
        Ok(valid) => CasefoldStr::new(valid)
            .casefold_normalized_chars()
            .flat_map(utf8_bytes)
            .collect(),
...
impl DirEntryChildren {
    ...
    pub fn insert(&mut self, name: &FsStr, child: Weak<DirEntry>) -> Option<Weak<DirEntry>> {
        self.entries
            .insert(DirEntryChildKey::new(name, self.casefold), child)
    }
    ...
    >(
        ...
        let create_child = |create_fn: F| {
            ...
            let (node, create_result) = match self.entry.node.lookup(current_task, mount, name) {
                ...
                Err(e) if e == ENOENT => (
                    create_fn(&self.entry.node, mount, name)?,
        ...
        if let Some(child) = self.children.get(name).and_then(Weak::upgrade) {
            ...
            if self
                .entry
                .node
                .fail_if_locked(current_task, &self.entry.node.info())
                .is_ok()
            {
                child.node.fs().did_access_dir_entry(&child);
            }
...
impl<'a> RenameGuard<'a> {
    fn lock(old_parent: &'a DirEntryHandle, new_parent: &'a DirEntryHandle) -> Self {
        if Arc::ptr_eq(old_parent, new_parent) {
            ...
            let old_parent_guard = DirEntryLockedChildren {
                entry: old_parent,
                children: g1,
            };
            let new_parent_guard = DirEntryLockedChildren {
                entry: new_parent,
                children: g2,
            };
            Self {
...
impl<'a> RenameContext<'a> {
    ...
    pub fn new_parent(&self) -> &DirEntryHandle {
        self.new_parent_guard
            .as_ref()
            .map(|g| g.entry)
            .unwrap_or(self.old_parent_guard.entry)
    }
    ...
    pub fn parent_infos_mut(
        ...
        &mut crate::vfs::FsNodeInfo,
        Option<&mut crate::vfs::FsNodeInfo>,
    ) {
    ...
    fn new_parent_children(&mut self) -> &mut DirEntryLockedChildren<'a> {
        self.new_parent_guard
            .as_mut()
            .unwrap_or(&mut self.old_parent_guard)
    }
