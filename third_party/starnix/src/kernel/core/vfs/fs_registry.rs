...
impl FsRegistry {
    ...
    where
        F: Fn(&CurrentTask, FileSystemOptions) -> Result<FileSystemHandle, Errno>
            ...
            + 'static,
    {
        let entry = FsRegistryEntry {
            flags,
            create_fs: Arc::new(create_fs),
        };
    ...
    pub fn create(
        ...
        let create_fs = self
            ...
            .lock()
            .get(fs_type)
            .map(|e| Arc::clone(&e.create_fs))?;
        Some(create_fs(current_task, options).and_then(|fs| {
            assert_eq!(
                fs_type,
                fs.name(),
                "FileSystem::name() must match the registered name."
            );
    ...
    pub fn list_all(&self) -> Vec<(FsString, FileSystemTypeFlags)> {
        self.registry
            .lock()
            .iter()
            .map(|(name, entry)| (name.clone(), entry.flags))
            .collect()
    }
}
