...
impl RecordRange {
    fn new(start: usize, length: usize) -> Self {
        Self {
            start,
            length: RecordLength::new(length),
        }
    }
...
impl RecordLocksState {
    ...
    fn apply_lock(
        ...
        for lock in self
            .locks
            .iter()
            .filter(|record| range.intersects(&record.range))
        {
            if owner == lock.owner {
    ...
    pub fn lock(
        ...
        if cmd.is_get() {
            ...
            Ok(self
                .state
                .lock()
                .get_conflicting_lock(owner, lock_type, &range)
                .or_else(|| {
            ...
            match lock_type {
                Some(lock_type) => {
                    ...
                    loop {
                        ...
                        let process_id = if cmd.is_ofd() {
                            None
                        } else {
                            Some(current_task.pid.clone())
                        };
...
mod test {
    ...
    fn test_range_add() {
        let r1 = RecordRange::new(25, 3);
        assert_eq!(
            r1 + RecordRange::new(0, 2),
            vec!(RecordRange::new(0, 2), r1)
        );
        assert_eq!(
            r1 + RecordRange::new(30, 2),
            vec!(r1, RecordRange::new(30, 2))
        );
        assert_eq!(
            r1 + RecordRange::new(30, 0),
            vec!(r1, RecordRange::new(30, 0))
        );
