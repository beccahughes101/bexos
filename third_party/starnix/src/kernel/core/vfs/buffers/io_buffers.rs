...
impl Iovec for zx::sys::zx_iovec_t {
    fn create(buffer: &UserBuffer) -> Self {
        Self {
            buffer: buffer.address.ptr() as *const u8,
            capacity: buffer.length,
        }
    }
}
    ...
    fn peek_all(&mut self) -> Result<Vec<u8>, Errno> {
        ...
        unsafe {
            read_to_vec::<u8, _>(self.available(), |buf| {
                self.peek(buf).map(NumberOfElementsRead)
            })
        }
    }
    ...
    fn read_to_array<const N: usize>(&mut self) -> Result<[u8; N], Errno> {
        ...
        unsafe {
            read_to_array::<_, _, N>(|buf| {
                self.read_exact(buf)
                    .map(|bytes_read| debug_assert_eq!(bytes_read, buf.len()))
            })
        }
    }
...
mod tests {
        ...
        executor.run_singlethreaded(async {
            spawn_kernel_and_run(async |current_task| {
                ...
                // Test incorrect callback.
                {
                    ...
                    assert!(
                        input_buffer
                            .peek_each(&mut |data| Ok(data.len() + 1))
                            .is_err()
                    );
                }
        ...
        executor.run_singlethreaded(async {
            spawn_kernel_and_run(async |current_task| {
                ...
                // Test incorrect callback.
                {
                    ...
                    assert!(
                        output_buffer
                            .write_each(&mut |data| Ok(data.len() + 1))
                            .is_err()
                    );
                }
    ...
    fn test_vec_input_buffer() {
        ...
        assert_eq!(
            input_buffer
                .read_exact(slice_to_maybe_uninit_mut(&mut buffer))
                .expect("read"),
            5
        );
