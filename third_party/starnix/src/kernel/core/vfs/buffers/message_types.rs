...
impl AncillaryData {
    ...
    pub fn from_cmsg(current_task: &CurrentTask, message: ControlMsg) -> Result<Self, Errno> {
        match (message.level, message.r#type) {
            (SOL_SOCKET, SCM_RIGHTS | SCM_CREDENTIALS) => Ok(AncillaryData::Unix(
                UnixControlData::new(current_task, message)?,
            )),
            (SOL_IP, IP_TOS) => Ok(AncillaryData::Ip(syncio::ControlMessage::IpTos(
                u8::read_from_prefix(&message.data[..])
            ...
            (SOL_IP, IP_RECVORIGDSTADDR) => Ok(AncillaryData::Ip(
                syncio::ControlMessage::IpRecvOrigDstAddr(
            ...
            (level, type_) => {
                error!(
                    EINVAL,
                    format!("invalid cmsg_level/type: 0x{:x}/0x{:x}", level, type_)
                )
            }
        }
    }
    ...
    pub fn into_controlmsg(
        ...
        match self {
            ...
            AncillaryData::Ip(syncio::ControlMessage::IpTos(value)) => Ok(Some(ControlMsg::new(
                SOL_IP,
                IP_TOS,
                value.as_bytes().to_vec(),
            ))),
            AncillaryData::Ip(syncio::ControlMessage::IpTtl(value)) => Ok(Some(ControlMsg::new(
                SOL_IP,
                IP_TTL,
                (value as c_int).as_bytes().to_vec(),
            ))),
            AncillaryData::Ip(syncio::ControlMessage::IpRecvOrigDstAddr(value)) => Ok(Some(
                ControlMsg::new(SOL_IP, IP_RECVORIGDSTADDR, value.as_bytes().to_vec()),
            )),
            ...
            AncillaryData::Ip(syncio::ControlMessage::IpPacketInfo {
                ...
                let pktinfo = uapi::in_pktinfo {
                    ipi_ifindex: iface,
                    ipi_spec_dst: uapi::in_addr {
                        s_addr: u32::from_ne_bytes(local_addr),
                    },
                    ipi_addr: uapi::in_addr {
            ...
            AncillaryData::Ip(syncio::ControlMessage::Ipv6PacketInfo { iface, local_addr }) => {
                let pktinfo = uapi::in6_pktinfo {
                    ipi6_addr: uapi::in6_addr {
                        in6_u: uapi::in6_addr__bindgen_ty_1 {
                            u6_addr8: local_addr,
                        },
                    },
    ...
    pub fn into_bytes(
        ...
        if space_available < header_size + minimum_data_size {
            return Ok(AncillaryDataConvertedToBytes {
                bytes: vec![],
                truncated: true,
            });
        }
            ...
            SCM_RIGHTS => {
                ...
                let files = (0..num_file_descriptors * bytes_per_file_descriptor)
                    ...
                    .map(|fd| {
                        current_task
                            .files()
                            .get_allowing_opath(FdNumber::from_raw(fd))
                    })
...
mod tests {
    ...
    fn make_rights(current_task: &CurrentTask, n: usize) -> AncillaryData {
        let files: Vec<_> = (0..n)
            .map(|i| open_file(current_task, &format!("file{i}")))
            .collect();
        AncillaryData::Unix(UnixControlData::Rights(files))
    }
