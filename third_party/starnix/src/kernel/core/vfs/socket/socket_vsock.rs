...
impl SocketOps for VsockSocket {
    ...
    fn close(&self, _current_task: &CurrentTask, socket: &Socket) {
        ...
        self.shutdown(
            socket,
            SocketShutdownFlags::READ | SocketShutdownFlags::WRITE,
        )
        .unwrap();
    }
...
mod tests {
    ...
    async fn test_vsock_socket() {
        spawn_kernel_and_run(async |current_task| {
            ...
            listen_socket
                .listen(&current_task, 10)
                .expect("Failed to listen.");

            let listen_socket = vsock_ns
                .lookup(&VSOCK_PORT)
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            let closure = move |current_task: &CurrentTask| {
                let bytes_read = socket_clone
                    .read(current_task, &mut VecOutputBuffer::new(XFER_SIZE))
                    .unwrap();
            ...
            socket_file
                .write(&current_task, &mut VecInputBuffer::new(&[0; XFER_SIZE]))
                .unwrap();
    ...
    async fn test_vsock_poll() {
        spawn_kernel_and_run(async |current_task| {
            ...
            epoll_file
                .add(&current_task, &socket, &epoll_object, event)
