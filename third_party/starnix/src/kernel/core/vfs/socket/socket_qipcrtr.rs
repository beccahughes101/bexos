...
fn connect_to_connector() -> Result<fqrtr::QrtrConnectorSynchronousProxy, anyhow::Error> {
    ...
    Ok(fqrtr::QrtrConnectorSynchronousProxy::from_channel(
        client_end,
    ))
}
...
mod tests {
    ...
    fn mock_qipcrtr_socket() -> (
        QipcrtrSocket,
        fidl::endpoints::ServerEnd<fqrtr::QrtrClientConnectionMarker>,
    ) {
    ...
    async fn test_qipcrtr_sockopt() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Test SO_SNDBUF
            let sndbuf = socket_obj
                .getsockopt(&current_task, SOL_SOCKET, SO_SNDBUF, 4)
                .unwrap();
            ...
            // Test SO_RCVBUF
            let rcvbuf = socket_obj
                .getsockopt(&current_task, SOL_SOCKET, SO_RCVBUF, 4)
                .unwrap();
    ...
    async fn test_qipcrtr_read_write() {
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            let written = socket_obj
                .write(&current_task, &mut input, &mut None, &mut vec![])
                .unwrap();
            assert_eq!(written, 5);
            ...
            let info = socket_obj
                .read(&current_task, &mut output, SocketMessageFlags::empty())
                .unwrap();
