...
impl ZxioBackedSocket {
    ...
    fn sendmsg(
        ...
        {
            self.run_sockaddr_ebpf(socket, current_task, SockAddrOp::UdpSendMsg, &addr)?;
        }
        ...
        let sent_bytes = match data.peek_all_segments_as_iovecs() {
            Ok(mut iovecs) => {
                ...
                let ranges = iovecs
                    .as_ref()
                    .iter()
                    .filter(|iovec| iovec.iov_len > 0)
                    .map(|iovec| {
                        (
                            UserAddress::from_ptr(iovec.iov_base as usize),
                            Some(iovec.iov_len),
                        )
                    });
                ...
                Some(map_errors(self.zxio.sendmsg(
                    &mut addr,
                    &mut iovecs,
                    &cmsgs,
                    flags,
    ...
    fn recvmsg(
        ...
        if matches!(
            ...
            self.run_sockaddr_ebpf(socket, current_task, SockAddrOp::UdpRecvMsg, &info.address)?;
        }
...
impl SocketOps for ZxioBackedSocket {
    ...
    fn connect(
        ...
        match peer {
            ...
            ) => self.run_sockaddr_ebpf(socket, current_task, SockAddrOp::Connect, address)?,
            _ => (),
        };
    ...
    fn accept(&self, socket: &Socket, current_task: &CurrentTask) -> Result<SocketHandle, Errno> {
        ...
        if let (Ok(parent_cookie), Ok(child_cookie)) = (parent_cookie, child_cookie) {
            current_task
                .kernel()
                .ebpf_state
                .clone_sk_storage_entries(parent_cookie, child_cookie);
        }
    ...
    fn bind(
        ...
        self.run_sockaddr_ebpf(socket, current_task, SockAddrOp::Bind, &socket_address)?;
    ...
    fn read(
        ...
        let mut info = self.recvmsg(socket, current_task, data, flags)?;
    ...
    fn write(
        ...
        let dest_address = if socket.socket_type == SocketType::Stream {
            &None
        } else {
            dest_address
        };
    ...
    fn close(&self, current_task: &CurrentTask, socket: &Socket) {
        if matches!(socket.domain, SocketDomain::Inet | SocketDomain::Inet6) {
            ...
            // socket release.
            let _: SockProgramResult = current_task
                .kernel()
                .ebpf_state
                .attachments
                .root_cgroup()
        ...
        if let Ok(cookie) = cookie {
            current_task
                .kernel()
                .ebpf_state
                .remove_sk_storage_entries(cookie);
        }
    }
    ...
    fn setsockopt(
        ...
        match (level, optname) {
            ...
            (SOL_IP, IP_MULTICAST_ALL) => {
                track_stub!(
                    TODO("https://fxbug.dev/404596095"),
                    "SOL_IP.IP_MULTICAST_ALL"
                );
                return Ok(());
            }
    ...
    fn getsockopt(
        ...
        match (level, optname) {
            ...
            (SOL_SOCKET, SO_MARK) => {
                ...
                let mark = if socket_mark.is_present {
                    socket_mark.value
                } else {
                    0
                };
            ...
            (SOL_SOCKET, SO_COOKIE) => self
                .get_socket_cookie()
                .map(|cookie| cookie.as_bytes().to_owned()),
            (SOL_IP, IP_MULTICAST_ALL) => {
                track_stub!(
                    TODO("https://fxbug.dev/404596095"),
                    "SOL_IP.IP_MULTICAST_ALL"
...
mod tokens_store {
    ...
    impl TokenCollection {
        fn new() -> Arc<Self> {
            Arc::new(Self {
                sharing_domain_token: OnceLock::new(),
            })
        }
        ...
        fn get_handles_ref_count(&self) -> u32 {
            self.sharing_domain_token
                ...
                .map(|token| {
                    token
                        .count_info()
                        .expect("ZX_INFO_HANDLE_COUNT query failed")
                        .handle_count
                        - 1
                })
                .unwrap_or(0)
    ...
    impl<H: SocketTokenStoreHost> UidEntry<H> {
        ...
        fn get_state(&self) -> UidEntryState {
            match (
                self.tokens.get_handles_ref_count(),
                self.weak_resolver.strong_count(),
            ) {
    ...
    impl<H: SocketTokenStoreHost> Drop for SocketTokenResolver<H> {
        fn drop(&mut self) {
            if let Some(host) = self.host.upgrade() {
                host.get_socket_tokens_store()
                    .on_resolver_dropped(&host, self.uid);
            }
        }
    }
    ...
    mod tests {
        ...
        impl TestSocketTokenStoreHost {
            fn new() -> Arc<Self> {
                Arc::new(Self {
                    socket_tokens_store: SocketTokensStore::default(),
                })
            }
        }
...
const_assert_eq!(
    syncio::zxio::SO_SECURITY_AUTHENTICATION,
    uapi::SO_SECURITY_AUTHENTICATION
);
const_assert_eq!(
...
const_assert_eq!(
    syncio::zxio::SO_ATTACH_REUSEPORT_CBPF,
    uapi::SO_ATTACH_REUSEPORT_CBPF
);
const_assert_eq!(
    syncio::zxio::SO_ATTACH_REUSEPORT_EBPF,
    uapi::SO_ATTACH_REUSEPORT_EBPF
);
...
const_assert_eq!(
    syncio::zxio::SO_DETACH_REUSEPORT_BPF,
    uapi::SO_DETACH_REUSEPORT_BPF
);
...
const_assert_eq!(
    syncio::zxio::IP_BIND_ADDRESS_NO_PORT,
    uapi::IP_BIND_ADDRESS_NO_PORT
);
...
const_assert_eq!(
    syncio::zxio::IP_ADD_SOURCE_MEMBERSHIP,
    uapi::IP_ADD_SOURCE_MEMBERSHIP
);
const_assert_eq!(
    syncio::zxio::IP_DROP_SOURCE_MEMBERSHIP,
    uapi::IP_DROP_SOURCE_MEMBERSHIP
);
...
const_assert_eq!(
    syncio::zxio::IP_PMTUDISC_INTERFACE,
    uapi::IP_PMTUDISC_INTERFACE
);
const_assert_eq!(syncio::zxio::IP_PMTUDISC_OMIT, uapi::IP_PMTUDISC_OMIT);
const_assert_eq!(
    syncio::zxio::IP_DEFAULT_MULTICAST_TTL,
    uapi::IP_DEFAULT_MULTICAST_TTL
);
const_assert_eq!(
    syncio::zxio::IP_DEFAULT_MULTICAST_LOOP,
    uapi::IP_DEFAULT_MULTICAST_LOOP
);
...
const_assert_eq!(
    syncio::zxio::IPV6_ADDR_PREFERENCES,
    uapi::IPV6_ADDR_PREFERENCES
);
...
const_assert_eq!(
    syncio::zxio::IPV6_RECVORIGDSTADDR,
    uapi::IPV6_RECVORIGDSTADDR
);
...
const_assert_eq!(
    syncio::zxio::IPV6_DROP_MEMBERSHIP,
    uapi::IPV6_DROP_MEMBERSHIP
);
...
const_assert_eq!(
    syncio::zxio::IPV6_PMTUDISC_INTERFACE,
    uapi::IPV6_PMTUDISC_INTERFACE
);
...
const_assert_eq!(
    syncio::zxio::IPV6_PREFER_SRC_PUBLIC,
    uapi::IPV6_PREFER_SRC_PUBLIC
);
const_assert_eq!(
...
const_assert_eq!(
    syncio::zxio::IPV6_PREFER_SRC_HOME,
    uapi::IPV6_PREFER_SRC_HOME
);
const_assert_eq!(syncio::zxio::IPV6_PREFER_SRC_CGA, uapi::IPV6_PREFER_SRC_CGA);
const_assert_eq!(
    syncio::zxio::IPV6_PREFER_SRC_NONCGA,
    uapi::IPV6_PREFER_SRC_NONCGA
);
