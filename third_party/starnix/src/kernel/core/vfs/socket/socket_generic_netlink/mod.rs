...
impl<S: Sender<GenericMessage>> GenericNetlinkServerState<S> {
    ...
    fn get_family(&self, family_id: u16) -> Option<Arc<dyn GenericNetlinkFamily<S>>> {
        ...
        {
            Some(Arc::clone(
                &self.families[(family_id - MIN_FAMILY_ID) as usize],
            ))
        } else {
            None
        }
    ...
    fn handle_ctrl_message(
        ...
        match genl_ctrl.cmd {
            ...
            GenlCtrlCmd::NewMcastGrp => {
                track_stub!(
                    TODO("https://fxbug.dev/297431602"),
                    "NetlinkCtrlNewMcastGrp"
                )
            }
            GenlCtrlCmd::DelMcastGrp => {
                track_stub!(
                    TODO("https://fxbug.dev/297431602"),
                    "NetlinkCtrlDelMcastGrp"
                )
            }
            GenlCtrlCmd::GetMcastGrp => {
                track_stub!(
                    TODO("https://fxbug.dev/297431602"),
                    "NetlinkCtrlGetMcastGrp"
                )
            }
...
impl<S: Sender<GenericMessage>> GenericNetlinkServer<S> {
    fn new(new_family_sender: mpsc::UnboundedSender<Arc<dyn GenericNetlinkFamily<S>>>) -> Self {
        Self {
            state: Arc::new(GenericNetlinkServerState::new(new_family_sender).into()),
        }
    }
    ...
    async fn handle_generic_message(
        ...
        match req {
            GenericMessage::Ctrl(ctrl_message) => {
                self.state
                    .lock()
                    .handle_ctrl_message(netlink_header, ctrl_message, sender)
            }
            GenericMessage::Other {
                family: family_id,
                payload,
            } => {
    ...
    async fn run_generic_netlink_client(self, mut client: GenericNetlinkClient<S>) {
        ...
        loop {
            match client.receiver.next().await {
                Some(message) => {
                    self.handle_generic_message(message, &mut client.sender)
                        .await
                }
                None => {
    ...
    async fn pipe_multicast_traffic_for_family(self, family: Arc<dyn GenericNetlinkFamily<S>>) {
        ...
        let family_id = *self
            ...
            .family_ids
            .get(&family_name)
            .expect("Failed to get family id");
        for mcast_group in family.multicast_groups() {
            ...
            unordered.push(Box::pin(
                self.pipe_single_multicast_group(mcast_group_id, receiver),
            ));
        }
...
fn run_generic_netlink_worker_internal<S: Sender<GenericMessage>>(
    ...
    let GenericNetlinkWorkerParams {
        server,
        new_client_receiver,
        new_family_receiver,
    } = params;
    ...
    let multicast_fut = new_family_receiver.for_each_concurrent(None, move |family| {
        server_clone
            .clone()
            .pipe_multicast_traffic_for_family(family)
    });
    let new_client_fut = new_client_receiver.for_each_concurrent(None, move |client| {
        server.clone().run_generic_netlink_client(client)
    });
...
impl<S: Sender<GenericMessage>> GenericNetlink<S> {
    pub fn new() -> (Self, GenericNetlinkWorkerParams<S>) {
        ...
        let generic_netlink = Self {
            server: server.clone(),
            new_client_sender,
        };
        let worker_params = GenericNetlinkWorkerParams {
            server,
            new_client_receiver,
            new_family_receiver,
        };
    ...
    pub fn new_generic_client(
        ...
        let handle = GenericNetlinkClientHandle {
            client_id,
            server: self.server.clone(),
        };
        let new_client = GenericNetlinkClient {
            client_id,
            sender,
            receiver,
        };
...
mod tests {
    ...
    fn getfamily_request() -> NetlinkMessage<GenericMessage> {
        ...
        let mut genl_message = GenlMessage::new(
            GenlHeader { cmd: 0, version: 0 },
            getfamily_ctrl,
            GENL_ID_CTRL,
        );
        genl_message.finalize();
    ...
    fn netlink_with_test_family() -> (
        ...
        netlink
            ...
            .state
            .lock()
            .add_family(Arc::clone(&test_family) as _);
    ...
    fn new_client(
        ...
        let sender = TestSender {
            messages: messages_to_client.clone(),
        };
        let client_handle = netlink
            .new_generic_client(sender, receiver)
            .expect("Failed to add new generic client");
    ...
    fn test_ctrl_getfamily_missing() {
        ...
        sender
            .unbounded_send(getfamily_request())
            .expect("Failed to send getfamily request");
    ...
    fn test_ctrl_getfamily() {
        ...
        assert!(
            ctrl_payload
                .nlas
                .iter()
                .any(|nla| matches!(nla, GenlCtrlAttrs::FamilyId(_)))
        );
        let multicast_groups = ctrl_payload
            .nlas
            .iter()
            .filter_map(|nla| {
                if let GenlCtrlAttrs::McastGroups(vec) = nla {
                    Some(vec)
                } else {
                    None
                }
        ...
        assert!(multicast_groups.iter().any(|group| {
            group
                .iter()
                .any(|attr| matches!(attr, McastGrpAttrs::Id(_)));
            group
                .iter()
                .any(|attr| *attr == McastGrpAttrs::Name(MCAST_GROUP_1.into()))
        }));
        assert!(multicast_groups.iter().any(|group| {
            group
                .iter()
            ...
            group
                .iter()
                .any(|attr| *attr == McastGrpAttrs::Name(MCAST_GROUP_2.into()))
        }));
    }
    ...
    fn test_send_family_message() {
        ...
        netlink_message.finalize();
        sender
            .unbounded_send(netlink_message)
            .expect("Failed to send test family message");
    ...
    fn test_server_gets_multicast_messages() {
        ...
        assert!(
            test_family
                ...
                .get(&scope, MCAST_GROUP_1)
                .is_some()
        );
        assert!(
            test_family
                .multicast_message_sinks
                .get(&scope, MCAST_GROUP_2)
                .is_some()
        );
    }
    ...
    fn test_multicast_subscriptions() {
        ...
        client_handle_1
            .add_membership(mcast_group_1_id)
            .expect("add_membership failed");
        client_handle_2
            .add_membership(mcast_group_1_id)
            .expect("add_membership failed");
