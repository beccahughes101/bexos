...
pub trait SocketOps: Send + Sync + AsAny {
    ...
    fn listen(&self, socket: &Socket, backlog: i32, credentials: uapi::ucred) -> Result<(), Errno>;
...
impl SockOptValue {
    ...
    pub fn read<T: FromBytes>(&self, current_task: &CurrentTask) -> Result<T, Errno> {
        match self {
            Self::Value(buffer) => T::read_from_prefix(&buffer)
    ...
    pub fn read_bytes(
        ...
        match self {
            ...
            Self::User(user_buffer) => {
                ...
                current_task.read_buffer(&UserBuffer {
                    address: user_buffer.address,
                    length: bytes,
                })
            }
        }
...
impl Socket {
    ...
    pub fn setsockopt(
        ...
        let read_timeval = || {
            ...
            Ok(if duration == zx::MonotonicDuration::default() {
                None
            } else {
                Some(duration)
            })
        ...
        match (level, optname) {
            ...
            _ => self
                .ops
                .setsockopt(self, current_task, level, optname, optval)?,
        }
        Ok(())
    }
    ...
    pub fn getsockopt(
        ...
        let value = match level {
            SOL_SOCKET => match optname {
                ...
                _ => self
                    .ops
                    .getsockopt(self, current_task, level, optname, optlen)?,
            },
            _ => self
                .ops
                .getsockopt(self, current_task, level, optname, optlen)?,
    ...
    pub fn ioctl(
        ...
        let res = super::netlink_ioctl::netlink_ioctl(current_task, request, arg);
        match &res {
    ...
    pub fn listen(&self, current_task: &CurrentTask, backlog: i32) -> Result<(), Errno> {
        ...
        let max_connections = current_task
            .kernel()
            .system_limits
            .socket
            .max_connections
    ...
    pub fn write(
        ...
        self.ops
            .write(self, current_task, data, dest_address, ancillary_data)
    }
...
mod tests {
    ...
    async fn test_dgram_socket() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let opt_buf = UserBuffer {
                address: user_address,
                length: opt_size,
            };
            rec_dgram
                .setsockopt(&current_task, SOL_SOCKET, SO_PASSCRED, opt_buf.into())
                .unwrap();

            rec_dgram
                .bind(&current_task, bind_address)
