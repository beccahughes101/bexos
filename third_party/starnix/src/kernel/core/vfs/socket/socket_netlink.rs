...
impl NetlinkSocketInner {
    ...
    fn read_datagram(
        ...
        if self.passcred {
            track_stub!(
                TODO("https://fxbug.dev/297373991"),
                "SCM_CREDENTIALS/SO_PASSCRED"
            );
            info.ancillary_data
                .push(AncillaryData::Unix(UnixControlData::unknown_creds()));
        }
    ...
    fn write_to_queue(
        ...
        let socket_address = match address {
            ...
            None => self
                .address
                .as_ref()
                .map(|addr| SocketAddress::Netlink(addr.clone())),
        };
        let bytes_written =
            self.receive_buffer
                .write_datagram(data, socket_address, ancillary_data)?;
        if bytes_written > 0 {
    ...
    fn getsockopt(&self, level: u32, optname: u32) -> Result<Vec<u8>, Errno> {
        let opt_value = match level {
            SOL_SOCKET => match optname {
                ...
                SO_RCVBUF => (self.receive_buffer.capacity() as socklen_t)
                    .to_ne_bytes()
                    .to_vec(),
                SO_SNDBUFFORCE => (self.send_buf_size as socklen_t).to_ne_bytes().to_vec(),
                SO_RCVBUFFORCE => (self.receive_buffer.capacity() as socklen_t)
                    .to_ne_bytes()
                    .to_vec(),
...
impl NflogNetlinkSocket {
    fn new(inner: Arc<LockDepMutex<NetlinkSocketInner, NetlinkSocketInnerLock>>) -> Self {
        NFLOG_LISTENERS.lock().unwrap().push(NflogListener {
...
impl Drop for NflogNetlinkSocket {
    fn drop(&mut self) {
        ...
        listeners.retain(|l| {
            if let Some(arc) = l.inner.upgrade() {
                !Arc::ptr_eq(&arc, &self.inner)
            } else {
                false
            }
        });
...
impl StubbedNetlinkSocket {
    pub fn new(family: NetlinkFamily) -> Self {
        ...
        StubbedNetlinkSocket {
            inner: NetlinkSocketInner::new(family).into(),
        }
    }
...
impl SocketOps for StubbedNetlinkSocket {
    ...
    fn write(
        ...
        if destination.groups != 0 {
            track_stub!(
                TODO("https://fxbug.dev/322874956"),
                "StubbedNetlinkSockets multicasting"
            );
            return Ok(data.drain());
        }

        self.lock()
            .write_to_queue(data, Some(NetlinkAddress::default()), ancillary_data)
    }
    ...
    fn shutdown(&self, _socket: &Socket, _how: SocketShutdownFlags) -> Result<(), Errno> {
        track_stub!(
            TODO("https://fxbug.dev/322875507"),
            "StubbedNetlinkSocket::shutdown"
        );
        Ok(())
    }
...
impl SocketOps for UEventNetlinkSocket {
    ...
    fn shutdown(&self, _socket: &Socket, _how: SocketShutdownFlags) -> Result<(), Errno> {
        track_stub!(
            TODO("https://fxbug.dev/322875507"),
            "UEventNetlinkSocket::shutdown"
        );
        Ok(())
    }

    fn close(&self, current_task: &CurrentTask, _socket: &Socket) {
...
fn new_route_socket(kernel: &Arc<Kernel>) -> Result<NetlinkSocket<NetlinkRouteClient>, Errno> {
    let inner = Arc::new(LockDepMutex::new(NetlinkSocketInner::new(
        NetlinkFamily::Route,
    )));
    ...
    fn setsockopt(
        ...
        match (level, optname) {
            ...
            _ => self
                .inner
                .lock()
                .setsockopt(current_task, level, optname, optval),
        }
    }
}
...
impl AuditNetlinkClient {
    fn new(audit_logger: Arc<AuditLogger>) -> Self {
        Self {
            ...
            audit_response: Default::default(),
        }
    }
    ...
    fn read_audit_log(self: &Arc<Self>) -> Option<Vec<u8>> {
        if let Some(AuditMessage {
            audit_type,
            message,
        }) = self.audit_logger.read_audit_log(self)
        {
    ...
    fn process_user_audit(
        ...
        self.audit_logger
            .audit_log(nl_hdr.message_type, move || audit_msg);
        Ok(AuditNetlinkClient::build_audit_ack(Ok(()), nl_hdr))
    }
    ...
    fn build_audit_nlmsg(
        ...
        let nl_payload = NetlinkPayload::InnerMessage(GenericMessage::Other {
            family: msg_type,
            payload,
        });
...
impl AuditNetlinkSocket {
    pub fn new(kernel: &Kernel) -> Result<Self, Errno> {
        ...
        Ok(Self {
            audit_client: Arc::new(AuditNetlinkClient::new(kernel.audit_logger())),
        })
    }
}
...
mod tests {
    ...
    fn test_netlink_to_client_sender(sufficient_capacity: bool) {
        ...
        assert_eq!(
            address,
            Some(SocketAddress::Netlink(NetlinkAddress {
                pid: 0,
                groups: 1 << MODERN_GROUP
            }))
        );
        ...
        assert_eq!(
            socket_inner.lock().receive_buffer.capacity(),
            final_queue_size
        );
    }
    ...
    async fn test_set_get_snd_rcv_buf() {
        crate::testing::spawn_kernel_and_run_sync(|current_task| {
            ...
            assert_eq!(
                getsockopt_u32(&socket, SOL_SOCKET, SO_SNDBUF),
                expected_default
            );
            assert_eq!(
                getsockopt_u32(&socket, SOL_SOCKET, SO_RCVBUF),
                expected_default
            );
