...
fn parse_socket_domain(domain: u32) -> Result<SocketDomain, Errno> {
    SocketDomain::from_raw(domain.try_into().map_err(|_| errno!(EAFNOSUPPORT))?).ok_or_else(|| {
        track_stub!(
            TODO("https://fxbug.dev/322875074"),
            "parse socket domain",
            domain
        );
        errno!(EAFNOSUPPORT)
...
fn parse_socket_type(domain: SocketDomain, socket_type: u32) -> Result<SocketType, Errno> {
    let socket_type = SocketType::from_raw(socket_type & 0xf).ok_or_else(|| {
        track_stub!(
            TODO("https://fxbug.dev/322875418"),
            "parse socket type",
            socket_type
        );
        errno!(EINVAL)
    ...
    Ok(
        if domain == SocketDomain::Unix && socket_type == SocketType::Raw {
            SocketType::Datagram
        } else {
            socket_type
        },
        ...
        SocketAddress::Unix(mut name) => {
            ...
            if name[0] == b'\0' {
                current_task
                    ...
                    .bind(current_task, name, socket)?;
            } else {
                let mode = file.node().info().mode;
                let mode = current_task
                    .fs()
                    .apply_umask(mode)
                    .with_type(FileMode::IFSOCK);
...
fn recvmsg_internal_with_header(
    ...
    for ancillary_data in info.ancillary_data {
        ...
        let conversion = ancillary_data.into_bytes(current_task, flags, space_available)?;
        ...
        current_task.write_memory(
            (message_header.control + aligned_offset)?,
            &conversion.bytes,
        )?;
...
fn sendmsg_internal_with_header(
    ...
    let header_size = CMsgHdrPtr::size_of_object_for(current_task);
    loop {
        let space = message_header
            .control_len
            .saturating_sub(next_message_offset);
        if space < header_size {
            break;
        }
        let cmsg_ref = CMsgHdrPtr::new(
            current_task,
            (message_header.control + next_message_offset)?,
        );
...
pub fn sys_getsockopt(
    ...
    let result = if socket.domain.is_inet() && IpTables::can_handle_getsockopt(level, optname) {
        current_task
            .kernel()
            .iptables()
            .getsockopt(current_task, socket, optname, optval.clone())
    } else {
...
pub fn sys_setsockopt(
    ...
    let user_opt = UserBuffer {
        address: user_optval,
        length: optlen as usize,
    };
    ...
    if socket.domain.is_inet() && IpTables::can_handle_setsockopt(level, optname) {
        current_task
            .kernel()
            .iptables()
            .setsockopt(current_task, socket, optname, optval)
    } else {
