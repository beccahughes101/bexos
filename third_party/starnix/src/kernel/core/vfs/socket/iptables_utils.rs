...
impl TryFrom<ipt_replace> for ReplaceInfo {
    ...
    fn try_from(replace: ipt_replace) -> Result<Self, Self::Error> {
        let valid_hooks = NfIpHooks::from_bits(replace.valid_hooks).ok_or(
            IpTableParseError::InvalidHookBits {
                hooks: replace.valid_hooks,
            },
        )?;
        Ok(Self {
...
impl TryFrom<ipt_entry> for EntryInfo {
    ...
    fn try_from(entry: ipt_entry) -> Result<Self, Self::Error> {
        ...
        if target_offset < size_of::<ipt_entry>() {
            return Err(IpTableParseError::TargetOffsetTooSmall {
        ...
        if next_offset < size_of::<ipt_entry>() {
            return Err(IpTableParseError::NextOffsetTooSmall {
                offset: next_offset,
            });
        }
...
impl IptReplaceParser {
    ...
    fn advance_parse_pos(&mut self, offset: usize) -> Result<(), IpTableParseError> {
        if self.parse_pos + offset > self.bytes.len() {
            return Err(IpTableParseError::AdvanceEndOfBuffer {
                offset,
                position: self.parse_pos,
            });
        }
    ...
    fn parse_entry(&mut self) -> Result<Entry, IpTableParseError> {
        ...
        if self.bytes_since_first_entry() != next_pos {
            return Err(IpTableParseError::InvalidNextOffset {
                offset: entry_info.next_offset,
            });
        }

        assert!(
            self.bytes_since_first_entry() > byte_pos,
            "parse_pos must advance"
        );
    ...
    fn view_as_mark_target(&self, remaining_size: usize) -> Result<Target, IpTableParseError> {
        ...
        Ok(Target::Mark {
            mark: mark_target.mark,
            mask: mark_target.mask,
        })
    }
...
impl IpTable {
    ...
    fn from_parser(
        ...
        for custom_routine in &custom_routines {
            if let Some(first_entry) = custom_routine.entries.first() {
                custom_routine_map.insert(
                    first_entry.byte_pos,
                    custom_routine.routine.id.name.to_owned(),
                );
            } else {
    ...
    fn check_and_remove_last_entry(entries: &mut Vec<Entry>) -> Result<(), IpTableParseError> {
        let last_entry = entries
            .last()
            .ok_or(IpTableParseError::NoTrailingErrorTarget)?;
...
impl InstalledRoutines {
    ...
    fn new_ip(parser: &IptReplaceParser, priority: i32) -> Result<Self, IpTableParseError> {
        let prerouting = if parser
            .replace_info
            .valid_hooks
            .contains(NfIpHooks::PREROUTING)
        {
        ...
        let postrouting = if parser
            .replace_info
            .valid_hooks
            .contains(NfIpHooks::POSTROUTING)
        {
            Some(InstalledRoutine {
                routine: fnet_filter_ext::Routine {
                    id: fnet_filter_ext::RoutineId {
    ...
    fn routines(&self) -> Vec<fnet_filter_ext::Routine> {
        self.iter()
            .map(|installed_routine| installed_routine.routine.clone())
            .collect()
    }
}
...
impl Entry {
    ...
    fn get_rule_matchers(
        ...
        if self
            .populate_matchers_with_match_extensions(context, &mut matchers)?
            .is_none()
        {
            return Ok(None);
        }
    ...
    fn populate_matchers_with_ipt_ip(
        ...
        if let Some(subnet) = ip_info.src_subnet {
            ...
            matchers.src_addr = Some(fnet_matchers_ext::Address {
                ...
                invert: ip_info
                    .inverse_flags
                    .contains(IptIpInverseFlags::SOURCE_IP_ADDRESS),
            });
        }
        ...
        if let Some(subnet) = ip_info.dst_subnet {
            ...
            matchers.dst_addr = Some(fnet_matchers_ext::Address {
                ...
                invert: ip_info
                    .inverse_flags
                    .contains(IptIpInverseFlags::DESTINATION_IP_ADDRESS),
            });
        }
        ...
        if let Some(ref interface) = ip_info.in_interface {
            if ip_info
                .inverse_flags
                .contains(IptIpInverseFlags::INPUT_INTERFACE)
            {
                track_stub!(
                    TODO("https://fxbug.dev/448203710"),
        ...
        if let Some(ref interface) = ip_info.out_interface {
            if ip_info
                .inverse_flags
                .contains(IptIpInverseFlags::OUTPUT_INTERFACE)
            {
                track_stub!(
                    TODO("https://fxbug.dev/448203710"),
                    "ignored rule-specification",
...
fn ascii_to_string(chars: &[c_char]) -> Result<String, AsciiConversionError> {
    ...
    let c_str =
        CStr::from_bytes_until_nul(&bytes).map_err(|_| AsciiConversionError::NulByteNotFound {
            chars: chars.to_vec(),
        })?;
    // We've verified that all characters are in ASCII range, so the conversion should succeed.
    Ok(c_str
        .to_str()
        .expect("failed CStr to Str conversion")
        .to_owned())
}
...
mod tests {
    ...
    fn extend_with_standard_verdict(bytes: &mut Vec<u8>, verdict: i32) {
        bytes.extend_from_slice(
            xt_entry_target {
                target_size: STANDARD_TARGET_SIZE as u16,
                ..Default::default()
            }
            .as_bytes(),
        );
    ...
    fn ipv4_subnet_test() {
        ...
        assert_eq!(
            ipv4_to_subnet(ipv4_subnet_addr(), ipv4_subnet_mask()),
            Ok(Some(IPV4_SUBNET))
        );
    }
    ...
    fn ipv6_subnet_addr() -> in6_addr {
        ...
        in6_addr {
            in6_u: in6_addr__bindgen_ty_1 { u6_addr8: bytes },
        }
    }
    ...
    fn ipv6_subnet_test() {
        ...
        assert_eq!(
            ipv6_to_subnet(ipv6_subnet_addr(), ipv6_subnet_mask()),
            Ok(Some(IPV6_SUBNET))
        );
    }
    ...
    fn parse_jump_target_test(
        ...
        let expected_rules = [
            ...
            fnet_filter_ext::Rule {
                id: fnet_filter_ext::RuleId {
                    index: 0,
                    routine: filter_custom_routine(&expected_namespace, "chain1")
                        .id
                        .clone(),
                },
                matchers: fnet_filter_ext::Matchers::default(),
            ...
            fnet_filter_ext::Rule {
                id: fnet_filter_ext::RuleId {
                    index: 0,
                    routine: filter_custom_routine(&expected_namespace, "chain2")
                        .id
                        .clone(),
                },
                matchers: fnet_filter_ext::Matchers::default(),
    ...
    fn parse_table_error(bytes: Vec<u8>, expected_error: IpTableParseError) {
        let mut context = TestIpTablesContext::new();
        assert_eq!(
            IpTable::from_ipt_replace(&mut context, bytes).unwrap_err(),
            expected_error
        );
    }
    ...
    fn ipv4_address_test(
        ...
        assert_eq!(
            ipv4_to_subnet(
                in_addr {
                    s_addr: u32::from_be_bytes(be_addr).to_be()
                },
                in_addr {
                    s_addr: u32::from_be_bytes(be_mask).to_be()
                },
            ),
            expected
        );
    ...
    fn ipv6_address_test(
        ...
        assert_eq!(
            ipv6_to_subnet(
                in6_addr {
                    in6_u: in6_addr__bindgen_ty_1 { u6_addr8: be_addr }
                },
                in6_addr {
                    in6_u: in6_addr__bindgen_ty_1 { u6_addr8: be_mask }
                },
            ),
            expected
        );
