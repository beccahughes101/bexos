    ...
    fn write(
        ...
        let write_result = {
            ...
            if socket.socket_type == SocketType::Datagram {
                ...
                ancillary_data.push(AncillaryData::Unix(UnixControlData::Security(
                    context.into(),
                )));
            }
            peer.write(
            ...
            if err.code == ECONNREFUSED && socket.socket_type == SocketType::Datagram {
                ...
                if let UnixSocketState::Connected(ref connected_peer) = inner.state {
                    if Arc::ptr_eq(connected_peer, &peer) {
                        ...
                        self.waiters
                            .notify_fd_events(FdEvents::POLLOUT | FdEvents::POLLHUP);
                    }
                }
            }
        }
        ...
        match level {
            SOL_SOCKET => match optname {
                SO_PEERCRED => Ok(UcredPtr::into_bytes(
                    current_task,
                    self.peer_cred().unwrap_or(ucred {
                        pid: 0,
                        uid: uid_t::MAX,
                        gid: gid_t::MAX,
                    }),
                ...
                SO_ACCEPTCONN =>
                {
                    ...
                    Ok(if self.is_listening(socket) {
                        1u32
                    } else {
                        0u32
                    }
                ...
                SO_SNDBUF => Ok((self.get_send_capacity() as socklen_t)
                    .to_ne_bytes()
                    .to_vec()),
                SO_RCVBUF => Ok((self.get_receive_capacity() as socklen_t)
                    .to_ne_bytes()
                    .to_vec()),
...
impl UnixSocketInner {
    ...
    fn write(
        ...
        let bytes_written = if socket_type == SocketType::Stream {
            self.messages
                .write_stream_with_filter(data, address, ancillary_data, filter)?
        } else {
            self.messages
                .write_datagram_with_filter(data, address, ancillary_data, filter)?
        };
        Ok(bytes_written)
    }
}
...
impl<'a> PacketWithLoadBytes for &'a mut SkBuf {
    fn load_bytes_relative(
        ...
        track_stub!(
            TODO("https://fxbug.dev/385015056"),
            "bpf_load_bytes_relative"
        );
        -1
    }
}
...
mod tests {
    ...
    async fn test_socket_send_capacity() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_memory(user_address, &send_capacity.to_ne_bytes())
                .unwrap();
            ...
            let opt_bytes = server_socket
                .getsockopt(&current_task, SOL_SOCKET, SO_SNDBUF, 0)
                .unwrap();
