...
impl EncodedEntry {
    ...
    fn new(entry: FdTableEntry) -> Self {
        Self {
            value: AtomicUsize::new(Self::encode(entry.file, entry.flags)),
        }
    }
...
impl<'a> FdTableEntryGuard<'a> {
    ...
    fn to_entry(&self) -> Option<FdTableEntry> {
        self.to_handle().map(|file| FdTableEntry {
            file,
            flags: self.flags,
        })
    }
...
/// A file descriptor table which is shared between tasks.
///
/// # Thread Safety
///
...
impl FdTable {
    ...
    pub fn get_allowing_opath(&self, fd: FdNumber) -> Result<FileHandle, Errno> {
        self.get_allowing_opath_with_flags(fd)
            .map(|(file, _flags)| file)
    }
    ...
    pub fn close(&self, fd: FdNumber) -> Result<(), Errno> {
        let mut state = self.write_active()?;
        let scope = RcuReadScope::new();
        if state.remove_entry(&scope, &fd) {
            Ok(())
        } else {
            error!(EBADF)
        }
    ...
    pub fn get_all_fds(&self) -> Vec<FdNumber> {
        ...
        view.slice
            ...
            .filter_map(|(index, encoded_entry)| {
                if encoded_entry.is_none() {
                    None
                } else {
                    Some(FdNumber::from_raw(index as i32))
                }
            })
            .collect()
    }
    ...
    pub fn remap<F: Fn(&FileHandle) -> Option<FileHandle>>(
        &self,
...
mod test {
    ...
    fn add(
        ...
        files.add(current_task, file, FdFlags::empty())
    }
    ...
    async fn test_fd_table_install() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                files.get(FdNumber::from_raw(fd1.raw() + 1)).map(|_| ()),
                error!(EBADF)
            );
        })
        .await;
    ...
    async fn test_fd_table_fork() {
        spawn_kernel_and_run(async |current_task| {
            ...
            files
                .set_fd_flags_allowing_opath(fd0, FdFlags::CLOEXEC)
                .unwrap();
            assert_eq!(
                FdFlags::CLOEXEC,
                files.get_fd_flags_allowing_opath(fd0).unwrap()
            );
            assert_ne!(
                FdFlags::CLOEXEC,
                forked.get_fd_flags_allowing_opath(fd0).unwrap()
            );
        })
        .await;
    }
