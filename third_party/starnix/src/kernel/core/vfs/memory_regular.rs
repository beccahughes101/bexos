...
impl FsNodeOps for MemoryRegularNode {
    ...
    fn create_file_ops(
        ...
        if flags.contains(OpenFlags::TRUNC) {
            ...
            node.write_guard_state
                .lock()
                .check_no_seal(SealFlags::SHRINK)?;
        }
        ...
        let scoped_memory = Arc::new(
            self.memory
                .duplicate_handle(desired_rights)
...
impl MemoryRegularFile {
    pub fn read(
        ...
        let actual = {
            ...
            if offset < file_length {
                let to_read = if file_length < offset + want_read {
                    file_length - offset
                } else {
                    want_read
                };
                let buf = memory
                    .read_to_vec(offset as u64, to_read as u64)
                    .map_err(|_| errno!(EIO))?;
                drop(info);
                data.write_all(&buf[..])?;
                to_read
    ...
    pub fn write(
        ...
        let result = file.node().update_info(|info| {
            ...
            memory
                .write(&buf[..want_write], offset as u64)
                .map_err(|_| errno!(EIO))?;
...
impl FileOps for MemoryRegularFile {
    ...
    fn ioctl(
        ...
        match request {
            ASHMEM_GET_SIZE => {
                track_stub!(
                    TODO("https://fxbug.dev/389102161"),
                    "ashmem get_size on memfd"
                );
