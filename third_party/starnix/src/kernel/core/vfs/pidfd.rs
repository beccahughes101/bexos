...
fn get_memory_manager_drop_event(pid: &Pid) -> Option<zx::EventPair> {
    ...
    task.ok()
        .and_then(|task| task.mm().ok())
        .map(|mm| mm.drop_notifier.event())
}
...
async fn wait_for_process_release(
    ...
    while let Some(event) = mm_event {
        if fasync::OnSignals::new(&event, zx::Signals::EVENTPAIR_PEER_CLOSED)
            .await
            .is_err()
        {
            break;
...
async fn monitor_pidfd(
    ...
    let pidfd_closed = std::pin::pin!(fasync::OnSignals::new(
        &local_event,
        zx::Signals::EVENTPAIR_PEER_CLOSED
    ));
