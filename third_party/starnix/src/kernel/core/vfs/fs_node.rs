...
impl FlockOperation {
    pub fn from_flags(operation: u32) -> Result<Self, Errno> {
        ...
        if [LOCK_SH, LOCK_EX, LOCK_UN]
            .iter()
            .filter(|&&o| operation & o == o)
            .count()
            != 1
        {
            return error!(EINVAL);
...
impl FsNode {
    ...
    fn new_internal(
        ...
        let fs_lockdep_type = fs
            .upgrade()
            .map(|fs| fs.fs_lockdep_type())
            .unwrap_or(FsLockDepType::Normal);
    ...
    pub fn fs(&self) -> FileSystemHandle {
        self.fs
            .upgrade()
            .expect("FileSystem did not live long enough")
    }
    ...
    pub fn record_lock(
        ...
        self.ensure_rare_data()
            .record_locks
            .lock(current_task, file, cmd, flock)
    }
    ...
    pub fn create_node(
        ...
        let new_node = if mode.is_dir() {
            ...
            self.ops()
                .mknod(self, current_task, name, mode, dev, owner)?
        };
    ...
    pub fn create_symlink(
        ...
        let new_node = self
            .ops()
            .create_symlink(self, current_task, name, target, owner)?;
    ...
    pub fn link(
        ...
        if let Some(child_rare_data) = child.rare_data.get() {
            if matches!(
                child_rare_data.link_behavior.get(),
                Some(FsNodeLinkBehavior::Disallowed)
            ) {
                return error!(ENOENT);
            }
        }
    ...
    pub fn fallocate_locked(
        ...
        self.ops()
            .allocate(guard, self, current_task, mode, offset, length)?;
        self.update_ctime_mtime();
        Ok(())
    }
        ...
        F: FnOnce(&mut FsNodeInfo) -> Result<(), Errno>,
    {
        ...
        let mut has = zxio_node_attr_has_t {
            ..Default::default()
        };
        ...
        {
            self.ops()
                .update_attributes(self, current_task, &new_info, has)?;
        }
    ...
    pub fn chown(
        ...
        self.update_attributes(current_task, |info| {
            ...
            let (euid, is_in_group) = {
                ...
                (
                    current_creds.euid,
                    group.map(|gid| current_creds.is_in_group(gid)),
                )
            };
    ...
    fn check_xattr_access(
        ...
        let enodata_if_read = |e: Errno| {
            if access == Access::READ && e.code == EPERM {
                errno!(ENODATA)
            } else {
                e
            }
    ...
    pub fn list_xattrs(
        ...
        Ok(self
            .ops()
            .list_xattrs(self, current_task, max_size)?
            .map(|mut v| {
    ...
    pub fn fetch_and_refresh_info(
        ...
        self.ops()
            .fetch_and_refresh_info(self, current_task, &self.info)
    }
    ...
    fn ensure_rare_data(&self) -> &FsNodeRareData {
        self.rare_data
            .get_or_init(|| Box::new(FsNodeRareData::default()))
    }
...
fn check_access(
    ...
    let dac_read_search_access = if mode.is_dir() {
    ...
    async fn open_device_file() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let device_file = current_task
                .open_file("zero".into(), OpenFlags::RDONLY)
                .expect("open device file");
            device_file
                .read(&current_task, &mut buffer)
