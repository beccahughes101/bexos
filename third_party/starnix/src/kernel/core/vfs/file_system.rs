...
impl FileSystem {
    ...
    pub fn new(
        ...
        assert_eq!(
            ops.uses_external_node_ids(),
            node_cache.uses_external_node_ids()
        );
    ...
    pub fn root(&self) -> &DirEntryHandle {
        self.root
            .get()
            .unwrap_or_else(|| panic!("FileSystem {} has no root", self.name()))
    }
    ...
    {
        self.node_cache
            .get_and_validate_or_create_node(node_key, validate_fn, create_fn)
    }
    ...
    pub fn rename(
        ...
        self.ops
            .rename(self, current_task, context, old_name, new_name)
    }
    ...
    pub fn drop_backend_caches(&self) {
        if let Err(e) = self.ops.drop_caches(self) {
            log_warn!(
                "drop_caches failed for filesystem {:?}: {:?}",
                self.name(),
                e
            );
        }
