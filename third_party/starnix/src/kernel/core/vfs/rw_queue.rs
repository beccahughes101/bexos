...
impl<L: LockLevel> RwQueue<L> {
    pub fn read<'a>(
        ...
        if !inner.try_read() {
            ...
            current_task
                .block_until(guard, zx::MonotonicInstant::INFINITE)
                .map_err(|e| {
    ...
    pub fn read_for_lock_ordering<'a>(&'a self) -> RwQueueReadGuard<'a, L> {
        ...
        assert!(
            ...
            "Cannot fail to acquire a read for lock ordering."
        );
    ...
    async fn test_read_in_parallel() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let info = Arc::new(Info {
                ...
                queue: RwQueue::<TestLevel>::default(),
            });
            ...
            let closure1 = move |current_task: &CurrentTask| {
                let guard = info1
                    .queue
                    .read(current_task)
                    .expect("shouldn't be interrupted");
                info1.barrier.wait();
                std::mem::drop(guard);
            };
            let (thread1, req) = SpawnRequestBuilder::new()
                .with_sync_closure(closure1)
            ...
            let (thread2, req) = SpawnRequestBuilder::new()
                .with_sync_closure(closure2)
                .build_with_async_result();
            kernel.kthreads.spawner().spawn_from_request(req);
        ...
        fn spawn_writer(
            ...
            let closure = move |current_task: &CurrentTask| {
                ...
                for _ in 0..count {
                    ...
                    assert_eq!(
                        writer_count, 1,
                        "More than one writer held the lock at once."
                    );
                    assert_eq!(
                        reader_count, 0,
        ...
        fn spawn_reader(
            ...
            let closure = move |current_task: &CurrentTask| {
                ...
                for _ in 0..count {
                    ...
                    assert!(
                        reader_count > 0,
                        "A reader held the lock without being counted."
                    );
                }
            };
            let (result, req) = SpawnRequestBuilder::new()
    ...
    async fn test_thundering_reads_and_writes() {
        spawn_kernel_and_run(async |current_task| {
            ...
            block_on(join_all(threads))
                .into_iter()
                .for_each(|r| r.expect("failed to join thread"));
        })
        .await;
    }
}
