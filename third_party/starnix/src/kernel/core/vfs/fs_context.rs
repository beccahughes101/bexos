...
impl FsContext {
    ...
    pub fn fork(&self) -> Arc<FsContext> {
        ...
        Arc::new(FsContext {
            state: self.state.read().clone().into(),
        })
    }
    ...
    pub fn chroot(&self, current_task: &CurrentTask, name: NamespaceNode) -> Result<(), Errno> {
        name.check_access(current_task, AccessCheck::for_chroot())
...
mod test {
    ...
    async fn test_umask() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                FileMode::from_bits(0o22),
                fs.set_umask(FileMode::from_bits(0o3020))
            );
            assert_eq!(
                FileMode::from_bits(0o646),
                fs.apply_umask(FileMode::from_bits(0o666))
            );
            assert_eq!(
                FileMode::from_bits(0o3646),
                fs.apply_umask(FileMode::from_bits(0o3666))
            );
            assert_eq!(
                FileMode::from_bits(0o20),
                fs.set_umask(FileMode::from_bits(0o11))
            );
        })
        .await;
    }
    ...
    async fn test_chdir() {
        spawn_kernel_and_run_with_pkgfs(async |current_task| {
            ...
            let bin = current_task
                .open_file("bin".into(), OpenFlags::RDONLY)
                .expect("missing bin directory");
            current_task
                .fs()
                .chdir(&current_task, bin.name.to_passive())
            ...
            current_task
                .fs()
                .chdir(&current_task, previous_directory)
                .expect("Failed to chdir");
            assert_eq!("/", current_task.fs().cwd().path_escaping_chroot());
