...
impl TimelineChangeObserver {
    pub fn new(
        ...
        Self {
            timeline_change_counter,
            timeline_change_registration,
        }
    }
    ...
    pub fn reset_timeline_change_counter(&self) -> i64 {
        ...
        if value != 0 {
            ...
            counter
                .add(-value)
                .expect("it is possible to set the counter to zero");
        }
        return value;
    }
    ...
    pub fn set_timeline_change_interest(&mut self, is_interested: bool) {
        ...
        self.timeline_change_registration
            .unbounded_send(is_interested)
            .expect("can send");
    }
}
    ...
    pub fn current_timer_spec(&self) -> itimerspec {
        ...
        let remaining_time = if interval == zx::MonotonicDuration::default() && deadline <= now {
            ...
            timespec_from_duration(
                *deadline
                    .delta(&now)
                    .expect("deadline and now come from same timeline"),
            )
        };

        itimerspec {
            it_interval: timespec_from_duration(interval),
            it_value: remaining_time,
        }
    }
        ...
        file.blocking_op(
            ...
            || {
                ...
                let count: i64 = if tfi.interval > zx::MonotonicDuration::default() {
                    ...
                    // is reached.
                    self.timer
                        .start(current_task, Some(file.weak_handle.clone()), new_deadline)?;
