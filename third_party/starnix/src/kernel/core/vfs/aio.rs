...
impl AioContext {
    pub fn create(
        ...
        let context = Arc::new(AioContext {
            inner: AioContextInner::new(max_operations),
        });
        context
            .inner
            .spawn_worker_for(current_task, WorkerType::Read);
        context
            .inner
            .spawn_worker_for(current_task, WorkerType::Write);
    ...
    pub fn get_events(
        ...
        self.inner
            .get_events(current_task, min_results, max_results, deadline)
    }
...
impl AioContextInner {
    ...
    fn spawn_worker_for(self: &Arc<Self>, current_task: &CurrentTask, worker_type: WorkerType) {
        ...
        let closure = move |current_task: &CurrentTask| {
            current_task.override_creds(creds, || {
                inner.perform_next_action(current_task, worker_type)
            })
        };
    ...
    fn perform_next_action(&self, current_task: &CurrentTask, worker_type: WorkerType) {
        while let Ok(IoAction::Op(op)) = self
            .operations
            .block_until_dequeue(current_task, worker_type)
        {
...
impl TryFrom<u32> for OpType {
    ...
    fn try_from(opcode: u32) -> Result<Self, Self::Error> {
        match opcode {
            ...
            _ => {
                track_stub!(
                    TODO("https://fxbug.dev/297433877"),
                    "io_submit opcode",
                    opcode
                );
                return error!(ENOSYS);
...
impl IoOperation {
    fn new(
        ...
        let file = current_task
            .files()
            .get(FdNumber::from_raw(control_block.aio_fildes as i32))?;
        let op_type = (control_block.aio_lio_opcode as u32).try_into()?;
        let offset = control_block
            .aio_offset
            .try_into()
