...
impl DeviceMetadata {
    pub fn new(devname: FsString, devt: DeviceId, mode: DeviceMode) -> Self {
        Self {
            ...
            devt,
            mode,
            properties: Arc::new(RcuHashMap::default()),
        }
    }
        ...
        for command in Self::parse_commands(&content) {
            ...
            match UEventAction::try_from(command) {
                Ok(c) => current_task
                    .kernel()
                    .device_registry
                    .dispatch_uevent(c, self.device.clone()),
                Err(e) => {
                    track_stub!(
                        TODO("https://fxbug.dev/297435061"),
                        "synthetic uevent variables"
                    );
                    return Err(e);
                }
            }
...
mod tests {
    ...
    fn test_get_uevent_properties_list() {
        ...
        assert_eq!(
            ...
            (
                "DEVPATH".into(),
                "/devices/virtual/android_usb/android0".into()
            )
        );
