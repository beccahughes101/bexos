...
impl TtyState {
    ...
    pub fn get_next_terminal(
        ...
        assert!(
            self.terminals
                .write()
                .insert(id, Arc::downgrade(&terminal))
                .is_none()
        );
        Ok(terminal)
    }
...
impl Default for TtyState {
    fn default() -> Self {
        Self {
            ...
            pts_ids_set: PtsIdsSet::new(DEVPTS_COUNT).into(),
        }
    }
}
...
impl Terminal {
    ...
    pub fn to_owned(&self) -> Arc<Terminal> {
        self.weak_self
            .upgrade()
            .expect("This should never be called while releasing the terminal")
    }
