...
impl MemoryObject {
        ...
        match self {
            ...
            Self::RingBuf(info) => {
                ...
                if offset + size > vmo_size {
                    info.vmo
                        .op_range(op, 2 * *PAGE_SIZE, offset + size - vmo_size)?;
    ...
    pub fn map_in_vmar(
        ...
        match self {
            ...
            Self::MemoryMappedClock { utc_clock, .. } => {
                ...
                assert_eq!(
                    0, memory_offset,
                    "memory mapped clock must be at memory offset 0"
                );
    ...
    pub fn memmove(
        ...
        match self {
            Self::Vmo(info) => info
                .vmo
                .transfer_data(options, dst_offset, size, &info.vmo, src_offset),
            Self::RingBuf(_) | Self::MemoryMappedClock { .. } => Err(zx::Status::NOT_SUPPORTED),
        }
    }
    ...
    pub fn clone_memory(
        ...
        if self.is_clock() {
            return Err(errno!(
                ENOTSUP,
                "clone_memory not supported on memory mapped clock"
            ));
        }
        ...
        // rights.
        Ok(
            if !options.contains(MappingOptions::ANONYMOUS) && !rights.contains(zx::Rights::WRITE) {
                self.clone()
            } else {
