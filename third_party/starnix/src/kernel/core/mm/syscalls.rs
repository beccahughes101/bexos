...
pub fn do_mmap(
    ...
    let prot_flags = ProtectionFlags::from_access_bits(prot).ok_or_else(|| {
        track_stub!(
            TODO("https://fxbug.dev/322874211"),
            "mmap parse protection",
            prot
        );
        errno!(EINVAL)
    })?;
    ...
    if flags & !valid_flags != 0 {
        ...
        track_stub!(
            TODO("https://fxbug.dev/322873638"),
            "mmap check flags",
            flags
        );
        return error!(EINVAL);
    ...
    let file = if flags & MAP_ANONYMOUS != 0 {
        ...
        Some(current_task.files().get(fd)?)
    };
    ...
    let length_aligned = length
        ...
        & !(page_size - 1);
    let rlimit_as = current_task
        .thread_group()
        .get_rlimit(starnix_uapi::resource_limits::Resource::AS) as usize;
    ...
    let memory_offset = if flags & MAP_ANONYMOUS != 0 {
        0
    } else {
        offset
    };
    ...
    if flags & MAP_ANONYMOUS != 0 {
        ...
        current_task
            .mm()?
            .map_anonymous(addr, length, prot_flags, options, MappingName::None)
    } else {
...
pub fn sys_mprotect(
    ...
    let prot_flags = ProtectionFlags::from_bits(prot).ok_or_else(|| {
        track_stub!(
            TODO("https://fxbug.dev/322874672"),
            "mprotect parse protection",
            prot
        );
        errno!(EINVAL)
    })?;
    current_task
        .mm()?
        .protect(current_task, addr, length, prot_flags)?;
    Ok(())
}
...
pub fn sys_mremap(
    ...
    let addr =
        current_task
            .mm()?
            .remap(current_task, addr, old_length, new_length, flags, new_addr)?;
    Ok(addr)
}
...
pub fn sys_msync(
    ...
    current_task.mm()?.msync(current_task, addr, length, flags)
}
...
pub fn sys_process_vm_readv(
    ...
    track_stub!(
        TODO("https://fxbug.dev/322874765"),
        "process_vm_readv single-copy"
    );
...
pub fn sys_process_vm_writev(
    ...
    track_stub!(
        TODO("https://fxbug.dev/322874339"),
        "process_vm_writev single-copy"
    );
...
pub fn sys_membarrier(
    ...
    match cmd {
        ...
        uapi::membarrier_cmd_MEMBARRIER_CMD_PRIVATE_EXPEDITED => {
            ...
            if current_task
                .mm()?
                .membarrier_private_expedited_registered(MembarrierType::Memory)
            {
        ...
        uapi::membarrier_cmd_MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE => {
            if current_task
                .mm()?
                .membarrier_private_expedited_registered(MembarrierType::SyncCore)
            {
        ...
        uapi::membarrier_cmd_MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED => {
            let _ = current_task
                .mm()?
                .register_membarrier_private_expedited(MembarrierType::Memory)?;
            Ok(0)
        }
...
fn do_futex<Key: FutexKey>(
    ...
    let read_timeout = |current_task: &CurrentTask| {
        ...
        if is_realtime {
            ...
            track_stub!(
                TODO("https://fxbug.dev/356912301"),
                "FUTEX_CLOCK_REALTIME timeout"
            );
        }
        Ok(deadline)
    };
    let read_deadline = |current_task: &CurrentTask| {
        let timespec = read_timespec(current_task)?;
        if is_realtime {
            Ok(TargetTime::RealTime(time_from_timespec::<UtcTimeline>(
                timespec,
            )?))
        } else {
            Ok(TargetTime::Monotonic(time_from_timespec::<
    ...
    match cmd {
        ...
        FUTEX_REQUEUE | FUTEX_CMP_REQUEUE => {
            ...
            let expected_value = if cmd == FUTEX_CMP_REQUEUE {
                Some(value3)
            } else {
                None
            };
        ...
        _ => {
            track_stub!(
                TODO("https://fxbug.dev/322875124"),
                "futex unknown command",
                cmd
            );
            error!(ENOSYS)
        }
...
pub fn sys_get_robust_list(
    ...
    let task = if tid == 0 {
        ...
        current_task.check_ptrace_access_mode(PTRACE_MODE_READ_REALCREDS, &task)?;
        task
    };
...
pub fn sys_mlock(
    ...
    sys_mlock2(current_task, addr, length, 0)
}
...
pub fn sys_mlock2(
    ...
    current_task
        .mm()?
        .mlock(current_task, addr, length, on_fault)
}
...
pub fn sys_mlockall(_current_task: &CurrentTask, _flags: u64) -> Result<(), Errno> {
    track_stub!(TODO("https://fxbug.dev/297292097"), "mlockall()");
    error!(ENOSYS)
}

pub fn sys_munlockall(_current_task: &CurrentTask, _flags: u64) -> Result<(), Errno> {
...
mod tests {
    ...
    async fn test_munmap() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                current_task.read_memory_to_array::<5>(mapped_address),
                error!(EFAULT)
            );
        })
        .await;
    ...
    async fn test_munmap_not_aligned() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert!(
                current_task
                    .read_memory_to_array::<5>(mapped_address)
                    .is_ok()
            );
        })
        .await;
    ...
    async fn test_munmap_many_mappings() {
        spawn_kernel_and_run(async |current_task| {
            ...
            for mapped_address in mapped_addresses {
                assert_eq!(
                    current_task.read_memory_to_vec(mapped_address, 5),
                    error!(EFAULT)
                );
            }
        })
    ...
    async fn test_msync_validates_address_range() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                sys_msync(&current_task, addr, *PAGE_SIZE as usize * 3, 0),
                Ok(())
            );
            assert_eq!(
                sys_msync(&current_task, addr, *PAGE_SIZE as usize * 2, 0),
                Ok(())
            );
            assert_eq!(
                sys_msync(
            ...
            .expect("unmap middle");
            assert_eq!(
                sys_msync(&current_task, addr, *PAGE_SIZE as usize, 0),
                Ok(())
            );
            assert_eq!(
                sys_msync(
    ...
    async fn test_mremap_clobber_memory_mapping() {
        spawn_kernel_and_run(async |current_task| {
            ...
            dst_memory
                .write(&['x' as u8].repeat(*PAGE_SIZE as usize), 0)
                .unwrap();
            dst_memory
                .write(&['y' as u8].repeat(*PAGE_SIZE as usize), *PAGE_SIZE)
                .unwrap();
