...
impl UserFault {
    pub fn new(mm: Weak<MemoryManager>) -> Self {
        Self {
            mm,
            state: LockDepMutex::new(UserFaultState::new()),
        }
    }
    ...
    pub fn get_first_populated_page_after(&self, addr: UserAddress) -> Option<UserAddress> {
        self.state
            ...
            .userfault_pages
            .get(addr)
            .map(|(affected_range, is_populated)| {
                if *is_populated {
                    addr
                } else {
                    affected_range.end
    ...
    pub fn op_register(
        ...
        if !self.has_features(UserFaultFeatures::SIGBUS) {
            track_stub!(
                ...
                "userfault without SIGBUS feature"
            );
...
impl UserFaultState {
    pub fn new() -> Self {
        Self {
            features: None,
            userfault_pages: RangeMap::default(),
        }
    }
}
