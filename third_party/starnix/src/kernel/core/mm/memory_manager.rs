...
impl Drop for ReleasedMappings {
    fn drop(&mut self) {
        assert!(
            self.is_empty(),
            "ReleasedMappings::finalize() must be called before drop"
        );
    }
}
...
fn map_in_vmar(
    ...
    let vmar_offset = addr
        .addr()
        .checked_sub(vmar_info.base)
...
impl MemoryManagerState {
    ...
    fn pick_placement(
        ...
        let mut candidate = Range {
            ...
            end: subrange.start.checked_add(length)?,
        };
        let mut occupied_ranges = self.get_occupied_address_ranges(subrange);
        loop {
            let Some(mapping) = occupied_ranges.next() else {
                ...
                let res = candidate
                    .start
                    .checked_add(chosen_placement_idx * *PAGE_SIZE as usize)?;
            ...
            if mapping.start < candidate.end {
                ...
                candidate = Range {
                    start: mapping.end,
                    end: mapping.end.checked_add(length)?,
                };
                continue;
            }
            let unused_space =
    ...
    fn is_hint_acceptable(&self, hint_addr: UserAddress, length: usize) -> bool {
        ...
        self.get_occupied_address_ranges(&(hint_addr..hint_end))
            .next()
            .is_none()
    }
    ...
    fn select_address(
        ...
        let find_address = || -> Result<SelectedAddress, Errno> {
            let new_addr = if flags.contains(MappingFlags::LOWER_32BIT) {
                ...
                self.find_next_unused_range(adjusted_length)
                    .ok_or_else(|| errno!(ENOMEM))?
            };
        ...
        for (addr, length) in ranges {
            match length {
                ...
                Some(len) => {
                    ...
                    let end = addr
                        .checked_add(len)
                        .expect("address overflowed after validation");
                    if self
                        .mappings
                        .range(addr..end)
    ...
    fn remap(
        ...
        if flags.contains(MremapFlags::MAYMOVE) {
            let dst_address = if flags.contains(MremapFlags::FIXED) {
                Some(new_addr)
            } else {
                None
            };
    ...
    fn try_remap_in_place(
        ...
        let old_range = old_addr
            ..old_addr
                .checked_add(old_length)
                .ok_or_else(|| errno!(EINVAL))?;
        let new_range_in_place = old_addr
            ..old_addr
        ...
        if self
            .mappings
            .range(old_range.end..new_range_in_place.end)
            .next()
            .is_some()
        {
    ...
    fn remap_move(
        ...
        let dst_addr_for_map = match dst_addr {
            ...
            Some(dst_addr) => {
                ...
                let dst_range = dst_addr
                    ..(dst_addr
                        .checked_add(dst_length)
                        .ok_or_else(|| errno!(EINVAL))?);
        ...
        match self.get_mapping_backing(&src_mapping) {
            MappingBacking::PrivateAnonymous => {
                let dst_addr = self
                    .select_address(dst_addr_for_map, dst_length, src_mapping.flags())?
                    .addr();
                ...
                if dst_addr != src_addr {
                    ...
                    mm.mapping_context
                        .private_anonymous
                        .move_pages(&range_to_move, dst_addr)?;
                }

                // Userfault registration is not preserved by remap
                let new_flags = src_mapping
                    .flags()
                    .difference(MappingFlags::UFFD | MappingFlags::UFFD_MISSING);
    ...
    fn update_after_unmap(
        ...
        let end_addr = addr
            .checked_add(length)
            .expect("address overflow during unmap");
        ...
        for (range, mapping) in self.mappings.range(unmap_range.clone()) {
            ...
            if let MappingBacking::PrivateAnonymous = self.get_mapping_backing(mapping) {
                ...
                if let Err(e) = mm.inflight_vmspliced_payloads.handle_unmapping(
                    &mm.mapping_context.private_anonymous.backing,
                    unmapped_range,
                ) {
                ...
                if let Err(e) = mm.mapping_context.private_anonymous.zero(
                    unmapped_range.start,
                    unmapped_range.end - unmapped_range.start,
                ) {
    ...
    fn madvise(
        ...
        if advice == MADV_NORMAL {
            track_stub!(
                TODO("https://fxbug.dev/322874202"),
                "madvise undo hints for MADV_NORMAL"
            );
            return Ok(());
        }
        ...
        for (range, mapping) in self.mappings.range(range_for_op.clone()) {
                ...
                || advice == MADV_UNMERGEABLE
            {
                ...
                memory
                    .op_range(op, start_offset, end_offset - start_offset)
                    .map_err(|s| match s {
    ...
    fn mlock(
        ...
        let desired_end_addr = desired_addr
            .checked_add(desired_length)
            .ok_or_else(|| errno!(EINVAL))?;
                ...
                let shadow_mapping = match current_task.kernel().features.mlock_pin_flavor {
                    ...
                    MlockPinFlavor::ShadowProcess => {
                        ...
                        Some(
                            shadow_process
                                .0
                                .pin_pages(vmo, offset, range.end - range.start)?,
                        )
                    }
        ...
        for (range, mapping, shadow_mapping) in updates {
            if let Some(shadow_mapping) = shadow_mapping {
                released_mappings.extend_pins(
                    self.shadow_mappings_for_mlock
                        .insert(range.clone(), shadow_mapping),
                );
            }
            released_mappings.extend(self.mappings.insert(range, mapping));
        }
    ...
    fn zero(
        ...
        for (mapping, len) in self.get_contiguous_mappings_at(addr, length, context)? {
            let next_offset = bytes_written + len;
            if self
                .zero_mapping((addr + bytes_written)?, mapping, len, context)
                .is_err()
            {
                break;
            }
        ...
        let brk = match self.brk.clone() {
            None => {
                let brk = ProgramBreak {
                    base: self.brk_origin,
                    current: self.brk_origin,
                };
    ...
    fn unregister_uffd(
        ...
        self.userfaultfds
            .retain(|uf| !Weak::ptr_eq(uf, &weak_userfault));
    }
...
impl MemoryManager {
    ...
    pub fn ensure_range_mapped_in_user_vmar(
        ...
        if !self
            .state
            .read()
            .any_ranges_lazy(std::iter::once((addr, length)))
        {
            return Ok(false);
        }
        ...
        I: IntoIterator<Item = (UserAddress, Option<usize>)>,
    {
        ...
        self.state
            .write()
            .ensure_ranges_mapped_in_user_vmar(ranges, &self.mapping_context)
    }
    ...
    pub fn syscall_read_memory<'a>(
        ...
        self.state
            .read()
            .read_memory(addr, bytes, &self.mapping_context)
    }
    ...
    pub fn syscall_read_memory_partial_until_null_byte<'a>(
        ...
        self.state
            .read()
            .read_memory_partial_until_null_byte(addr, bytes, &self.mapping_context)
    }
    ...
    pub fn syscall_read_memory_partial<'a>(
        ...
        self.state
            .read()
            .read_memory_partial(addr, bytes, &self.mapping_context)
    }
    ...
    pub fn unified_write_memory(
        ...
        let copied = self.unified_transfer_loop(addr, len, |cur_addr, offset| {
            Ok(ControlFlow::Continue(
                usercopy().copyout(&bytes[offset..], cur_addr.ptr()),
            ))
        })?;
        if copied < bytes.len() {
            error!(EFAULT)
    ...
    pub fn syscall_write_memory(&self, addr: UserAddress, bytes: &[u8]) -> Result<usize, Errno> {
        self.state
            .read()
            .write_memory(addr, bytes, &self.mapping_context)
    }
    ...
    pub fn syscall_write_memory_partial(
        ...
        self.state
            .read()
            .write_memory_partial(addr, bytes, &self.mapping_context)
    }
    ...
    pub fn unified_zero(
        ...
        let copied = self.unified_transfer_loop(addr, length, |cur_addr, offset| {
            Ok(ControlFlow::Continue(
                usercopy().zero(cur_addr.ptr(), length - offset),
            ))
        })?;
    ...
    pub fn register_membarrier_private_expedited(
        ...
        self.state
            .write()
            .register_membarrier_private_expedited(mtype)
    }
    ...
    pub fn membarrier_private_expedited_registered(&self, mtype: MembarrierType) -> bool {
        self.state
            .read()
            .membarrier_private_expedited_registered(mtype)
    }
}
...
impl MemoryManager {
    ...
    fn protect_vmar_range(
        ...
        unsafe {
            self.mapping_context
                .user_vmar
                .protect(addr.ptr(), length, vmar_flags)
        }
        .map_err(|s| match s {
            zx::Status::INVALID_ARGS => errno!(EINVAL),
            zx::Status::NOT_FOUND => errno!(ENOMEM),
    ...
    fn new(
        ...
        if arch_width.is_arch32() {
            debug_assert_eq!(
                LOWER_4GB_LIMIT.ptr() - user_vmar_info.base,
                user_vmar_info.len
            );
        } else {
        ...
        F: FnOnce(&MemoryManagerState, usize) -> Result<usize, Errno>,
    {
        ...
        let first_populated = userfault
            .get_first_populated_page_after(addr)
    ...
    pub fn snapshot_of(
        ...
        let private_anonymous = source_mm
            .mapping_context
            .private_anonymous
            .snapshot(backing_size)?;
                ...
                match state.get_mapping_backing(mapping) {
                    MappingBacking::Memory(backing) => {
                            ...
                            || mapping.name().is_vvar()
                        {
                            ...
                            let memory = clone_cache
                                .entry(memory_obj.get_koid())
                                .or_insert_with_fallible(|| {
                        ...
                        assert!(
                            target_state
                                .mappings
                                .append_non_overlapping(range.clone(), mapping)
                        );
                    }
                    MappingBacking::PrivateAnonymous => {
    ...
    pub fn log_memory_map(&self, task: &Task, fault_address: UserAddress) {
        ...
        for (range, map) in state.mappings.iter() {
            ...
            let fault_marker = if range.contains(&fault_address) {
                " <= FAULT"
            } else {
                ""
            };
                ...
                if mapping.flags().contains(MappingFlags::UFFD) {
                    ...
                    if let Some(_uffd) = state.find_uffd(addr) {
                        ...
                        return ExceptionResult::Signal(SignalInfo::with_detail(
                            SIGBUS,
                            BUS_ADRERR as i32,
                            SignalDetail::SigFault {
                                addr: decoded.faulting_address,
                            },
                        ));
                    };
                }
    ...
    pub fn get_mapping_memory(
        ...
        match state.get_mapping_backing(mapping) {
            MappingBacking::Memory(backing) => Ok((
                Arc::clone(backing.memory()),
                mapping.address_to_offset(addr),
            )),
            MappingBacking::PrivateAnonymous => Ok((
                Arc::clone(&self.mapping_context.private_anonymous.backing),
                addr.ptr() as u64,
            )),
        }
    }
    ...
    pub fn get_aio_context(&self, addr: UserAddress) -> Option<Arc<AioContext>> {
        let state = self.state.read();
        state
            .get_aio_context(addr)
            .map(|(_, aio_context)| aio_context)
    }
    ...
    pub fn extend_growsdown_mapping_to_address(
        ...
        self.state
            .write()
            .extend_growsdown_mapping_to_address(self, addr, is_write)
    }
    ...
    pub fn get_stats(&self, current_task: &CurrentTask) -> MemoryStats {
        ...
        let is_self_read = current_task
            .mm()
            .map_or(false, |mm| std::ptr::eq(Arc::as_ptr(&mm), self));
    ...
    pub fn atomic_load_u32_acquire(&self, futex_addr: FutexAddress) -> Result<u32, Errno> {
        self.run_atomic_op(futex_addr, |uc| {
            uc.atomic_load_u32_acquire(futex_addr.ptr())
        })
    }

    pub fn atomic_load_u32_relaxed(&self, futex_addr: FutexAddress) -> Result<u32, Errno> {
        self.run_atomic_op(futex_addr, |uc| {
            uc.atomic_load_u32_relaxed(futex_addr.ptr())
        })
    }
...
fn write_map(
    ...
    let line_length = write!(
        ...
        if let MappingNameRef::File(file) = &map.name() {
            file.node().ino
        } else {
            0
        }
...
impl ProcMapsFile {
    pub fn new(task: Arc<Task>) -> DynamicFile<Self> {
        ...
        let mm = task
            .mm()
            .map_or_else(|_| Weak::default(), |mm| Arc::downgrade(&mm));
        DynamicFile::new(Self {
            mm,
...
impl DynamicFileSource for ProcSmapsFile {
    fn generate(&self, current_task: &CurrentTask, sink: &mut DynamicFileBuf) -> Result<(), Errno> {
        ...
        if !lazy_ranges.is_empty() {
            mm.state
                .write()
                .ensure_ranges_mapped_in_user_vmar(lazy_ranges, &mm.mapping_context)?;
        }
        ...
        let committed_bytes_vec = mm.with_zx_mappings(current_task, |zx_mappings| {
            ...
            for (mm_range, mm_mapping) in state.mappings.iter() {
                ...
                for (zx_range, zx_mapping_idx) in zx_memory_info.range(mm_range.clone()) {
                    ...
                    let zx_details = zx_mapping.details();
                    let Some(zx_details) = zx_details.as_mapping() else {
                        continue;
                    };
            ...
            state.mappings.iter().zip(committed_bytes_vec.into_iter())
        {
            ...
            let anonymous_kb = if mm_mapping.private_anonymous() {
                rss_kb
            } else {
                0
            };
            ...
            let locked_kb = if mm_mapping.flags().contains(MappingFlags::LOCKED) {
                rss_kb
            } else {
                0
            };
...
impl DynamicFileSource for ProcSmapsRollupFile {
    fn generate(&self, current_task: &CurrentTask, sink: &mut DynamicFileBuf) -> Result<(), Errno> {
        ...
        writeln!(
            sink,
            "00000000-ffffffffffffffff ---p 00000000 00:00 0 [rollup]"
        )?;
        writeln!(sink, "Rss:            {rss_kb:>8} kB")?;
        writeln!(sink, "Pss:            {pss_kb:>8} kB")?;
        ...
        while pages_processed < num_pages && bytes_written_total < to_read {
            ...
            if let Some((mm_range, mm_mapping)) = state.mappings.get(chunk_start_vaddr) {
                if mm_range.end >= chunk_end_vaddr {
                    ...
                    let chunk_byte_offset = if pages_processed == 0 {
                        unaligned_offset
                    } else {
                        0
                    };
...
fn generate_random_offset_for_aslr(arch_width: ArchWidth) -> usize {
    ...
    let randomness = {
        let random_bits = if arch_width.is_arch32() {
            ASLR_32_RANDOM_BITS
        } else {
            ASLR_RANDOM_BITS
        };
...
mod tests {
    ...
    async fn test_any_ranges_lazy() {
        spawn_kernel_and_run(async |current_task| {
...

            {
                ...
                state
                    .mappings
                    .insert(addr..(addr + length).unwrap(), mapping);
            }
            ...
            assert!(
                mm.ensure_range_mapped_in_user_vmar(addr, Some(length))
                    .unwrap()
            );
    ...
    async fn test_brk() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let addr0 = mm
                .set_brk(&current_task, (base_addr + 1u64).unwrap())
            ...
            let addr1 = mm
                .set_brk(&current_task, (base_addr + 2u64).unwrap())
                .expect("failed to grow brk");
    ...
    async fn test_mm_exec() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let new_mm = MemoryManager::exec(
                ...
                current_task.running_state().mm.upgrade(),
                node,
                ArchWidth::Arch64,
            )
    ...
    async fn test_get_contiguous_mappings_at() {
        spawn_kernel_and_run(async |current_task| {
...

            {
                ...
                assert_equal(
                    mm_state
                        .get_contiguous_mappings_at(addr_a, 0, &context)
                        .unwrap(),
                    vec![],
                );
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_a, page_size, &context)
                    .unwrap(),
                vec![(map_a, page_size)],
            );
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_a, page_size / 2, &context)
                    .unwrap(),
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_a, page_size * 3, &context)
                    .unwrap(),
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_b, page_size, &context)
                    .unwrap(),
                vec![(map_a, page_size)],
            );
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_d, page_size, &context)
                    .unwrap(),
                vec![(map_b, page_size)],
            );
            ...
            assert_equal(
                mm_state
                    .get_contiguous_mappings_at(addr_d, page_size * 10, &context)
                    .unwrap(),
                vec![(map_b, page_size)],
            );
        })
    ...
    async fn test_read_write_crossing_mappings() {
        spawn_kernel_and_run(async |current_task| {
            ...
            ma.write_memory(test_addr, &data)
                .expect("failed to write test data");

            // Read it back.
            let data_readback = ma
                .read_memory_to_vec(test_addr, data.len())
    ...
    async fn test_read_write_errors() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                ma.read_memory_to_vec(unmapped_addr, buf.len()),
                error!(EFAULT)
            );

            // However, accessing zero bytes in unmapped memory is not an error.
            ma.write_memory(unmapped_addr, &[])
                .expect("failed to write no data");
            ma.read_memory_to_vec(unmapped_addr, 0)
                .expect("failed to read no data");
        })
        .await;
    }
    ...
    async fn test_read_c_string_to_vec_large() {
        spawn_kernel_and_run(async |current_task| {
            ...
            ma.write_memory(addr, &random_data)
                .expect("failed to write test string");
            // We should read the same value minus the last byte (NUL char).
            assert_eq!(
                ma.read_c_string_to_vec(UserCString::new(current_task, addr), max_size)
                    .unwrap(),
    ...
    async fn test_read_c_string_to_vec() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let test_addr = addr
                ...
                .unwrap()
                .checked_sub(test_str.len())
                .unwrap();
            ma.write_memory(test_addr, test_str)
                .expect("failed to write test string");
            ...
            ma.write_memory((addr + (page_size - 1)).unwrap(), b"\0")
                .expect("failed to write nul");
            assert_eq!(
                ma.read_c_string_to_vec(UserCString::new(current_task, test_addr), max_size)
                    .unwrap(),
                "foo"
    ...
    async fn can_read_argv_like_regions() {
        spawn_kernel_and_run(async |current_task| {
            ...
            ma.write_memory(addr, &payload).unwrap();
            assert_eq!(
                ma.read_nul_delimited_c_string_list(addr, payload.len())
                    .unwrap(),
                expected_parses,
            );
    ...
    async fn truncate_argv_like_regions() {
        spawn_kernel_and_run(async |current_task| {
            ...
            ma.write_memory(addr, payload).unwrap();
            assert_eq!(
                ma.read_nul_delimited_c_string_list(addr, payload.len() - 3)
                    .unwrap(),
    ...
    async fn test_read_c_string() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Expect error if the address is invalid.
            assert_eq!(
                ma.read_c_string(UserCString::null(current_task), buf),
                error!(EFAULT)
            );
        })
        .await;
    ...
    async fn test_find_next_unused_range() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                mm.state
                    .read()
                    .find_next_unused_range(2 * page_size)
                    .unwrap(),
    ...
    async fn test_count_placements() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(11 * page_size, &subrange_ten),
                Some(0)
            );
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(10 * page_size, &subrange_ten),
                Some(1)
            );
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(9 * page_size, &subrange_ten),
                Some(2)
            );
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(page_size, &subrange_ten),
                Some(10)
            );
            ...
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(5 * page_size, &subrange_ten),
                Some(1)
            );
            assert_eq!(
                mm.state
                    .read()
                    .count_possible_placements(4 * page_size, &subrange_ten),
                Some(3)
            );
            assert_eq!(
                mm.state
    ...
    async fn test_pick_placement() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                mm.state
                    .read()
                    .pick_placement(4 * page_size, 0, &subrange_ten),
                Some(UserAddress::from_ptr(RESTRICTED_ASPACE_BASE))
            );
            assert_eq!(
                mm.state
                    .read()
                    .pick_placement(4 * page_size, 1, &subrange_ten),
                Some(UserAddress::from_ptr(RESTRICTED_ASPACE_BASE + page_size))
            );
            assert_eq!(
                mm.state
                    .read()
                    .pick_placement(4 * page_size, 2, &subrange_ten),
                Some(UserAddress::from_ptr(
                    RESTRICTED_ASPACE_BASE + 6 * page_size
                ))
            );
        })
        .await;
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            for _ in 0..10 {
                let addr = mm
                    .state
                    .read()
                    .find_random_unused_range(page_size, &subrange_ten);
                assert!(addr.is_some());
                assert_eq!(
                    map_memory(&current_task, addr.unwrap(), *PAGE_SIZE),
                    addr.unwrap()
                );
            }
            assert_eq!(
    ...
    async fn test_unmap_returns_multiple_mappings() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let addr = mm
                .state
                .read()
                .find_next_unused_range(3 * *PAGE_SIZE as usize)
                .unwrap();
    ...
    async fn test_read_object_partial() {
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            let items_read = ma
                .read_object_partial(items_ref, 8)
                .expect("Failed to read object");
            ...
            // the right thing.
            let items_read = ma
                .read_object_partial(items_ref, 0)
                .expect("Failed to read object");
    ...
    async fn test_partial_read() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let addr = mm
                .state
                .read()
                .find_next_unused_range(2 * *PAGE_SIZE as usize)
                .unwrap();
            let addr = map_memory(&current_task, addr, *PAGE_SIZE);
            ...
            released_mappings.finalize(state);
            assert_eq!(
                ma.read_memory_partial_to_vec(addr, bytes.len())
                    .unwrap()
                    .len(),
                *PAGE_SIZE as usize,
            );
    ...
    async fn test_grow_mapping_empty_mm() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                mm.extend_growsdown_mapping_to_address(addr, false),
                Ok(false)
            );
        })
        .await;
    ...
    async fn test_grow_write_fault_inside_read_only_mapping() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                mm.extend_growsdown_mapping_to_address(addr, true),
                Ok(false)
            );
        })
        .await;
    ...
    async fn test_snapshot_paged_memory() {
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            let buf = target
                .read_memory_to_vec(addr, 3)
                .expect("read_memory failed");
            assert_eq!(buf, b"foo");

            let buf = current_task
                .deref()
                .read_memory_to_vec(addr, 3)
                .expect("read_memory failed");
            assert_eq!(buf, b"foo");

            port.queue(&zx::Packet::from_user_packet(
                0,
                0,
    ...
    async fn test_set_vma_name() {
        spawn_kernel_and_run(async |mut current_task| {
            ...
            current_task
                .write_memory(name_addr, vma_name.as_bytes())
                .unwrap();
    ...
    async fn test_set_vma_name_beyond_end() {
        spawn_kernel_and_run(async |mut current_task| {
            ...
            current_task
                .mm()
                .unwrap()
                .unmap(second_page, *PAGE_SIZE as usize)
                .unwrap();
    ...
    async fn test_set_vma_name_before_start() {
        spawn_kernel_and_run(async |mut current_task| {
            ...
            current_task
                .mm()
                .unwrap()
                .unmap(mapping_addr, *PAGE_SIZE as usize)
                .unwrap();
    ...
    async fn test_set_vma_name_partial() {
        spawn_kernel_and_run(async |mut current_task| {
            ...
            // This should split the mapping into 3 pieces with the second piece having the name "foo"
            {
                ...
                let (_, mapping) = state
                    .mappings
                    .get((mapping_addr + *PAGE_SIZE).unwrap())
                    .unwrap();
                assert_eq!(mapping.name(), MappingName::Vma("foo".into()));
