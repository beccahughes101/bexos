...
pub unsafe fn read_to_vec<T: FromBytes, E>(
    ...
    debug_assert!(
        read_elements <= max_len,
        "read_elements={read_elements}, max_len={max_len}"
    );
...
pub trait MemoryAccessorExt: MemoryAccessor {
        ...
        unsafe {
            read_to_array(|buf| {
                self.read_memory(addr, buf)
                    .map(|bytes_read| debug_assert_eq!(bytes_read.len(), N))
            })
        }
    }
    ...
    fn read_multi_arch_ptr<T64, T32>(
        ...
        let address = if user.is_arch32() {
            self.read_object::<UserAddress32>(user.addr().into())?
                .into()
        } else {
    ...
    fn read_objects_to_smallvec<T: Clone + FromBytes, const N: usize>(
        ...
        if len > N {
            Ok(SmallVec::<[T; N]>::from_vec(
                self.read_objects_to_vec(user, len)?,
            ))
        } else {
