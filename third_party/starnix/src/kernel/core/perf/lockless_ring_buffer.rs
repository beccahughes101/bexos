...
impl Node {
    ...
    fn finalize(&self) {
        ...
        let old_val = self
            .active_writers
            .fetch_or(PAGE_FINALIZED_BIT, Ordering::AcqRel);
        ...
        self.page_data
            .write_at(8, &(data_size as u64).to_le_bytes());
        ...
        self.active_writers
            .fetch_and(!PAGE_ACTIVE_BIT, Ordering::Release);
    }
...
impl LocklessRingBuffer {
    ...
    pub fn new(
        ...
        for i in 0..circle_size {
            ...
            nodes[i]
                .next
                .store(Node::make_val(next_idx, FLAG_NORMAL), Ordering::Relaxed);
            nodes[i]
                .prev
        ...
        nodes[circle_size - 1]
            .next
            .store(Node::make_val(0, FLAG_HEADER), Ordering::Relaxed);
        // Initialize reader page pointers to point to the head page and its predecessor
        nodes[pages - 1]
            .next
            .store(Node::make_val(0, FLAG_NORMAL), Ordering::Relaxed);
        nodes[pages - 1].prev.store(
            Node::make_val(circle_size - 1, FLAG_NORMAL),
            Ordering::Relaxed,
        );
...
impl<'a> Reservation<'a> {
    pub fn write_at(&self, rel_offset: usize, data: &[u8]) {
        assert!(
            rel_offset + data.len() <= self.size,
            "Write exceeds reservation size"
        );
...
impl LocklessRingBuffer {
    ...
    fn try_reserve_on_page(
        ...
        let actual_prev = self
            .prev_timestamp
            .fetch_max(now_candidate, Ordering::AcqRel);
        ...
        let delta_nanos = if current_offset == LocklessRingBuffer::PAGE_HEADER_SIZE {
            tail_node
                .page_data
                .write_at(0, &final_now_nanos.to_le_bytes());
            0
        } else {
    ...
    fn advance_to_next_page(
        ...
        if should_advance {
            if won_reset {
                ...
                next_node
                    .active_writers
                    .store(PAGE_ACTIVE_BIT, Ordering::Release);
    ...
    pub fn swap_reader_page(&self) -> Option<usize> {
        ...
        loop {
                ...
                .is_ok()
            {
                ...
                next_node
                    .prev
                    .store(Node::make_val(reader_idx, FLAG_NORMAL), Ordering::Relaxed);
                ...
                self.nodes[reader_idx]
                    .active_writers
                    .store(PAGE_ACTIVE_BIT, Ordering::Relaxed);
                // Clear the size field in the page header to avoid reader seeing stale size.
                self.nodes[reader_idx]
                    .page_data
                    .write_at(8, &0u64.to_le_bytes());
    ...
    pub fn disable(&self) -> Result<u64, Errno> {
        ...
        self.ref_count
            .fetch_and(!RING_ENABLED_BIT, Ordering::AcqRel);
    ...
    pub fn enable(&self) -> Result<zx::BootInstant, Errno> {
        ...
        self.prev_timestamp
            .store(now.into_nanos() as u64, Ordering::Release);
        ...
        self.nodes[initial_pages - 1]
            .next
            .store(Node::make_val(0, FLAG_HEADER), Ordering::Relaxed);

        // Initialize reader page size to 0
        self.nodes[initial_pages]
            .page_data
            .write_at(8, &0u64.to_le_bytes());
        ...
        let _ = self
            .vmo
            .as_vmo()
            .expect("vmo must exist")
            .op_range(zx::VmoOp::ZERO, 0, *PAGE_SIZE);
...
mod tests {
    ...
    fn test_commit() {
        ...
        buffer.commit(res.0);
        assert_eq!(
            buffer.nodes[0].active_writers.load(Ordering::Relaxed),
            PAGE_ACTIVE_BIT
        );
    }
    #[test]
    ...
    fn test_concurrent_reserve() {
        ...
        for i in 0..buffer.nodes.len() {
            assert_eq!(
                buffer.nodes[i].active_writers.load(Ordering::Relaxed),
                PAGE_ACTIVE_BIT
            );
        }
    }
    ...
    fn test_reserve_moves_to_next_page() {
        ...
        assert_eq!(
            res2.0.offset,
            (*PAGE_SIZE) as usize + LocklessRingBuffer::PAGE_HEADER_SIZE
        );
    }
    #[test]
    ...
    fn test_read() {
        ...
        assert_eq!(
            &dest.data()[LocklessRingBuffer::PAGE_HEADER_SIZE..],
            &data[..]
        );
    }
    #[test]
    fn test_enable_disable() {
        ...
        assert_eq!(
            buffer.ref_count.load(Ordering::Relaxed) & RING_ENABLED_BIT,
            0
        );
    ...
    fn check_all_message_data(all_messages: &[TestMessage], num_threads: u32) {
        ...
        for msg in all_messages {
            if msg.timestamp_nanos < prev_timestamp {
                println!(
                    "OUT OF ORDER: prev_timestamp={}, current={:?}",
                    prev_timestamp, msg
                );
                out_of_order += 1;
            }
    ...
    fn test_reservation_drop_cancels_writer_count() {
        ...
        let res = buffer.reserve(100).unwrap();
        assert_eq!(
            buffer.ref_count.load(Ordering::Relaxed),
            RING_ENABLED_BIT | 1
        );
    ...
    fn test_overwrite_and_read_4_pages() {
        ...
        assert_eq!(
            all_messages.len(),
            4 * 127,
            "Expected exactly 4 pages of messages"
        );
    }
    #[test]
    ...
    fn test_concurrent_overwrite_stability() {
        ...
        assert!(
            buffer.dropped_pages() > 0,
            "Expected at least some dropped pages"
        );
    }
    ...
    fn test_stale_offset_livelock() {
        ...
        println!(
            "High contention completed. Total dropped pages = {}",
            dropped
        );
    ...
    fn test_extreme_disable_enable_stress() {
        ...
        println!(
            "Disable/enable stress completed. Total writes = {}",
            total_writes
        );
    ...
    fn test_failed_reservation_offset_boundary() {
        ...
        assert_eq!(
            buffer.nodes[0].write_offset.load(Ordering::Acquire),
            expected_offset
        );
