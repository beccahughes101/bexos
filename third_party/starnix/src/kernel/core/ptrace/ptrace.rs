...
impl PtraceEventData {
    pub fn new(option: PtraceOptions, msg: u64) -> Self {
        Self {
            event: PtraceEvent::from_option(&option),
            msg,
        }
    }
...
impl ZombiePtracees {
    ...
    pub fn detach(
        ...
        self.zombies
            .remove(tid)
            .and_then(|traced_zombie| traced_zombie.detach(pids))
    }
    ...
    pub fn has_zombie_matching(&self, selector: &ProcessSelector) -> bool {
        self.zombies
            .values()
            .any(|z| z.artificial_zombie.matches_selector(selector))
    }
    ...
    pub fn get_waitable_entry(
        ...
        ZombieProcess,
        Option<(Weak<ThreadGroup>, OwnedRef<ZombieProcess>)>,
    )> {
...
fn ptrace_cont(
    ...
    if let Some(siginfo) = siginfo {
        ...
        tracee
            .thread_group()
            .set_stopped(StopState::Waking, None, false);
    }
    if detach {
...
pub fn ptrace_detach(
    ...
    let zombie_notification = thread_group
        .write()
        .zombie_ptracees
        .detach(pids, &tracee.tid);
...
pub fn ptrace_dispatch(
    ...
    match request {
        ...
        PTRACE_SETOPTIONS => {
            ...
            {
                track_stub!(
                    TODO("https://fxbug.dev/322874463"),
...
mod tests {
    ...
    async fn test_set_ptracer() {
        spawn_kernel_and_run(async |current_task| {
            ...
            kernel
                .ptrace_scope
                .store(security::yama::SCOPE_RESTRICTED, Ordering::Relaxed);
            assert_eq!(
                sys_prctl(&mut tracee, PR_SET_PTRACER, 0xFFF, 0, 0, 0),
                error!(EINVAL)
            );
    ...
    async fn test_unspawned_task_remove_does_not_register_zombie() {
        spawn_kernel_and_run(async |current_task| {
            let kernel = current_task.kernel().clone();
            let tracee = create_task(&kernel, "tracee");
            let mut tracer = create_task(&kernel, "tracer");
