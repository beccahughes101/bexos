...
fn get_clock_res(current_task: &CurrentTask, which_clock: i32) -> Result<timespec, Errno> {
    match which_clock as u32 {
        ...
        | CLOCK_PROCESS_CPUTIME_ID => Ok(timespec {
            tv_sec: 0,
            tv_nsec: 1,
        }),
        _ => {
...
pub fn sys_gettimeofday(
    ...
    if !user_tz.is_null() {
        ...
        track_stub!(
            TODO("https://fxbug.dev/322874502"),
            "gettimeofday tz argument"
        );
    }
    Ok(())
}
...
pub fn sys_settimeofday(
    ...
    if let Some(ref proxy) = kernel.time_adjustment_proxy {
        ...
        let utc_now_sec_as_nanos = tv
            .tv_sec
            .checked_mul(SEC_IN_NANOS)
            .ok_or_else(|| errno!(EINVAL))?;
        let utc_now_usec_as_nanos = tv
            .tv_usec
            .checked_mul(USEC_IN_NANOS)
            .ok_or_else(|| errno!(EINVAL))?;
...
pub fn sys_clock_nanosleep(
    ...
    match which_clock {
        ...
        CLOCK_TAI => {
            track_stub!(
                TODO("https://fxbug.dev/322875165"),
                "clock_nanosleep, CLOCK_TAI",
                flags
            );
            return error!(EINVAL);
        }
...
pub fn sys_timer_create(
    ...
    let id = &thread_group
        .timers
        .create(timeline, timer_wakeup, checked_signal_event)?;
...
pub fn sys_timer_settime(
    ...
    let old_value =
        current_task
            .thread_group()
            .timers
            .set_time(current_task, id, flags, new_value)?;
...
pub fn sys_setitimer(
    ...
    let old_value = current_task
        .thread_group()
        .set_itimer(current_task, which, new_value)?;
...
pub fn sys_times(current_task: &CurrentTask, buf: UserRef<tms>) -> Result<i64, Errno> {
    ...
    Ok(duration_to_scheduler_clock(
        zx::MonotonicInstant::get() - zx::MonotonicInstant::ZERO,
    ))
}
...
mod test {
    ...
    async fn test_nanosleep_without_remainder() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_object(address_ptr, &duration)
                .expect("write_object");
