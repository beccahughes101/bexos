...
fn make_seccomp_data(
    ...
    #[cfg(target_arch = "aarch64")]
    let arch_val = if current_task.is_arch32() {
        AUDIT_ARCH_ARM
    } else {
        AUDIT_ARCH_AARCH64
    };
...
impl SeccompFilterContainer {
    ...
    pub fn register_listener(
        ...
        let handle = Anon::new_file(
            current_task,
            Box::new(SeccompNotifierFileObject {
...
impl SeccompState {
    pub fn from(state: &SeccompState) -> SeccompState {
        SeccompState {
            filter_state: AtomicU8::new(state.filter_state.load(Ordering::Acquire)),
        }
    }
    ...
    fn log_action(task: &CurrentTask, syscall: &Syscall) {
        ...
        task.kernel()
            .audit_logger()
            .audit_log(AUDIT_SECCOMP as u16, || {
                format!(
    ...
    pub fn do_user_defined(
        ...
        match action {
            ...
            SeccompAction::Trace => {
                track_stub!(
                    TODO("https://fxbug.dev/297311898"),
...
impl SeccompAction {
    ...
    pub fn set_actions_logged(kernel: &Kernel, data: &[u8]) -> Result<(), Errno> {
        ...
        kernel
            .actions_logged
            .store(new_actions_logged, Ordering::Relaxed);
        Ok(())
    }
...
impl SeccompNotifier {
    ...
    fn create_notification(&mut self, cookie: u64, notif: seccomp_notif) {
        self.pending_notifications
            .insert(cookie, SeccompNotification::new(notif));
        self.waiters
            .notify_fd_events(FdEvents::POLLIN | FdEvents::POLLRDNORM);
    }
    ...
    fn consume_some_notification(&mut self) -> Option<seccomp_notif> {
        for (_, notif) in self.pending_notifications.iter_mut() {
            if !notif.consumed {
                notif.consumed = true;
                self.waiters
                    .notify_fd_events(FdEvents::POLLOUT | FdEvents::POLLWRNORM);
