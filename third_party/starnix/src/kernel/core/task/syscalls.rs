...
pub fn do_clone(current_task: &mut CurrentTask, args: &clone_args) -> Result<pid_t, Errno> {
    ...
    if args.flags & (CLONE_SETTLS as u64) != 0 {
        new_task
            .thread_state
            .registers
            .set_thread_pointer_register(args.tls);
    }
...
fn read_c_string_vector(
    ...
    loop {
        ...
        vec_size = vec_size
            .checked_add(cstring.as_bytes_with_nul().len())
            .ok_or_else(|| errno!(E2BIG))?;
...
pub fn sys_setpgid(current_task: &CurrentTask, pid: pid_t, pgid: pid_t) -> Result<(), Errno> {
    ...
    current_task
        .thread_group()
        .setpgid(current_task, &task, &pgid)
}
...
pub fn sys_exit(current_task: &CurrentTask, code: i32) -> Result<(), Errno> {
    ...
    current_task
        .write()
        .set_exit_status_if_not_already(ExitStatus::Exit(code as u8));
    Ok(())
}
    ...
    match option {
        PR_SET_VMA => {
            if arg2 != PR_SET_VMA_ANON_NAME as u64 {
                track_stub!(
                    TODO("https://fxbug.dev/322874826"),
                    "prctl PR_SET_VMA",
                    arg2
                );
                return error!(ENOSYS);
        ...
        PR_GET_CHILD_SUBREAPER => {
            ...
            let value: i32 = if current_task.thread_group().read().is_child_subreaper {
                1
            } else {
                0
            };
        ...
        PR_SET_SECUREBITS => {
            ...
            let securebits = SecureBits::from_bits(arg2 as u32).ok_or_else(|| {
                track_stub!(
                    TODO("https://fxbug.dev/322875244"),
                    "PR_SET_SECUREBITS",
                    arg2
                );
                errno!(ENOSYS)
        ...
        PR_CAP_AMBIENT => {
            ...
            match operation {
                ...
                PR_CAP_AMBIENT_IS_SET => Ok(current_task
                    .current_creds()
                    .cap_ambient
                    .contains(capability_arg)
                    .into()),
        ...
        PR_GET_TIMERSLACK => Ok(current_task.read().timerslack_ns.into()),
        #[cfg(target_arch = "aarch64")]
        ...
        _ => {
            track_stub!(
                TODO("https://fxbug.dev/322874733"),
                "prctl fallthrough",
                option
            );
            error!(ENOSYS)
...
pub fn sys_capset(
    ...
    let mut creds = Credentials::clone(&current_task.current_creds());
    {
        ...
        if !creds
            .cap_inheritable
            .union(creds.cap_permitted)
            .contains(new_inheritable)
        {
            security::check_task_capable(current_task, CAP_SETPCAP)?;
        }
...
pub fn sys_seccomp(
    ...
    match operation {
        ...
        _ => {
            track_stub!(
                TODO("https://fxbug.dev/322874916"),
                "seccomp fallthrough",
                operation
            );
            error!(EINVAL)
...
pub fn sys_getpriority(current_task: &CurrentTask, which: u32, who: i32) -> Result<u8, Errno> {
    ...
    track_stub!(
        TODO("https://fxbug.dev/322893809"),
        "getpriority permissions"
    );
...
pub fn sys_setns(current_task: &CurrentTask, ns_fd: FdNumber, ns_type: c_int) -> Result<(), Errno> {
    ...
    if let Some(mount_ns) = file_handle.downcast_file::<MountNamespaceFile>() {
        ...
        track_stub!(
            TODO("https://fxbug.dev/297312091"),
            "setns CLONE_FS limitations"
        );
    ...
    track_stub!(
        TODO("https://fxbug.dev/322893829"),
        "unknown ns file for setns, see logs"
    );
    log_info!(
        "ns_fd was not a supported namespace file: {}",
        file_handle.ops_type_name()
    );
    error!(EINVAL)
}
...
pub fn sys_unshare(current_task: &CurrentTask, flags: u32) -> Result<(), Errno> {
    ...
    if flags & !IMPLEMENTED_FLAGS != 0 {
        track_stub!(
            TODO("https://fxbug.dev/322893372"),
            "unshare",
...
pub fn sys_kcmp(
    ...
    match resource_type {
        KcmpResource::FILE => {
            ...
            Ok(encode_ordering(
                obfuscate_arc(&file1).cmp(&obfuscate_arc(&file2)),
            ))
        }
        KcmpResource::FILES => {
            ...
            Ok(encode_ordering(
                obfuscate_value(files1.raw()).cmp(&obfuscate_value(files2.raw())),
            ))
        }
        KcmpResource::FS => {
            ...
            Ok(encode_ordering(
                obfuscate_arc(&fs1).cmp(&obfuscate_arc(&fs2)),
            ))
        }
        ...
        KcmpResource::VM => Ok(encode_ordering(
            obfuscate_arc(&task1.mm()?).cmp(&obfuscate_arc(&task2.mm()?)),
        )),
        _ => error!(EINVAL),
    }
}
...
pub fn sys_syslog(
    ...
    let syslog = current_task
        .kernel()
        .syslog
        .access(&current_task, SyslogAccess::Syscall(action))?;
    match action {
        SyslogAction::Read => {
        ...
        SyslogAction::ReadAll => {
            ...
            let mut output_buffer =
                UserBuffersOutputBuffer::unified_new_at(current_task, address, length as usize)?;
            syslog.read_all(current_task.kernel(), &mut output_buffer)
        }
...
mod arch32 {
    pub use super::{
        sys_execve as sys_arch32_execve, sys_execveat as sys_arch32_execveat,
        sys_getegid as sys_arch32_getegid32, sys_geteuid as sys_arch32_geteuid32,
        sys_getgid as sys_arch32_getgid32, sys_getgroups as sys_arch32_getgroups32,
        ...
        sys_quotactl as sys_arch32_quotactl,
...
mod tests {
    ...
    async fn test_prctl_set_vma_anon_name() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_memory(name_addr, name.as_bytes())
    ...
    async fn test_set_vma_name_special_chars() {
        spawn_kernel_and_run(async |current_task| {
            ...
            for c in 1..255 {
                ...
                current_task
                    .write_memory(name_addr, vma_name.as_bytes_with_nul())
                    .unwrap();
    ...
    async fn test_set_vma_name_long() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_memory(name_addr, name_too_long.as_bytes_with_nul())
                .unwrap();
    ...
    async fn test_set_vma_name_misaligned() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_memory(name_addr, name.as_bytes_with_nul())
                .unwrap();
    ...
    async fn test_set_affinity_size() {
        spawn_kernel_and_run(async |current_task| {
            let mapped_address = map_memory(&current_task, UserAddress::default(), *PAGE_SIZE);
            current_task
                .write_memory(mapped_address, &[0xffu8])
                .expect("failed to cpumask");
            let pid = current_task.get_pid();
            assert_eq!(
    ...
    async fn test_task_name() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let out_name = current_task
                .read_memory_to_vec(mapped_address, name_length)
                .unwrap();
    ...
    async fn test_sched_setscheduler() {
        spawn_kernel_and_run(async |current_task| {
            current_task.thread_group().limits.lock().set(
                Resource::RTPRIO,
                rlimit {
                    rlim_cur: 255,
                    rlim_max: 255,
            ...
            assert_eq!(
                ...
                "tasks should have normal scheduler by default"
            );

            let mapped_address = map_memory(&current_task, UserAddress::default(), *PAGE_SIZE);
            let requested_params = sched_param { sched_priority: 15 };
            current_task
                .write_object(mapped_address.into(), &requested_params)
                .unwrap();
            ...
            assert_eq!(
                ...
                "task should have been assigned fifo scheduler"
            );

            let mapped_address = map_memory(&current_task, UserAddress::default(), *PAGE_SIZE);
            sys_sched_getparam(&current_task, 0, mapped_address.into()).expect("sched_getparam");
            let param_value: sched_param = current_task
                .read_object(mapped_address.into())
                .expect("read_object");
    ...
    async fn test_read_c_string_vector() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task
                .write_memory(arg_addr, arg)
                .expect("failed to write test arg");
