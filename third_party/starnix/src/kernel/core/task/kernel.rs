...
impl MountsWriteToken {
    fn new() -> Self {
        Self {
            deferred_drops: RefCell::new(SmallVec::new()),
        }
    }
...
impl InterfacesHandler for InterfacesHandlerImpl {
    fn handle_new_link(&mut self, name: &str, interface_id: NonZeroU64) {
        if let Some(kernel) = self.kernel() {
            kernel
                .netstack_devices
                .add_device(&kernel, name.into(), interface_id);
        }
    }
...
impl Kernel {
    pub fn new(
        ...
        let cpu_feature_flags =
            zx::system_get_feature_flags::<CpuFeatureFlags>().unwrap_or_else(|e| {
                log_debug!(
                    "CPU feature flags are only supported on ARM64: {}, reporting 0",
                    e
                );
        ...
        let this = Arc::new_cyclic(|kernel| Kernel {
            ...
            root_uts_ns: Arc::new(UtsNamespace::default().into()),
            vdso: Vdso::new(),
            ...
            container_control_handle: Default::default(),
        ...
        this.inspect_node
            .record_lazy_child("thread_groups", move || {
    ...
    async fn run_shutdown(&self) {
        ...
        let maybe_init = self
            .get_init_task()
            .ok()
            .map(|t| Arc::downgrade(&t.thread_group));
        ...
        assert_eq!(
            kernel_job.children().unwrap(),
            &[],
            "starnix does not create any child jobs"
        );
        ...
        loop {
            ...
            for pid in remaining_processes {
                ...
                terminated_signals.push(fuchsia_async::OnSignals::new(
                    handle,
    ...
    pub fn open_device(
        ...
        self.device_registry
            .open_device(current_task, node, flags, dev, mode)
    }
    ...
    fn get_thread_groups_inspect(&self) -> fuchsia_inspect::Inspector {
        ...
        for thread_group in self.pids.get_thread_groups(&RcuReadScope::new()) {
            ...
            tg_node.record_bool(
                "stopped",
                thread_group.load_stopped() == StopState::GroupStopped,
            );
    ...
    pub fn open_ns_dir(
        ...
        match self.container_namespace.find_closest_channel(&ns_path) {
            Ok((root_channel, remaining_subdir)) => {
                ...
                Ok((
                    fio::DirectorySynchronousProxy::new(root_channel),
                    remaining_subdir,
                ))
            }
            Err(err) => {
    ...
    pub fn cmdline_args_iter(&self) -> impl Iterator<Item = ArgNameAndValue<'_>> {
        parse_cmdline(self.cmdline.to_str().unwrap_or_default()).filter_map(|arg| {
            arg.split_once('=')
                .map(|(name, value)| ArgNameAndValue {
                    name: name,
                    value: Some(value),
                })
                .or(Some(ArgNameAndValue {
                    name: arg,
                    value: None,
                }))
        })
    ...
    pub fn fs_cache_config(&self) -> CacheConfig {
        CacheConfig {
            capacity: self.features.dirent_cache_size as usize,
        }
    }
...
impl HwCaps {
    ...
    pub fn from_cpu_feature_flags(cpu_feature_flags: CpuFeatureFlags) -> Self {
        Self {
            arch32: arm32_hwcap(cpu_feature_flags),
            arch64: arm64_hwcap(cpu_feature_flags),
        }
    }
