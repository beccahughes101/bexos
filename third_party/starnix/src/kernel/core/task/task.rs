...
impl From<uapi::robust_list> for RobustList {
    fn from(robust_list: uapi::robust_list) -> Self {
        Self {
            next: RobustListPtr::from(robust_list.next),
        }
    }
}
...
impl TaskMutableState {
    ...
    pub fn is_ptrace_listening(&self) -> bool {
        self.ptrace
            .as_ref()
            .is_some_and(|ptrace| ptrace.stop_status == PtraceStatus::Listening)
    }

    pub fn ptrace_on_signal_consume(&mut self) -> bool {
        self.ptrace
            .as_mut()
            .is_some_and(|ptrace: &mut Box<PtraceState>| {
    ...
    pub fn is_signal_masked_by_saved_mask(&self, signal: Signal) -> bool {
        self.signals
            .saved_mask()
            .is_some_and(|mask| mask.has_signal(signal))
    }
    ...
    pub fn wait_on_signal_fd_events(
        ...
        self.signals
            .signal_wait
            .wait_async_signal_mask(waiter, mask, handler)
    }
...
impl TaskMutableState<Base = Task> {
    ...
    pub fn update_flags(&mut self, clear: TaskFlags, set: TaskFlags) {
        ...
        let swapped = self
            .base
            .flags
            .swap((observed | set) & !clear, Ordering::Relaxed);
        debug_assert_eq!(swapped, observed);
    }

    pub fn set_flags(&mut self, flag: TaskFlags, v: bool) {
        let (clear, set) = if v {
            (TaskFlags::empty(), flag)
        } else {
            (flag, TaskFlags::empty())
        };
...
impl TaskPersistentInfoState {
    ...
    pub fn lock_creds(&self) -> CredentialsReadGuard<'_> {
        ...
        CredentialsReadGuard {
            _lock: lock,
            creds: self.creds.read(),
        }
    }
...
impl Task {
    ...
    pub fn set_ptrace_zombie(&self, pids: &mut crate::task::PidTableGuard<'_>) {
        ...
        if let Some(ptrace) = &mut state.ptrace {
            ...
            if let Some(tracer_tg) = tracer_tg {
                ...
                tracer_state
                    .zombie_ptracees
                    .add(pids, self.tid.clone(), zombie);
            };
        }
    }
    ...
    pub fn exit_status(&self) -> Option<ExitStatus> {
        self.is_exitted()
            .then(|| self.read().exit_status.clone())
            .flatten()
    }
    ...
    pub fn ptracer_task(&self) -> Option<Arc<Task>> {
        self.read()
            .ptrace
            .as_ref()
            .and_then(|p| p.core_state.task.upgrade())
    }
    ...
    fn update_scheduler_state_then_role(
        ...
        let (new_scheduler_state, cpuset_path, last_applied_role) = {
            ...
            (
                ...
                state.cpuset_path.clone(),
                state.last_applied_role.clone(),
            )
        };
    ...
    pub fn time_stats(&self) -> TaskTimeStats {
        ...
        let info = match running_state.thread.get() {
            Some(thread) => thread
                .get_runtime_info()
                .expect("Failed to get thread stats"),
    ...
    pub fn record_pid_koid_mapping(&self) {
        ...
        if let Some(identity) = self.get_zircon_identity() {
            self.kernel()
                .trace_event_manager
                .record(self.get_pid(), self.get_tid(), identity);
        }
    }
}
...
mod test {
    ...
    async fn test_tid_allocation() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                pids.get(another_tid).unwrap().get_task().unwrap().get_tid(),
                another_tid
            );
        })
        .await;
    ...
    async fn test_clone_pid_and_parent_pid() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                ...
                child_task.thread_group().read().get_ppid()
            );
        })
        .await;
    }
    ...
    async fn test_root_capabilities() {
        spawn_kernel_and_run(async |current_task| {
            assert!(security::is_task_capable_noaudit(
                current_task,
                CAP_SYS_ADMIN
            ));
            assert_eq!(
                current_task.real_creds().cap_inheritable,
                Capabilities::empty()
            );
