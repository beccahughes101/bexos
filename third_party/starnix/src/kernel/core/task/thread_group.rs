...
impl fmt::Debug for ThreadGroup {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.process
                .get_name()
                .unwrap_or(zx::Name::new_lossy("<unknown>")),
            self.leader
        )
    }
}
...
impl ThreadGroup {
    pub fn sync_syscall_log_level(&self) {
        ...
        let prev_should_log = self
            .log_syscalls_as_info
            .swap(should_log, Ordering::Relaxed);
...
impl ThreadGroupParent {
    ...
    pub fn upgrade(&self) -> Arc<ThreadGroup> {
        self.0
            .upgrade()
            .expect("ThreadGroupParent references must always be valid")
    }
}
...
impl ThreadGroup {
        ...
        Arc::new_cyclic(|weak_self| {
            ...
            if let Some(mut parent) = parent {
                thread_group
                    ...
                    .reset(parent.base.next_seccomp_filter_id.get());
                parent
                    .children
                    .insert(thread_group.leader.id, weak_self.clone());
    ...
    pub fn remove(&self, mut pids: PidTableGuard<'_>, task: &Arc<Task>) {
        ...
        if state.tasks.is_empty() {
            ...
            self.kernel
                .cgroups
                .lock_cgroup2_pid_table()
                .remove_process(&self.leader);
...

            {
                ...
                if let Some(reaper) = reaper {
                    let reaper = reaper.upgrade();
                    {
                        ...
                        reaper_state
                            .zombie_children
                            .append(&mut state.zombie_children);
                    }
    ...
    fn detach_ptracees(&self, pids: &mut PidTableGuard<'_>) {
        let tracee_tids = self
            .ptracees
            .lock()
            .iter()
            .map(|info| info.tid.clone())
            .collect_vec();
    ...
    pub fn do_zombie_notifications(
        ...
        state
            ...
            .retain(|dzp| dzp.tracee_pid != zombie.pid);
        ...
        if state.zombie_ptracees.has_tracee(&tracee.tid) {
            if self == parent {
                ...
                drop(state);
                {
                    ...
                    parent_state
                        .deferred_zombie_ptracers
                        .push(DeferredZombiePTracer::new(self, tracee, tracee_pgid));
                ...
                state
                    .zombie_ptracees
                    .set_parent_of(&tracee.tid, Some(zombie), parent);
    ...
    pub fn set_itimer(
        ...
        if which == ITIMER_PROF || which == ITIMER_VIRTUAL {
            ...
            track_stub!(
                TODO("https://fxbug.dev/322874521"),
                "Unsupported itimer type",
                which
            );
    ...
    pub fn set_stopped(
        ...
        self.write()
            .set_stopped(new_stopped, siginfo, finalize_only)
    }
    ...
    pub fn set_controlling_terminal(
        ...
        let other_session = terminal_state
            .controller
            .as_ref()
            .and_then(|cs| cs.session.upgrade());
    ...
    pub fn get_waitable_ptracee(
        ...
        let waitable_entry = self
            .write()
            .zombie_ptracees
            .get_waitable_entry(selector, options);
                ...
                if let Some(tg) = tg.upgrade() {
                    if Arc::as_ptr(&tg) != self as *const Self {
                    ...
                    } else {
                        {
                            ...
                            state
                                .deferred_zombie_ptracers
                                .retain(|dzp| dzp.tracee_pid != z.pid);
                        }
                    ...
                    let mut event = ptrace
                        ...
                        .map_or(PtraceEvent::None, |ptrace| {
                            ptrace
                                .event_data
                                .as_ref()
                                .map_or(PtraceEvent::None, |data| data.event)
                        })
                        .clone();
    ...
    pub fn send_signal_unchecked_with_info(
        ...
        if let Some(signal) = self.check_signal_access(current_task, unchecked_signal)? {
            self.write()
                .send_signal(siginfo.into_signal_info(signal, options)?);
        }
        ...
        F: Fn(&SignalInfo) -> bool,
    {
        ...
        self.has_pending_signals
            .store(!signals.is_empty(), Ordering::Relaxed);
        r
    }
    ...
    pub async fn shut_down(this: Weak<Self>) {
        ...
        this.upgrade()
            .map(|tg| tg.write().send_signal(SignalInfo::kernel(SIGTERM)));
...
impl ThreadGroupMutableState<Base = ThreadGroup> {
    ...
    pub fn children(&self) -> impl Iterator<Item = Arc<ThreadGroup>> + '_ {
        self.children.values().map(|v| {
            v.upgrade()
                .expect("Weak references to processes in ThreadGroup must always be valid")
        })
    }
    ...
    pub fn tasks(&self) -> Vec<Arc<Task>> {
        self.tasks
            .iter()
            .flat_map(|info| info.tid.get_task().ok())
            .collect()
    }
    ...
    pub fn get_task(&self, tid: tid_t) -> Option<Arc<Task>> {
        self.tasks
            .iter()
            .find(|info| info.tid.id == tid)
            .and_then(|info| info.tid.get_task().ok())
    }
    ...
    fn leave_process_group(&mut self, pids: &mut PidTableGuard<'_>) -> SessionDisassociation {
        ...
        if is_empty {
            self.process_group
                .session
                .write()
                .remove(&self.process_group.leader);
    ...
    pub fn set_stopped(
        ...
        if new_stopped == StopState::Waking || new_stopped == StopState::ForceWaking {
            self.lifecycle_waiters
                .notify_value(ThreadGroupLifecycleWaitValue::Stopped);
        };

        let parent = (!new_stopped.is_in_progress())
...
mod test {
    ...
    async fn test_setsid() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                get_process_group(&current_task),
                get_process_group(&child_task)
            );
    ...
    async fn test_exit_status() {
        spawn_kernel_and_run(async |current_task| {
            ...
            std::mem::drop(child);
            assert_eq!(
                current_task.thread_group().read().zombie_children[0]
                    .state
                    .exit_status,
                ExitStatus::Exit(42)
            );
    ...
    async fn test_setgpid() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                child_task1
                    .thread_group()
                    .setpgid(&current_task, &current_task, &current_task.pid),
                error!(ESRCH)
            );
            assert_eq!(
            ...
            assert_eq!(
                current_task
                    .thread_group()
                    .setpgid(&current_task, &child_task1, &child_task2.pid),
                error!(EPERM)
            );
            assert_eq!(
                current_task.thread_group().setpgid(
            ...
            assert_eq!(
                child_task1
                    .thread_group()
                    .setpgid(&current_task, &child_task1, &child_task1.pid),
                Ok(())
            );
            assert_eq!(
                child_task1
            ...
            assert_eq!(
                child_task1.thread_group().read().process_group.leader,
                child_task1.tid
            );
            ...
            assert_eq!(
                current_task
                    .thread_group()
                    .setpgid(&current_task, &child_task2, &child_task1.pid),
                Ok(())
            );
            assert_eq!(
                child_task2.thread_group().read().process_group.leader,
                child_task1.tid
            );
            assert!(
                !old_process_group
            ...
            assert_eq!(
                child_task3
                    .thread_group()
                    .setpgid(&current_task, &child_task3, &child_task3.pid),
                Ok(())
            );
            ...
            assert_eq!(
                current_task
                    .thread_group()
                    .setpgid(&current_task, &child_task1, &child_task3.pid),
                Ok(())
            );
            assert_eq!(
                child_task1.thread_group().read().process_group.leader,
