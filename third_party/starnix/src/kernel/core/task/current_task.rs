...
impl CurrentTask {
    pub fn new(task: Arc<Task>, thread_state: ThreadState<RegisterStorageEnum>) -> Self {
        ...
        let running_state = task
            .running_state()
            .expect("CurrentTask must have TaskRunningState");
        let files = running_state
            .files()
            .expect("CurrentTask must have FdTable");
        Self {
            task,
            thread_state,
    ...
    pub fn exit(mut self) {
        ...
        *self.files.borrow_mut() = None;
        ...
        self.task
            .thread_group()
            .remove(self.kernel().pids.lock(), &self.task);
    ...
    pub async fn override_creds_async<R>(
        ...
        let saved = self
            .current_creds
            .replace(CurrentCreds::Overridden(new_creds));
    ...
    pub fn open_file_for_exec(
        ...
        let mut context = LookupContext::new(if nofollow {
    ...
    pub fn exec(
        ...
        if self.thread_group().read().tasks_count() > 1 {
            track_stub!(
                TODO("https://fxbug.dev/297434895"),
                "exec on multithread process"
            );
            return error!(EINVAL);
        }
    ...
    fn finish_exec(
        ...
        let elf_arch_width = resolved_program
            .arch_width()
...

        {
            ...
            let dumpable = if resolved_program.secure_exec {
                DumpPolicy::Disable
            } else {
                DumpPolicy::User
            };
    ...
    pub fn add_seccomp_filter(
        ...
        if flags & SECCOMP_FILTER_FLAG_TSYNC != 0 {
            ...
            task_state
                .seccomp_filters
                .add_filter(new_filter, code.len() as u16)?;
    ...
    pub fn clone_task(
        ...
        if clone_vm && !clone_thread {
            ...
            track_stub!(
                TODO("https://fxbug.dev/322875167"),
                "CLONE_THREAD without CLONE_VM"
            );
            return error!(ENOSYS);
        }
        ...
        let TaskInfo {
            ...
            let thread_group_state = {
                ...
                if clone_parent {
                    ...
                    weak_original_parent = thread_group_state
                        .parent
                        .clone()
                        .ok_or_else(|| errno!(EINVAL))?;
        ...
        let vfork_event = if clone_vfork {
    ...
    fn finalize_stop_state(&mut self) -> bool {
        ...
        if self
            .thread_group()
            .set_stopped(StopState::GroupStopped, None, true)
            == StopState::GroupStopped
        {
            ...
            self.write()
                .set_stopped(StopState::GroupStopped, signal, Some(self), event);
            return true;
        }
    ...
    fn block_while_stopped(&mut self) {
        ...
        loop {
            ...
            if self.is_exitted() {
                self.thread_group()
                    .set_stopped(StopState::ForceAwake, None, false);
                self.write()
                    .set_stopped(StopState::ForceAwake, None, Some(self), None);
                return;
            }
