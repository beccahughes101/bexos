...
impl IpTable {
    fn accept_policy_v4() -> Vec<u8> {
        [
            ...
            .as_bytes(),
            xt_entry_target {
                target_size: STANDARD_TARGET_SIZE,
                ..Default::default()
            }
            .as_bytes(),
    ...
    fn register_ebpf_programs(
        ...
        for (id, program_handle) in &ebpf_programs {
            let entry = self
                .ebpf_programs
                .entry(*id)
                .or_insert_with(|| EbpfProgramState {
...
impl IpTables {
    ...
    pub fn getsockopt(
        ...
        match optname {
            ...
            _ => {
                track_stub!(
                    TODO("https://fxbug.dev/322875228"),
                    "optname for network sockets"
                );
                Ok(vec![])
            }
        }
        ...
        match optname {
            ...
            IPT_SO_SET_ADD_COUNTERS => {
                ...
                for chunk in bytes.chunks(std::mem::size_of::<xt_counters>()) {
                    counters.push(
                        xt_counters::read_from_prefix(chunk)
    ...
    fn replace_ipv4_table(&self, current_task: &CurrentTask, bytes: Vec<u8>) -> Result<(), Errno> {
        ...
        self.state
            .write()
            .replace_table(Ip::V4, ebpf_programs, table)?;
    ...
    fn replace_ipv6_table(&self, current_task: &CurrentTask, bytes: Vec<u8>) -> Result<(), Errno> {
        ...
        self.state
            .write()
            .replace_table(Ip::V6, ebpf_programs, table)?;
