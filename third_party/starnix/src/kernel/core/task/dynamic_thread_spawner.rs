...
mod tests {
    ...
    async fn run_10_tasks() {
        spawn_kernel_and_run(async |current_task| {
            ...
            for _ in 0..10 {
                let closure = move |_: &CurrentTask| {};
                let opts = SpawnRequestBuilder::new()
                    .with_sync_closure(closure)
                    .build();
    ...
    async fn blocking_task_do_not_prevent_further_processing() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let pair = Arc::new((
                fuchsia_sync::Mutex::new(false),
                fuchsia_sync::Condvar::new(),
            ));
            for _ in 0..10 {
    ...
    async fn test_spawn_sync_to_async_result() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let (result2, req2) = SpawnRequestBuilder::new()
                .with_async_closure(fut2)
                .build_with_sync_result();
    ...
    async fn test_spawn_async_to_async_result() {
        spawn_kernel_and_run(async |current_task| {
            let spawner = DynamicThreadSpawner::new(2, current_task.weak_task(), "kthreadd");
            let fut = async |_: &CurrentTask| 42;
            let (result_fut, req) = SpawnRequestBuilder::new()
                .with_async_closure(fut)
