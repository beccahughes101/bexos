    ...
    pub fn init(&self, kernel: &Kernel) -> Result<(), anyhow::Error> {
        ...
        kernel.inspect_node.record_lazy_child("syslog", move || {
            ...
            async move {
                ...
                inspector
                    .root()
                    .record_uint("max_timeline_size", state_guard.max_timeline_size());
                inspector
                    .root()
    ...
    fn new(syslog: &Syslog, mode: fdiagnostics::StreamMode) -> Result<Self, Errno> {
        ...
        accessor
            ...
            .map_err(|err| {
                errno!(
                    EIO,
                    format!("ArchiveAccessor/StreamDiagnostics failed: {err}")
                )
            })?;
        let iterator = fdiagnostics::BatchIteratorSynchronousProxy::new(client_end.into_channel());
        if is_subscribe {
            let () = iterator
                .wait_for_ready(zx::MonotonicInstant::INFINITE)
                .map_err(|err| {
                    errno!(
                        EIO,
                        format!("Failed to wait for BatchIterator being ready: {err}")
                    )
                })?;
        }
        Ok(Self {
...
fn format_log<T: TimeFetcher>(
    ...
    let time = state
        .lock()
        .boot_time_to_monotonic_time(data.metadata.timestamp);
    ...
    write!(
        &mut result,
        "<{level}>[{time_secs:05}.{time_fract:06}] {component_name}",
    )?;
