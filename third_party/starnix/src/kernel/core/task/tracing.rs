...
impl PidKoidMap {
    fn insert(&mut self, pid: pid_t, tid: tid_t, identity: ZirconIdentity) {
        ...
        self.koid_to_pid
            .insert(identity.thread, LinuxIdentity::Thread { pid, tid });
        ...
        self.koid_to_pid
            .entry(identity.process)
            .or_insert(LinuxIdentity::Process { pid });
...
impl TracePerformanceEventManager {
    ...
    fn snapshot_existing_tasks(pid_table: &PidTable) -> PidKoidMap {
        ...
        log_debug!(
            "Initialized {} pid mappings. From {} ids",
            pid_map.tid_to_koid.len(),
            count
        );
        pid_map
    }
...
mod tests {
    ...
    fn identity(process: u64, thread: u64) -> ZirconIdentity {
        ZirconIdentity {
            process: Koid::from_raw(process),
            thread: Koid::from_raw(thread),
        }
    }
    ...
    fn test_seed_does_not_overwrite_fresh_record() {
        ...
        assert_eq!(
            manager.get_zircon_identity(1).map(|id| id.thread),
            Some(Koid::from_raw(201))
        );
    }
    ...
    fn test_reverse_resolution() {
        ...
        assert_eq!(
            session.resolve_koids(Koid::from_raw(1000), Koid::from_raw(9999)),
            None
        );
        assert!(
            !manager
                ...
                .read()
                .unmapped_processes
                .contains(&Koid::from_raw(1000))
        );

        // Native Fuchsia process: returns None and the process lands in the negative cache.
        assert_eq!(
            session.resolve_koids(Koid::from_raw(8), Koid::from_raw(7)),
            None
        );
        assert!(
            manager
        ...
        drop(session);
        assert_eq!(
            manager.resolve_koids(Koid::from_raw(1000), Koid::from_raw(2000)),
            None
        );
    }
    ...
    async fn test_overlapping_sessions() {
        spawn_kernel_and_run(async move |current_task| {
            ...
            assert_eq!(
                session_b.resolve_tid_to_koid(999),
                Some(Koid::from_raw(9901))
            );
