...
impl SchedulerManager {
    ...
    pub fn role_name_inner(
        ...
        if thread_group_state.did_exec {
            let process_name = if task.tid == task.pid {
                task.command()
            } else {
                task.pid.get_task().map_err(|_| errno!(EINVAL))?.command()
            };
    ...
    pub fn resolve_role_name(
        ...
        if let Some(name) =
            self.role_overrides
                .get_role_name(process_name, thread_name, cgroup_path)
        {
            return name;
        }
    ...
    fn set_thread_role_legacy(
        ...
        let _ = role_manager
            .set_role(request, zx::MonotonicInstant::INFINITE)
            .map_err(|err| {
...
impl NormalPriority {
    ...
    pub fn from_binder(user_nice: i8) -> Result<Self, Errno> {
        ...
        Ok(Self {
            value: u8::try_from(value).expect("normal priority should fit in a u8"),
        })
    }
...
impl RealtimePriority {
    ...
    const NON_REAL_TIME: RealtimePriority = RealtimePriority {
        value: Self::NON_REAL_TIME_VALUE,
    };
...
mod tests {
    ...
    fn build_policy_from_binder() {
        ...
        assert_eq!(
            SchedulerState::from_binder(SCHED_IDLE as u8, 11),
            error!(EINVAL)
        );
    ...
    async fn role_overrides_non_realtime() {
        crate::testing::spawn_kernel_and_run_sync(|current_task| {
            ...
            let role = manager
                .role_name_inner(current_task, state)
                .expect("role_name");
