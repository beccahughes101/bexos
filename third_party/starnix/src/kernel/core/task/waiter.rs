...
impl SignalHandler {
    fn handle(self, signals: zx::Signals) -> Option<Errno> {
        ...
        let events = match inner {
            ...
            SignalHandlerInner::Zxio(ZxioSignalHandler {
                zxio,
...
impl WaitCanceler {
    fn new_inner(inner: WaitCancelerInner) -> Self {
        Self {
            cancellers: smallvec::smallvec![inner],
        }
    }

    pub fn new_noop() -> Self {
        Self {
            cancellers: Default::default(),
        }
    }
        ...
        for canceller in cancellers.into_iter().rev() {
            match canceller {
                ...
                WaitCancelerInner::Queue(WaitCancelerQueue {
                    ...
                    let Some(wait_queue) = wait_queue.upgrade() else {
                        return;
                    };
...
impl PortWaiter {
    ...
    fn wait_internal(&self, deadline: zx::MonotonicInstant) -> Result<(), Errno> {
        ...
        match self.port.wait(deadline) {
            PortWaitResult::Notification {
                kind: NotifyKind::Regular,
            } => Ok(()),
            PortWaitResult::Notification {
                kind: NotifyKind::Interrupt,
            } => error!(EINTR),
...
impl Waiter {
    ...
    pub fn new() -> Self {
        Self {
            inner: PortWaiter::new(WaiterOptions::empty()),
        }
    }
    ...
    fn create_wait_entry_with_handler(
        ...
        let key = self
            .inner
            .register_callback(WaitCallback::EventHandler(handler));
        WaitEntry {
            waiter: self.weak(),
            filter,
            key,
    ...
    pub fn wake_on_zircon_signals(
        ...
        self.inner
            .wake_on_zircon_signals(handle, zx_signals, handler)
    }
        ...
        for wait_queue in wait_queues {
            if let Some(wait_queue) = wait_queue.upgrade() {
                wait_queue
                    .lock()
                    .waiters
                    .retain(|_, entry| entry.entry.waiter != *self)
            }
        }
    }
}
        ...
        for wait_queue in &self.wait_queues {
            if let Some(wait_queue) = wait_queue.upgrade() {
                wait_queue
                    .lock()
                    .waiters
                    .retain(|_, entry| entry.entry.waiter != self.event)
            }
        }
    }
}
...
impl WaitQueue {
    fn add_waiter(&self, entry: WaitEntry) -> WaitEntryId {
        ...
        WaitEntryId {
            key: wait_queue.waiters.insert(WaitEntryWithId { entry, id }),
            id,
        }
    }
    ...
    fn wait_async_entry(&self, waiter: &Waiter, entry: WaitEntry) -> WaitCanceler {
        ...
        waiter
            ...
            .wait_queues
            .lock()
            .insert(wait_key, wait_queue.clone());
...
impl<T: Into<u64>> TypedWaitQueue<T> {
    ...
    pub fn wait_async_value_with_handler(
        ...
        self.wait_queue
            .wait_async_value_with_handler(waiter, value.into(), handler)
    }
...
mod tests {
    ...
    async fn test_async_wait_exec() {
        spawn_kernel_and_run(async |current_task| {
            ...
            pipe.wait_async(&current_task, &waiter, FdEvents::POLLIN, handler)
                .expect("wait_async");
            ...
            queue
                .lock()
                .iter()
                .for_each(|item| assert!(item.events.contains(FdEvents::POLLIN)));
    ...
    async fn single_waiter_multiple_waits_cancel_one_waiter_still_notified() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert!(
                waiter
                    .wait_until(&current_task, zx::MonotonicInstant::ZERO)
                    .is_ok()
            );
        })
        .await;
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            let woken = || {
                waiters
                    .iter()
                    .filter(|w| {
                        w.wait_until(&current_task, zx::MonotonicInstant::ZERO)
                            .is_ok()
                    })
                    .count()
            };
