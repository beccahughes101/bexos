...
impl KernelThreads {
    ...
    pub fn init(&self, system_task: CurrentTask) -> Result<(), Errno> {
        self.system_task
            .set(SystemTask::new(system_task))
            .map_err(|_| errno!(EEXIST))?;
        self.spawner
            .set(DynamicThreadSpawner::new(
                2,
                self.system_task().weak_task(),
                "kthreadd/init",
            ))
            .map_err(|_| errno!(EEXIST))?;
...
impl<F, C: Clone> WrappedFuture<F, C> {
    pub(crate) fn new_with_cleaner(context: C, cleaner: fn(C), fut: F, name: &'static str) -> Self {
        ...
        Self {
            fut,
            cleaner,
            context: ScopeGuard::with_strategy(context, cleaner),
            name,
        }
    }
}
