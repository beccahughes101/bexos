...
impl<T: RcuDroppable + Send + Sync + 'static> Idr<T> {
    ...
    fn find_free_slot<'a>(
        ...
        while let Some(top) = stack.last_mut() {
            ...
            if node.layer == 0 {
                let id = stack.iter().fold(0u32, |acc, entry| {
                    acc | entry.node.id_for_index(entry.chosen_index)
                });
                path.extend(
                    stack
                        .into_iter()
                        .map(|entry| (entry.node, entry.chosen_index)),
                );
                return Some(id);
            }
...
impl<'a, T: RcuDroppable + Send + Sync + 'static> IdrGuard<'a, T> {
        ...
        F: FnOnce(u32) -> Arc<T>,
    {
        ...
        let id_opt = self
            .idr
            .find_free_slot(start_id, max_id, &mut path, &scope)
            .or_else(|| {
...
impl<'a, T: RcuDroppable + Send + Sync + 'static> Iterator for IdrIterator<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some((node, index, id_base)) = self.stack.pop() {
            ...
            let mask = if index >= NODE_CAPACITY {
                0
            } else {
                !((1u64 << index) - 1)
            };
...
mod tests {
    ...
    fn test_basic_alloc_lookup() {
        ...
        let (id1, _item1) = idr
            .lock()
            .alloc(|id| Arc::new(MockItem { value: id * 10 }))
            .unwrap();
        assert_eq!(id1, 0);
    ...
    fn test_tree_growth() {
        ...
        for i in 0..150 {
            let (id, item) = idr
                .lock()
                .alloc(|id| Arc::new(MockItem { value: id }))
                .unwrap();
            _items.push(item);
            assert_eq!(id, i);
        }
    ...
    fn test_reserve_id() {
        ...
        assert!(
            !allocated_200,
            "ID 200 was allocated despite being reserved"
        );
    ...
    fn test_iter_and_remove() {
        ...
        _items.push(
            idr.lock()
                .alloc(|_| Arc::new(MockItem { value: 10 }))
                .unwrap()
                .1,
        );
        ...
        _items.push(
            idr.lock()
                .alloc(|_| Arc::new(MockItem { value: 20 }))
                .unwrap()
                .1,
        );
        ...
        _items.push(
            idr.lock()
                .alloc(|_| Arc::new(MockItem { value: 30 }))
                .unwrap()
                .1,
        );
    ...
    fn test_iter_minimal() {
        ...
        _items.push(
            idr.lock()
                .alloc(|value| Arc::new(MockItem { value }))
                .unwrap()
                .1,
        );
        let scope = RcuReadScope::new();
    ...
    fn test_alloc_cyclic_min_after_wrap() {
        ...
        let (id0_after, _) = guard_zero
            .alloc(|value| Arc::new(MockItem { value }))
            .unwrap();
        assert_eq!(id0_after, 0);

        let (id1_after, _) = guard_zero
            .alloc(|value| Arc::new(MockItem { value }))
            .unwrap();
    ...
    fn test_linear_alloc_max() {
        ...
        let mut guard = idr.lock();
        assert_eq!(
            guard.alloc(|value| Arc::new(MockItem { value })).unwrap().0,
            0
        );
        assert_eq!(
            guard.alloc(|value| Arc::new(MockItem { value })).unwrap().0,
