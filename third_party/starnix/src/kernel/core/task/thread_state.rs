...
impl ArchExtendedPstateStorage {
    ...
    pub fn as_ptr(&mut self) -> ExtendedPstatePointer {
        match self {
            ArchExtendedPstateStorage::State64(state) => ExtendedPstatePointer {
                extended_pstate: state.as_mut() as *mut _,
            },
            #[cfg(target_arch = "aarch64")]
            ArchExtendedPstateStorage::State32(state) => ExtendedPstatePointer {
                extended_aarch32_pstate: state.as_mut() as *mut _,
            },
        }
    }
...
impl<T: RegisterStorage> ThreadState<T> {
    pub fn arch_width(&self) -> ArchWidth {
        #[cfg(target_arch = "aarch64")]
        {
            return if self.registers.is_arch32() {
                ArchWidth::Arch32
            } else {
                ArchWidth::Arch64
            };
    ...
    pub fn get_user_register(&mut self, offset: usize) -> Result<usize, Errno> {
        ...
        self.registers
            .apply_user_register(offset, &mut |register| result = *register as usize)?;
        Ok(result)
    }
    ...
    pub fn set_user_register(&mut self, offset: usize, value: usize) -> Result<(), Errno> {
        ...
        let result = self
            .registers
            .apply_user_register(offset, &mut |register| *register = value as u64);
