...
pub fn build_block_device_directory(
    ...
    dir.subdir("queue", 0o755, |dir| {
        dir.entry(
            "nr_requests",
            BytesFile::new_node(NrRequestsFile::new()),
            mode!(IFREG, 0o644),
        );
        dir.entry(
            "read_ahead_kb",
            BytesFile::new_node(ReadAheadKbFile),
            mode!(IFREG, 0o644),
        );
        dir.entry(
            "scheduler",
    ...
    dir.entry(
        "size",
        BlockDeviceSizeFile::new_node(block_info),
        mode!(IFREG, 0o444),
    );
}
...
impl BytesFileOps for ReadAheadKbFile {
    fn write(&self, _current_task: &CurrentTask, _data: Vec<u8>) -> Result<(), Errno> {
        track_stub!(
            TODO("https://fxbug.dev/297295673"),
            "updating read_ahead_kb"
        );
        Ok(())
    }
