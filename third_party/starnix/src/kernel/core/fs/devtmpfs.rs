...
impl DevTmpFs {
    pub fn from_kernel(kernel: &Kernel) -> FileSystemHandle {
        ...
        kernel
            .expando
            .get_or_init(|| DevTmpFsHandle(Self::init(kernel)))
            .0
            .clone()
    }
...
fn devtmpfs_create_device_internal(
    ...
    let separator_pos = device_metadata
        .devname
        .iter()
        .rposition(|&c| c == path::SEPARATOR);
...
pub fn devtmpfs_remove_path(current_task: &CurrentTask, path: &FsStr) -> Result<(), Errno> {
    ...
    parent_node
        .entry
        .remove_child(device_name.into(), &current_task.kernel().mounts);
    Ok(())
}
