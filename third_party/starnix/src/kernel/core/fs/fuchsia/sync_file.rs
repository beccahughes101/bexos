    ...
    fn ioctl(
        ...
        match ioctl_number {
            SYNC_IOC_MERGE => {
                ...
                let file2 = current_task
                    .files()
                    .get(FdNumber::from_raw(merge_data.fd2))?;
                ...
                let mut fence = SyncFence {
                    sync_points: Vec::with_capacity(max_capacity),
                };
                ...
                if fence.sync_points.is_empty() {
                    fence
                        .sync_points
                        .push(last_signaled_sync_point.expect("No sync points left."));
                }
...
mod test {
    ...
    async fn test_sync_file_merge() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let merge_data = sync_merge_data {
                name: [0; 32],
                fd2: fd2.raw(),
                fence: 0,
                flags: 0,
            ...
            current_task
                .write_object(UserRef::new(user_addr), &merge_data)
                .unwrap();
            ...
            let res = sync_file1
                .ioctl(&file1, &current_task, request, user_addr.into())
                .unwrap();
