...
impl FileSystemOps for RemoteVolume {
    ...
    fn rename(
        ...
        self.remotefs
            .rename(fs, current_task, context, old_name, new_name)
    }
...
impl VolumeKeys {
    ...
    fn create(data: &fio::DirectorySynchronousProxy, key_path: &str) -> Result<Self, Errno> {
        ...
        let (status, token) = data
            ...
            .map_err(|e| {
                log_error!(
                    "transport error on get_token for the data directory, error: {:?}",
                    e
                );
                errno!(ENOENT)
            })?;
        zx::Status::ok(status).map_err(|e| {
            let err = from_status_like_fdio!(e);
            log_error!(
                "Failed to get_token for the data directory, error: {:?}",
                err
            );
            err
        })?;
...
pub fn new_remote_vol(
    ...
    let mode = if created_key_file {
        ...
        volume_provider
            ...
            .map_err(|e| {
                log_error!(
                    "FIDL transport error on StarnixVolumeProvider.Check {:?}",
                    e
                );
                errno!(ENOENT)
            })?
    ...
    let guid = volume_provider
        ...
        .map_err(|e| {
            log_error!(
                "FIDL transport error on StarnixVolumeProvider.Mount {:?}",
                e
            );
            errno!(ENOENT)
        })?
