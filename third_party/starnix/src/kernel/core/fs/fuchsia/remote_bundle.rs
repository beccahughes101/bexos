    ...
    pub fn new_fs_in_base(
        ...
        let metadata = {
            ...
            file.read_to_end(&mut buf)
                .source_context("read metadata file")?;
        ...
        let fs = FileSystem::new(
            kernel,
            CacheMode::Cached(CacheConfig {
                capacity: REMOTE_BUNDLE_NODE_LRU_CAPACITY,
            }),
            RemoteBundle {
                metadata,
                root,
...
impl FileSystemOps for RemoteBundle {
    fn statfs(&self, _fs: &FileSystem, _current_task: &CurrentTask) -> Result<statfs, Errno> {
        ...
        track_stub!(
            TODO("https://fxbug.dev/540853536"),
            "RemoteBundle statfs block count"
        );
    ...
    fn update_flags(
        ...
        fs.options
            .flags
            .store(flags | FileSystemFlags::RDONLY, Ordering::Relaxed);
        Ok(())
    }
}
...
impl Inner {
    fn get_memory(&mut self) -> Result<Arc<MemoryObject>, Errno> {
        ...
        let Inner::Memory(memory) = &*self else {
            unreachable!()
        };
        Ok(memory.clone())
    }
        ...
        match metadata_node.info() {
            ...
            NodeInfo::File(_) => {
                ...
                Ok(fs.create_node(
                    ino,
                    File {
                        inner: Inner::NeedsVmo(file).into(),
                    },
                    info,
                ))
            }
...
fn to_fs_node_info(metadata_node: &ext4_metadata::Node) -> FsNodeInfo {
    ...
    let owner = FsCred {
        uid: metadata_node.uid.into(),
        gid: metadata_node.gid.into(),
    };
...
mod test {
    ...
    async fn test_read_image() {
        spawn_kernel_and_run_with_pkgfs(async |current_task| {
            ...
            if let SymlinkTarget::Path(target) = test_symlink
            ...
            assert_eq!(
                ...
                [
                    (
                        b".".into(),
                        (test_dir.entry.node.ino, DirectoryEntryType::DIR)
                    ),
                    (b"..".into(), (root.entry.node.ino, DirectoryEntryType::DIR)),
                    (
                        b"file".into(),
                        (test_file.node().ino, DirectoryEntryType::REG)
                    ),
                    (
                        b"symlink".into(),
                        (test_symlink.entry.node.ino, DirectoryEntryType::LNK)
                    )
                ]
                .into()
            );
