...
impl SocketOps for RemoteUnixDomainSocket {
    ...
    fn read(
        ...
        let written = if let Some(received_data) = response.data {
...
mod tests {
    ...
    impl Data {
        fn try_clone(&mut self) -> Result<Self, zx::Status> {
            ...
            let new_handles = new_handles
                .into_iter()
                .collect::<Result<Vec<zx::NullableHandle>, zx::Status>>()?;
            Ok(Self {
        ...
        fn read(
            ...
            if state.buffer.is_empty() {
                state
                    .remote_event
                    .as_handle_ref()
                    .signal(READABLE_SIGNAL, WRITABLE_SIGNAL)?;
            }
    ...
    async fn test_remote_uds() {
        ...
        spawn_kernel_and_run(async move |current_task| {
            ...
            let file = new_remote_file(
                current_task,
                original_file
                    .to_handle(current_task)
                    .expect("to_handle")
                    .expect("has_handle"),
                OpenFlags::RDWR,
            )
            .expect("new_remote_file");
            let ancillary_data = vec![AncillaryData::Unix(UnixControlData::Rights(vec![
                original_file,
            ]))];
    ...
    async fn test_remote_uds_peer_closed() {
        ...
        let handle = std::thread::spawn(move || {
            let mut executor = fasync::LocalExecutor::default();
            executor.run_singlethreaded(async move {
                let uds_impl = UnixDomainSocketImpl {
                    close_on_read: true,
                    ..Default::default()
                };
                Arc::new(uds_impl).serve(server).await;
