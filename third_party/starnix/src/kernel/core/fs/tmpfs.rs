...
impl FsNodeOps for TmpFsDirectory {
    ...
    fn mkdir(
        ...
        Ok(node
            .fs()
            .create_node_and_allocate_node_id(TmpFsDirectory::new(), info))
    }
    ...
    fn unlink(
        ...
        if child_to_unlink.is_dir() {
            ...
            // - TmpFsDirectory is the ops for directories in this filesystem.
            let child_count = &child_to_unlink
                .downcast_ops::<TmpFsDirectory>()
                .unwrap()
                .child_count;
...
mod test {
    ...
    async fn test_write_read() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let wr_file = current_task
                .open_file(path.into(), OpenFlags::RDWR)
                .unwrap();
    ...
    async fn test_read_past_eof() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _file = current_task
                ...
                .unwrap();
            let rd_file = current_task
                .open_file(path.into(), OpenFlags::RDONLY)
                .unwrap();
            ...
            let test_offset = 100;
            let result = rd_file
                .read_at(&current_task, test_offset, &mut output_buffer)
                .unwrap();
            assert_eq!(result, 0);
        })
        .await;
    ...
    async fn test_permissions() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_eq!(
                0,
                file.read(&current_task, &mut VecOutputBuffer::new(0))
                    .expect("failed to read")
            );

            assert!(
                file.write(&current_task, &mut VecInputBuffer::new(&[]))
                    .is_err()
            );
        ...
        spawn_kernel_and_run(async |current_task| {
            {
                ...
                let usr = root
                    .create_dir(&current_task, "usr".into())
                    .expect("failed to create usr");
                root.create_dir(&current_task, "etc".into())
                    .expect("failed to create usr/etc");
                usr.create_dir(&current_task, "bin".into())
            ...
            assert_eq!(
                0,
                txt.read(&current_task, &mut VecOutputBuffer::new(0))
                    .expect("failed to read")
            );
            std::mem::drop(txt);
            assert_eq!(
            ...
            assert_eq!(
                errno!(ENOENT),
                current_task
                    .open_file("/usr/foo".into(), OpenFlags::RDONLY)
                    .unwrap_err()
            );
            usr.name
                .unlink(
