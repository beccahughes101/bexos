...
pub fn create_zircon_process(
    ...
    let (process, root_vmar) = create_shared(
...
pub fn create_init_child_process(
    ...
    let security_state = if let Some(seclabel) = seclabel {
        ...
        security::task_for_context(&init_task, b"".into()).map_err(|_| {
            errno!(
                EINVAL,
                "Container has SELinux enabled but no Security Context specified"
            )
        })?
    };
    ...
    )?;
    {
        ...
        new_process_writer.parent = Some(ThreadGroupParent::new(Arc::downgrade(
            &init_task.thread_group(),
        )));
        init_writer
            .children
            .insert(task.tid.id, Arc::downgrade(task.thread_group()));
    }
    ...
    F: FnOnce(Pid, Arc<ProcessGroup>) -> Result<TaskInfo, Errno>,
{
    ...
    release_on_error!(builder, {
        ...
        for (resource, limit) in rlimits {
            builder.thread_group().limits.lock().set(
                *resource,
                rlimit {
                    rlim_cur: *limit,
                    rlim_max: *limit,
...
pub fn create_kernel_thread(
    ...
    release_on_error!(current_task, {
        current_task
            .thread_group()
            .add(Arc::clone(&current_task.task))?;
