...
impl CrashReporter {
    ...
    pub fn begin_crash_report(&self, current_task: &CurrentTask) -> Option<PendingCrashReport> {
        ...
        self.throttler
            .should_report(argv, argv0, zx::MonotonicInstant::get())
    }
    ...
    pub fn handle_core_dump(
        ...
        let thread_koid = current_task
            ...
            .koid;
        let linux_pid = current_task.pid.id as i64;
        ...
        let crash_report = CrashReport {
            ...
            annotations: Some(vec![
                ...
                Annotation {
                    key: "linux.pid".to_string(),
                    value: linux_pid.to_string(),
                },
                Annotation {
                    key: "linux.argv".to_string(),
                    value: argv_joined,
                },
                Annotation {
                    key: "linux.env".to_string(),
                    value: env_joined,
                },
                Annotation {
                    key: "linux.signal".to_string(),
                    value: signal_str,
                },
            ]),
            is_fatal: Some(true),
...
mod tests {
    ...
    fn truncate_adds_ellipsis() {
        ...
        assert_eq!(
            s, "123456...",
            "truncate must add ellipsis and still fit under max len"
        );
    }
    ...
    fn truncate_is_sensible_in_middle_of_multibyte_chars() {
        ...
        assert_eq!(
            s.len(),
            7,
            "may end up shorter than provided max length w/ multi-byte chars"
        );
        assert_eq!(
            s, "ææ...",
            "truncate must remove whole characters and add ellipsis"
        );
    }
}
