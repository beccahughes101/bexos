...
const UCONTEXT_RESERVED_DATA_SIZE: usize = max_const(
    std::mem::size_of::<uapi::ucontext>(),
    std::mem::size_of::<uapi::arch32::ucontext>(),
);
...
impl SignalStackFrame {
    ...
    fn get_ucontext32(&self) -> &uapi::arch32::ucontext {
        uapi::arch32::ucontext::ref_from_prefix(&self.context)
            .unwrap()
            .0
    }
}
            ...
            loop {
                match _aarch64_ctx::read_from_prefix(&data[offset..]) {
                    ...
                    Ok((
                        ...
                        log_debug!(
                            "Invalid size for `fpsimd_context` in `sigcontext`: {}",
                            size
                        );
                        return error!(EINVAL);
                    }
                    ...
                    Ok((
                        ...
                        track_stub!(
                            TODO("https://fxbug.dev/322873793"),
                            "sigcontext EXTRA_MAGIC"
                        );
                        offset += ESR_CONTEXT_SIZE as usize;
                    }
