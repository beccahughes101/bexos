...
pub fn execute_syscall(
    ...
    current_task
        .thread_state
        .registers
        .save_registers_for_restart(syscall.decl.number);
    ...
    let return_value = match result {
        Ok(return_value) => {
            ...
            current_task
                .thread_state
                .registers
                .set_return_register(return_value.value());
            None
        }
        Err(errno) => {
...
pub fn process_completed_restricted_exit(
    ...
    if let Some(ExitStatus::CoreDump(signal_info)) = &result {
        if current_task.flags().contains(TaskFlags::DUMP_ON_EXIT) {
            ...
            if let Some(pending_report) = current_task
                .kernel()
                .crash_reporter
                .begin_crash_report(&current_task)
            {
