...
async fn dump_thread_backtrace<'a>(
    ...
    file_buffer
        .reset()
        .map_err(|e| anyhow::anyhow!("Failed to reset CFileBuffer: {}", e))?;
...
fn build_annotations(lockups: &Lockups, rcu_stalls: &Vec<RcuStall>) -> Vec<ffeedback::Annotation> {
    ...
    let rcu_koids_str = format!(
        "{:?}",
        rcu_stalls
            .iter()
            .map(|k| k.koid.raw_koid())
            .collect::<Vec<_>>()
    );
...
async fn report_lockups(
    ...
    for (thread, koid, start_time) in sorted_threads {
        ...
        let vmo = match zx::Vmo::create(size) {
            ...
            Err(e) => {
                log_warn!(
                    "Failed to create VMO for thread {}: {:?}",
                    koid.raw_koid(),
                    e
                );
                continue;
        ...
        if let Err(e) = vmo.write(bt.as_bytes(), 0) {
            log_warn!(
                "Failed to write backtrace to VMO for thread {}: {:?}",
                koid.raw_koid(),
                e
            );
            continue;
        ...
        match reporter.file_report(report).await {
            Ok(Ok(_)) => {
                log_debug!(
                    "Filed crash report for thread lockup (thread {}).",
                    koid.raw_koid()
                );
            }
            Ok(Err(e)) => {
            ...
            Err(e) => {
                log_warn!(
                    "FIDL error calling file_report for thread {}: {:?}",
                    koid.raw_koid(),
                    e
                );
            }
...
fn check_lockups(context: &mut LockupDetectorContext) -> Lockups {
    ...
    context
        .reported_lockup_koids
        .retain(|koid| current_koids.contains(koid));
...
fn check_rcu_stalls(context: &mut LockupDetectorContext) -> Vec<RcuStall> {
    ...
    context
        .active_rcu_reads
        .retain(|koid, _| active_koids.contains(koid));
    context
        .reported_rcu_koids
        .retain(|koid| active_koids.contains(koid));
...
pub fn start_thread_lockup_detector() -> fasync::Task<()> {
    fasync::Task::spawn(async {
        ...
        loop {
            fasync::Timer::new(zx::MonotonicInstant::after(
                zx::MonotonicDuration::from_minutes(LOCKUP_DETECTOR_INTERVAL_MINUTES),
            ))
            .await;
...
pub fn inspect_lazy_node_callback() -> BoxFuture<'static, Result<Inspector, anyhow::Error>> {
    Box::pin(async {
        ...
        for info in long_running {
            ...
            inspector
                .root()
                .record_string(info.koid.raw_koid().to_string(), name);
        }
        Ok(inspector)
    })
}
