...
pub async fn start_component(
    ...
    let weak_task = system_task.override_creds(
        ...
        || {
            ...
            let mount_record = Arc::new(LockDepMutex::<_, ComponentMountRecordLock>::new(
                MountRecord::default(),
            ));
            ...
            let ns_mount_options = system_task
                .kernel()
                .features
            ...
            for entry in ns {
                if let (Some(dir_path), Some(dir_handle)) = (entry.path, entry.directory) {
                    ...
                    let mount_options = ns_mount_options
                        .and_then(|mount_options| mount_options.get(dir_path_str).cloned());
                    ...
                    match dir_path_str {
                        ...
                        _ => {
                            ...
                            mount_record
                                ...
                                .with_context(|| {
                                    format!(
                                        "failed to mount_remote on path {component_path}/{dir_path}"
                                    )
                                })?;
                            if dir_path == "/pkg" {
            ...
            let uid = program
                .uid
                .map(|uid| uid.0)
                .unwrap_or_else(|| system_task.kernel().features.default_uid);
            ...
            run_component_features(system_task.kernel(), &program.features, maybe_svc)
                .unwrap_or_else(|e| {
...
impl MountRecord {
    ...
    fn mount_remote(
        ...
        let mut current_node = system_task
        ...
        directory
            .clone(ServerEnd::new(server_end))
            .context("cloning directory")?;
        ...
        current_node
            .mount(WhatToMount::Fs(fs), flags.mountpoint_flags())
            .context("mounting fs")?;
