        ...
        (payload.console_in, payload.console_out, payload.binary_path)
    {
        ...
        let current_task = create_init_child_process(
            &kernel
                .weak_self
                .upgrade()
                .expect("Kernel must still be alive"),
        ...
        .try_for_each_concurrent(None, |event| async {
            match event {
                fstarcontainer::ControllerRequest::VsockConnect {
                    payload:
                        fstarcontainer::ControllerVsockConnectRequest {
                            port,
                            bridge_socket,
                            ..
                        },
                    ...
                    connect_to_vsock(port, bridge_socket, system_task)
                        .await
                        .unwrap_or_else(|e| {
                ...
                fstarcontainer::ControllerRequest::GetVmoReferences { payload, responder } => {
                    if let Some(koid) = payload.koid {
                        let mut results = vec![];
                        for thread_group in system_task
                            .kernel()
                            .pids
...
async fn connect_to_vsock(
    ...
    let socket = loop {
        if let Ok(socket) = system_task
            .kernel()
            .default_abstract_vsock_namespace
            .lookup(&port)
        {
            break socket;
        };
    ...
    socket
        ...
        .unwrap()
        .remote_connection(&socket, system_task, pipe)?;
...
pub fn serve_memory_attribution_provider_container(
    ...
    async move {
        while let Some(event) = request_stream
            ...
            .inspect_err(|err| {
                log_warn!(
                    "Error while serving container memory attribution: {:?}",
                    err
                )
            })
            .ok()
...
mod tests {
    ...
    async fn container_info_test() {
        ...
        let client_fut = async move {
            let response = info_proxy
                .get_job_koid()
                .await
                .expect("get_job_koid failed");
