...
pub fn parse_features(
        ...
        .chain(extra_features.iter())
    {
        ...
        match (feature, raw_args) {
            ...
            (Feature::AspectRatio, Some(args)) => {
                ...
                let width: u32 = components[0]
                    .parse()
                    .map_err(|_| anyhow!("Invalid aspect ratio width"))?;
                let height: u32 = components[1]
                    .parse()
                    .map_err(|_| anyhow!("Invalid aspect ratio height"))?;
            ...
            (Feature::Bpf, None) => {
                return Err(anyhow!(
                    "bpf feature must have an argument (e.g. \"bpf:v2\")"
                ));
            }
    ...
    if features.boot_notifier_cpu_boost.is_some() && !features.boot_notifier {
        return Err(anyhow!(
            "boot_notifier_cpu_boost feature requires boot_notifier"
        ));
    }
...
pub fn run_container_features(kernel: &Arc<Kernel>, features: &Features) -> Result<(), Error> {
    if features.framebuffer {
        ...
        *framebuffer.view_bound_protocols.lock() = Some(view_bound_protocols);
        ...
        touch_device
            .clone()
            .register(kernel, DEFAULT_TOUCH_DEVICE_ID)?;
        keyboard_device
            .clone()
            .register(kernel, DEFAULT_KEYBOARD_DEVICE_ID)?;
        mouse_device
            .clone()
            .register(kernel, DEFAULT_MOUSE_DEVICE_ID)?;
