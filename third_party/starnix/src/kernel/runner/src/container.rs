...
impl ContainerMemoryAttributionManager {
    ...
    pub fn new(kernel: Weak<Kernel>, component_instance: zx::Event) -> Self {
        ...
        ContainerMemoryAttributionManager {
...
fn attribution_info_for_kernel(
    ...
    fuchsia_async::Task::spawn(serve_memory_attribution_provider_container(
        server_end, kernel,
    ))
    .detach();
    ...
    let starnix_kernel_principal = fattribution::NewPrincipal {
        ...
        description: Some(fattribution::Description::Part(
            "starnix_kernel".to_string(),
        )),
    ...
    let starnix_kernel_attribution = fattribution::UpdatedPrincipal {
        ...
        resources: Some(fattribution::Resources::Data(fattribution::Data {
            resources: vec![fattribution::Resource::ProcessMapped(
                fattribution::ProcessMapped {
...
impl ContainerStartInfo {
    fn new(mut start_info: frunner::ComponentStartInfo) -> Result<Self, Error> {
        ...
        let ns = start_info
            .ns
            .take()
            .context("retrieving container namespace")?;
...
impl Container {
    ...
    async fn serve_outgoing_directory(
        ...
        if let Some(outgoing_dir) = outgoing_dir {
            ...
            fs.serve_connection(outgoing_dir.into())
    ...
    if let Some(event) = request_stream.try_next().await? {
        match event {
            frunner::ComponentRunnerRequest::Start {
                ...
                let service_config = ContainerServiceConfig {
                    start_info,
                    request_stream,
                    receiver,
                };
    ...
    device_tree
        ...
        .and_then(|n| {
            n.get_property("bootargs").map(|p| {
                let end = if p.value.last() == Some(&0) {
                    p.value.len() - 1
                } else {
                    p.value.len()
                };
...
async fn create_container(
    ...
    if features.android_bootreason {
        ...
        let tmp_channel = start_info
            .container_namespace
            .get_namespace_channel("/tmp_lifecycle");
        let tmp_proxy = match tmp_channel {
            Ok(channel) => Some(fio::DirectoryProxy::new(fidl::AsyncChannel::from_channel(
                channel,
            ))),
            _ => None,
        };
    ...
    let node = inspect::component::inspector()
        .root()
    ...
    let init_pid = kernel
        ...
        .lock()
        .allocate_pid()
        .source_context("allocating init pid")?;
    ...
    kernel
        .syslog
        .init(&kernel)
        .source_context("initializing syslog")?;
    ...
    if !start_info.program.startup_file_path.is_empty() {
        wait_for_init_file(
            &start_info.program.startup_file_path,
            &system_task,
            init_tid,
        )
        .await?;
    ...
    let memory_attribution_manager = ContainerMemoryAttributionManager::new(
        ...
        start_info
            .component_instance
            .take()
            .ok_or_else(|| Error::msg("No component instance"))?,
    );
...
fn create_fs_context(
    ...
    let mut mounts_iter = start_info
        .program
        .mounts
        .iter()
        .chain(start_info.config.additional_mounts.iter());
    let root = MountAction::new_for_root(
        kernel,
        pkg_dir_proxy,
        mounts_iter
            .next()
            .ok_or_else(|| anyhow!("Mounts list is empty"))?,
    )?;
    if root.path != "/" {
    ...
    if features.container {
        ...
        let container_remotefs_options = FileSystemOptions {
            ...
            params: kernel
                .features
                .ns_mount_options("#container")
                .context("#container options")?,
            ..Default::default()
        };
    ...
    if features.test_data {
        let mount_options = FileSystemOptions {
            params: kernel
                .features
                .ns_mount_options("#test_data")
                .context("#test_data options")?,
            ..Default::default()
        };
        let fs = TmpFs::new_fs_with_options(kernel, mount_options)?;
    ...
    Ok((
        FsContext::new(Namespace::new_with_flags(root_fs, root.flags)),
        feature_mounts,
    ))
}
...
fn parse_rlimits(rlimits: &[String]) -> Result<Vec<(Resource, u64)>, Error> {
    ...
    for rlimit in rlimits {
        let (key, value) = rlimit
            .split_once('=')
            .ok_or_else(|| anyhow!("Invalid rlimit: {rlimit}"))?;
...
fn init_remote_block_devices(kernel: &Kernel) -> Result<(), Error> {
    ...
    for entry in entries {
        ...
        let path = path_buf
            .to_str()
            .ok_or_else(|| anyhow!("Invalid block device path"))?;
...
fn create_job_id_vmo() -> Result<zx::Vmo, Error> {
    let job_id = fuchsia_runtime::job_default()
        .koid()
        .context("reading own job koid")?;
    let job_id_str = job_id.raw_koid().to_string();
    let job_id_vmo = zx::Vmo::create(job_id_str.len() as u64).context("creating job id vmo")?;
    job_id_vmo
        .write(job_id_str.as_bytes(), 0)
        .context("write job id to vmo")?;
    Ok(job_id_vmo)
}
