...
fn maybe_serve_lifecycle(container: Rc<OnceCell<Container>>) -> Option<fasync::Task<()>> {
    if let Some(lifecycle) = fruntime::take_startup_handle(fruntime::HandleInfo::new(
        fruntime::HandleType::Lifecycle,
        0,
    )) {
...
async fn main() -> Result<(), Error> {
    ...
    fruntime::job_default()
        .set_critical(
            zx::JobCriticalOptions::RETCODE_NONZERO,
            &fruntime::process_self(),
        )
    ...
    let KernelStructuredConfig {
        extra_features: kernel_extra_features,
        prefetch,
        ..
    ...
    if prefetch {
        ...
        vmo.op_range(zx::VmoOp::ALWAYS_NEED, 0, size)
            .context("pin executable vmo")?;
    ...
    fs.dir("svc")
        .add_fidl_service_at(
            ...
            KernelServices::ContainerRunner,
        )
    ...
    inspector
        .root()
        .record_lazy_child("not_found", starnix_logging::not_found_lazy_node_callback);
    inspector
        .root()
        .record_lazy_child("stubs", starnix_logging::track_stub_lazy_node_callback);
    inspector.root().record_lazy_child(
        "thread_lockup_detector",
        lockup_detector::inspect_lazy_node_callback,
    );
                ...
                fuchsia_async::Task::local(async move {
                    ...
                    let container = container
                        .get_or_try_init(|| async {
                            create_component_from_stream(stream, kernel_extra_features)
                                .await
                                .map(|(container, new_config)| {
