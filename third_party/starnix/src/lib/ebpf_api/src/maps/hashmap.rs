...
mod internal {
    ...
    impl<'a> FreeListState<'a> {
        fn new(store: HashMapStore<'a>) -> RwMapLock<'a, Self> {
            ...
            unsafe {
                let hash_table_header = store
                    .buf
                    .ptr()
                    .get_ptr::<HashTableHeader>(0)
                    .unwrap()
                    .deref();
                RwMapLock::new(
                    ...
                    LOCK_SIGNAL,
                    FreeListState {
                        header: &hash_table_header.free_list_header,
                        store,
                    },
                )
            }
        ...
        pub fn alloc(&mut self) -> Option<HashMapEntryRef<'a>> {
                ...
                && num_used_entries < self.store.layout.max_entries
            {
                ...
                self.header
                    .num_used_entries
                    .store(num_used_entries + 1, Ordering::Relaxed);
    ...
    impl<'a> DataEntry<'a> {
        ...
        pub fn key(&self) -> EbpfBufferPtr<'a> {
            ...
            self.buf
                .slice(key_pos..(key_pos + self.key_size as usize))
                .unwrap()
        }
    ...
    impl<'a> BucketState<'a> {
        ...
        pub fn remove(&mut self, key: &[u8]) -> Option<HashMapEntryRef<'a>> {
            let Some((found, previous)) = self.find_internal(key) else {
                return None;
            };
        ...
        pub fn insert<'b>(
            ...
            let index = entry_ref
                .index
                .take()
                .expect("insert called for a null EntryRef");
...
impl HashMap {
    pub fn new(schema: &MapSchema, vmo: impl Into<VmoOrName>) -> Result<Self, MapError> {
        ...
        let buffer = MapBuffer::new(
            layout.total_size(),
            vmo.into().with_name_prefix("ebpf:hashmap"),
        )?;
    ...
    fn hash_key<'a>(&'a self) -> (u64, u64) {
        ...
        (
            header.hash_key.0.load(Ordering::Relaxed),
            header.hash_key.1.load(Ordering::Relaxed),
        )
    }
