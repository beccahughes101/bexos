...
impl<'a, T> RwMapLockState<'a, T> {
    ...
    fn try_lock_read_fast(&self) -> bool {
        ...
        loop {
            ...
            assert!(
                state & Self::NUM_READERS_MASK < Self::NUM_READERS_MASK,
                "Too many readers"
            );
        }
    }
    ...
    fn lock_read_slow(&self) {
        ...
        let state = self
            .state_cell
            .fetch_add(Self::WAITING_READER_INCREMENT, Ordering::Relaxed);
        assert!(
        ...
        self.lock_slow_loop(|state, num_waits| {
            ...
            assert!(
                state & Self::NUM_WAITING_READERS_MASK > 0,
                "invalid lock state"
            );
    ...
    fn lock_write_slow(&self) {
        ...
        let state = self
            .state_cell
            .fetch_add(Self::WAITING_WRITER_INCREMENT, Ordering::Relaxed);
        assert!(
    ...
    fn unlock_write(&self) {
        let state = self
            .state_cell
            .fetch_and(!Self::WRITER_LOCKED_BIT, Ordering::Release);
        ...
        fn lock<'a>(&'a self) -> RwMapLock<'a, LockedState> {
            ...
            unsafe {
                ...
                RwMapLock::new(
                    ...
                    LOCK_SIGNAL,
                    LockedState {
                        addr: base_ptr.byte_offset(8) as usize,
                    },
                )
            }
        }
            ...
            std::thread::spawn(move || {
                ...
                for i in 0..count {
                    ...
                    *guard.value_mut() += 1;
                    ...
                    assert_eq!(
                        writer_count, 1,
                        "More than one writer held the RwLock at once."
                    );
                    assert_eq!(
                        reader_count, 0,
        ...
        fn spawn_reader(state: Arc<Self>, count: usize) -> std::thread::JoinHandle<()> {
            std::thread::spawn(move || {
                ...
                for i in 0..count {
                    ...
                    assert!(
                        reader_count > 0,
                        "A reader held the RwLock without being counted."
                    );
                }
            })
        }
