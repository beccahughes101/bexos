...
impl RingBuffer {
    ...
    pub fn new(schema: &MapSchema, vmo: impl Into<VmoOrName>) -> Result<Pin<Box<Self>>, MapError> {
        ...
        technical_vmo
            .set_name(&zx::Name::new_lossy("ebpf:ring_buffer_technical_vmo"))
            .unwrap();
        vmar.map(
            0,
            &technical_vmo,
    ...
    fn state<'a>(&'a self) -> RwMapLock<'a, RingBufferState> {
        ...
        unsafe {
            ...
            RwMapLock::new(
                ...
                RINGBUF_LOCK_SIGNAL,
                RingBufferState {
                    base_addr: self.vmar.base() + page_size,
                    mask: self.mask,
                },
            )
        }
...
mod test {
    ...
    fn test_ring_buffer_wrapping() {
        ...
        // Use multiples of 8 to avoid alignment panic!
        {
            ...
            state
                .producer_position()
                .store(0xFFFFFFFFFFFFFFF8, Ordering::Release);
            state
                .consumer_position()
                .store(0xFFFFFFFFFFFFFFF0, Ordering::Release);
        }
    ...
    fn test_ring_buffer_consumer_advance_wrap() {
        ...
        assert!(
            result.is_ok(),
            "Expected ringbuf_reserve to succeed, but got {:?}",
            result
        );
    ...
    fn test_ring_buffer_overflow() {
        ...
        assert_eq!(
            ringbuf.ringbuf_reserve(0xffff_ffff, 0),
            Err(MapError::InvalidParam)
        );
        assert_eq!(
            ringbuf.ringbuf_reserve(0xffff_fff1, 0),
            Err(MapError::InvalidParam)
        );
        assert_eq!(
            ringbuf.ringbuf_reserve(0x3fff_ffff, 0),
