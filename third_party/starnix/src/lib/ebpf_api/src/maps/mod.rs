...
impl MapReference for PinnedMap {
    ...
    fn get_data_ptr(&self) -> Option<BpfValue> {
        ...
        self.0
            .lookup(&key)
            .map(|v| BpfValue::from(v.ptr().raw_ptr()))
    }
}
...
fn create_map_impl(
    ...
    match schema.map_type {
        ...
        bpf_map_type_BPF_MAP_TYPE_DEVMAP_HASH => {
            ...
            // for `DEVMAP` maps.
            let schema = MapSchema {
                flags: schema.flags.difference(MapFlags::ProgReadOnly),
                ..*schema
            };
            Ok(Box::pin(hashmap::HashMap::new(&schema, vmo)?))
        }
...
mod test {
    ...
    fn test_sharing_array() {
        ...
        map1.update(&MapKey::from_vec(key.clone()), (&mut value).into(), 0)
            .unwrap();
    ...
    fn test_hash_map() {
        ...
        for i in 0..10000 {
            assert!(
                map.update(&get_key(i), (&mut get_value(i, 0)).into(), 0)
                    .is_ok()
            );
        }
        ...
        for i in 8000..9000 {
            assert!(
                map.update(&get_key(i), (&mut get_value(i, 1)).into(), 0)
                    .is_ok()
            );
        }
        for i in 8000..9000 {
        ...
        for i in 10000..15000 {
            assert!(
                map.update(&get_key(i), (&mut get_value(i, 2)).into(), 0)
                    .is_ok()
            );
        }
    ...
    fn test_lpm_trie_invalid_key_size() {
        ...
        assert_eq!(
            Map::new(make_schema(4), "test").err(),
            Some(MapError::InvalidParam)
        );
        ...
        assert_eq!(
            Map::new(make_schema(261), "test").err(),
            Some(MapError::InvalidParam)
        );
    }
    ...
    fn test_hash_map_update_direct() {
        ...
        value_ref
            ...
            .slice(0..4)
            .unwrap()
            .store(&[0xae, 0xad, 0xac, 0xab]);

        assert_eq!(
            &map.load(&key).unwrap()[..],
            &[0xae, 0xad, 0xac, 0xab, 4, 5, 6, 7, 8, 9, 10]
        );
    }
    ...
    fn test_hash_map_ref_counting() {
        ...
        assert_eq!(
            map.update(&key.clone(), (&mut value).into(), 0),
            Err(MapError::SizeLimit)
        );
        drop(value_ref);
        assert!(map.update(&key.clone(), (&mut value).into(), 0).is_ok());
    ...
    fn test_ringbug_sharing() {
        ...
        let map = Map::new(schema, "test").unwrap();
        map.ringbuf_reserve(8000, 0)
            .expect("ringbuf_reserve failed");

        let map2 = Map::new_shared(map.share().unwrap()).unwrap();

        // Expected to fail since there is no space left.
        map2.ringbuf_reserve(2000, 0)
            .expect_err("ringbuf_reserve expected to fail");
    }
