...
impl<'a> LpmKey for EbpfBufferPtr<'a> {
    fn read_u32(&self, index: usize) -> u32 {
        self.get_ptr::<u32>(index * std::mem::size_of::<u32>())
            .unwrap()
            .load_relaxed()
    }
}
...
mod internal {
    ...
    impl Layout {
        ...
        pub fn checked_total_size(&self) -> Option<usize> {
            TRIE_HEADER_SIZE
                ...
                .checked_add(
                    self.padded_data_entry_size()
                        .checked_mul(self.max_entries as usize)?,
                )
        }
        ...
        fn new(store: LpmTrieStore<'a>) -> RwMapLock<'a, Self> {
            ...
            unsafe {
                ...
                RwMapLock::new(
                    ...
                    LOCK_SIGNAL,
                    HeapState {
                        header: &lpm_trie_header.heap_header,
                        store,
                    },
                )
            }
    ...
    impl<'a> LpmTrieNode<'a> {
        fn new(index: u32, buffer: EbpfBufferPtr<'a>) -> Self {
            ...
            Self {
                index,
                header,
                key: buffer.slice(NODE_HEADER_SIZE..).unwrap(),
            }
        }
    ...
    impl<'a> DataEntry<'a> {
        ...
        pub fn value(&self) -> EbpfBufferPtr<'a> {
            self.buf
                .slice(DATA_ENTRY_HEADER_SIZE..self.buf.len())
                .unwrap()
        }
    }
...
impl LpmTrie {
    pub fn new(schema: &MapSchema, vmo: impl Into<VmoOrName>) -> Result<Self, MapError> {
        ...
        let buffer = MapBuffer::new(
            layout.total_size(),
            vmo.into().with_name_prefix("ebpf:lpm_trie"),
        )?;
...
mod test {
    ...
    fn test_lpm_key() {
        ...
        assert_eq!(
            long_key1.as_slice().num_matching_bits(long_key2.as_slice()),
            37
        );
    ...
    fn test_update_and_lookup() {
        ...
        trie.update(&key1, (&mut [9, 10, 11, 12]).into(), 0)
            .unwrap();
    ...
    fn test_max_entries() {
        ...
        for i in 0..10 {
            ...
            trie.update(&key, (&mut [i, 1, 2, 3]).into(), 0)
    ...
    fn test_invalid_prefix_len_lookup() {
        ...
        trie.update(&valid_key, (&mut [1, 2, 3, 4]).into(), 0)
            .unwrap();
    ...
    fn test_invalid_prefix_len_get_next_key() {
        ...
        let bogus_key = serialize_key(u32::MAX, &[192, 168, 1, 0]);
        assert_eq!(
            trie.get_next_key(Some(&bogus_key)),
    ...
    fn test_random_insert_order(get_key: impl Fn(usize) -> Vec<u8>) {
        ...
        for id in ids.iter() {
            ...
            trie.update(&key, (&mut [id, 1, 2, 3]).into(), 0)
                .expect("Failed to update map entry");
        }
