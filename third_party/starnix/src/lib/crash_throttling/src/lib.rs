...
impl CrashThrottler {
    ...
    pub fn should_report(
        ...
        if !self.enable_throttling {
            return Some(PendingCrashReport {
                argv,
                argv0,
                weight: 1,
            });
...
impl CrashInfo {
    ...
    fn is_throttled_at(
        ...
        self.crash_runtimes
            .iter()
            .filter(|&&x| (runtime - x) < crash_loop_age_out)
            .count()
            > CRASH_LOOP_LIMIT
    }
    ...
    fn prune_crash_runtimes(
        ...
        self.crash_runtimes
            .retain(|&x| (runtime - x) < crash_loop_age_out);
    }
}
...
mod tests {
    ...
    fn reports_some_crashes_while_throttled() {
        ...
        for _ in 0..CRASH_LOOP_LIMIT {
            assert!(
                throttler
                    .should_report(vec![], "test-process".to_string(), RUNTIME)
                    .is_some()
            );
        }
    ...
    fn is_throttled_filters() {
        ...
        crash_info
            .crash_runtimes
            .push_back(zx::MonotonicInstant::from_nanos(0));
        for _ in 0..CRASH_LOOP_LIMIT {
            crash_info
                .crash_runtimes
                .push_back(zx::MonotonicInstant::from_nanos(50));
        }
    ...
    fn throttling_ends() {
        ...
        for _ in 0..CRASH_LOOP_LIMIT {
            assert!(
                throttler
                    .should_report(vec![], "test-process".to_string(), time)
                    .is_some()
            );
        }
