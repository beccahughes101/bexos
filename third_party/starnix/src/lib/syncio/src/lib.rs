...
impl ZxioDirent {
    fn from(dirent: zxio_dirent_t, name_buffer: Vec<u8>) -> ZxioDirent {
        let protocols = if dirent.has.protocols {
            Some(dirent.protocols)
        } else {
            None
        };
        let abilities = if dirent.has.abilities {
            Some(dirent.abilities)
        } else {
            None
        };
    ...
    pub fn is_dir(&self) -> bool {
        self.protocols
            .map(|p| p & ZXIO_NODE_PROTOCOL_DIRECTORY > 0)
            .unwrap_or(false)
    }

    pub fn is_file(&self) -> bool {
        self.protocols
            .map(|p| p & ZXIO_NODE_PROTOCOL_FILE > 0)
            .unwrap_or(false)
    }
}
        ...
        let (size, level, type_) = match self {
            ...
            ControlMessage::IpPacketInfo {
                ...
                let pktinfo = zxio::in_pktinfo {
                    ipi_ifindex: *iface,
                    ipi_spec_dst: zxio::in_addr {
                        s_addr: u32::from_ne_bytes(*local_addr),
                    },
            ...
            ControlMessage::Ipv6PacketInfo { iface, local_addr } => {
                let pktinfo = zxio::in6_pktinfo {
                    ipi6_addr: zxio::in6_addr {
                        __in6_union: zxio::in6_addr__bindgen_ty_1 {
                            __s6_addr: *local_addr,
                        },
                    },
                    ipi6_ifindex: *iface,
                };
            ...
            ControlMessage::TimestampNs { sec, nsec } => {
                let timespec = zxio::timespec {
                    tv_sec: *sec,
                    tv_nsec: *nsec,
                };
                timespec.write_to_prefix(data).unwrap();
                (
                    size_of_val(&timespec),
                    zxio::SOL_SOCKET,
                    zxio::SO_TIMESTAMPNS,
                )
            }
...
fn serialize_control_messages(messages: &[ControlMessage]) -> Vec<u8> {
    let size = messages.iter().fold(0, |sum, x| {
        sum + CMSG_HEADER_SIZE + align_cmsg_size(x.get_data_size())
    });
    ...
    loop {
        ...
        let msg = match (header.cmsg_level as u32, header.cmsg_type as u32) {
            ...
            (zxio::SOL_IP, zxio::IP_RECVORIGDSTADDR) => ControlMessage::IpRecvOrigDstAddr(
                <[u8; size_of::<zxio::sockaddr_in>()]>::read_from_prefix(msg_data)
                    .unwrap()
                    .0,
            ),
...
const_assert_eq!(
    AllocateMode::KEEP_SIZE.bits(),
    zxio::ZXIO_ALLOCATE_KEEP_SIZE
);
const_assert_eq!(
    AllocateMode::UNSHARE_RANGE.bits(),
    zxio::ZXIO_ALLOCATE_UNSHARE_RANGE
);
const_assert_eq!(
    AllocateMode::PUNCH_HOLE.bits(),
    zxio::ZXIO_ALLOCATE_PUNCH_HOLE
);
const_assert_eq!(
    AllocateMode::COLLAPSE_RANGE.bits(),
    zxio::ZXIO_ALLOCATE_COLLAPSE_RANGE
);
const_assert_eq!(
    AllocateMode::ZERO_RANGE.bits(),
    zxio::ZXIO_ALLOCATE_ZERO_RANGE
);
const_assert_eq!(
    AllocateMode::INSERT_RANGE.bits(),
    zxio::ZXIO_ALLOCATE_INSERT_RANGE
);
...
impl Default for Zxio {
    fn default() -> Self {
        Self {
            inner: Arc::pin(ZxioStorage::default()),
        }
    }
}
...
impl ZxioSocketMark {
    fn new(domain: u8, value: u32) -> Self {
        ZxioSocketMark(zxio_socket_mark_t {
            is_present: true,
            domain,
            value,
            ..Default::default()
...
impl Zxio {
    ...
    pub fn create_dirent_iterator(&self) -> Result<DirentIterator<'_>, zx::Status> {
        ...
        zx::ok(status)?;
        let iterator = DirentIterator {
            iterator: zxio_iterator,
    ...
    pub fn setsockopt(
        ...
        optname: i32,
        optval: &[u8],
        access_token: Option<zx::NullableHandle>,
    ) -> Result<Result<(), ZxioErrorCode>, zx::Status> {
        let mut out_code = 0;
        let access_token_handle = access_token
...
fn directory_open(
    ...
    directory
        .open(path, flags, &Default::default(), server_end)
    ...
    match node.wait_for_event(deadline).map_err(map_fidl_error)? {
        ...
        fio::NodeEvent::OnRepresentation { payload } => Ok(DescribedNode {
            node,
            kind: payload.into(),
        }),
...
mod test {
    ...
    async fn test_directory_enumerate() -> Result<(), Error> {
        ...
        let mut found_dir_names = iter
            .map(|e| {
                ...
                std::str::from_utf8(&dirent.name)
                    .expect("name was not valid utf8")
                    .to_string()
            })
            .collect::<Vec<_>>();
    ...
    fn test_parse_ip_pktinfo_control_message() {
        let pktinfo = zxio::in_pktinfo {
            ...
            ipi_addr: zxio::in_addr {
                s_addr: u32::from_ne_bytes([192, 0, 2, 2]),
            },
        };
        ...
        header.write_to_prefix(&mut out[..]).unwrap();
        pktinfo
            .write_to_prefix(&mut out[CMSG_HEADER_SIZE..])
            .unwrap();
