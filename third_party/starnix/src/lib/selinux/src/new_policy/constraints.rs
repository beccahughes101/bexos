...
impl<T: PolicyId> ConstraintSets<T> {
    pub fn empty() -> Self {
        Self {
            include_set: IdSet::empty(),
            exclude_set: IdSet::empty(),
            flags: 0,
        }
    }
        ...
        match constraint_term_type {
            ...
            Self::NAME_EXPR => {
                ...
                match operand_val & !TARGET_FLAG {
                    USER => Ok(Self::UserNameOp(NameExpression::parse(
                        cursor, subject, true,
                    )?)),
                    ROLE => Ok(Self::RoleNameOp(NameExpression::parse(
                        cursor, subject, true,
                    )?)),
                    TYPE => Ok(Self::TypeNameOp(NameExpression::parse(
                        cursor, subject, false,
                    )?)),
                    _ => Err(ParseError::InvalidConstraintOperandType { value: operand_val }),
                }
            }
            _ => Err(ParseError::InvalidConstraintTermType {
                value: constraint_term_type,
            }),
        }
    }
}
...
mod tests {
    ...
    fn test_constraint_node_validate_term_sequence() {
        ...
        assert_eq!(
            ...
            Err(ValidateError::InvalidConstraintTermSequence)
        );
    }
}
