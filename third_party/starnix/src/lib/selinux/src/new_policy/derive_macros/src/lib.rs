...
pub fn derive_parse(input: TokenStream) -> TokenStream {
    ...
    let generics = add_trait_bounds(
        input.generics,
        syn::parse_quote!(crate::new_policy::traits::Parse),
    );
...
pub fn derive_serialize(input: TokenStream) -> TokenStream {
    ...
    let generics = add_trait_bounds(
        input.generics,
        syn::parse_quote!(crate::new_policy::traits::Serialize),
    );
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
...
pub fn derive_validate(input: TokenStream) -> TokenStream {
    ...
    let generics = add_trait_bounds(
        input.generics,
        syn::parse_quote!(crate::new_policy::traits::Validate),
    );
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
...
pub fn derive_has_name(input: TokenStream) -> TokenStream {
    ...
    let has_name_field = fields
        .iter()
        .any(|f| f.ident.as_ref().map_or(false, |ident| ident == "name"));
...
pub fn derive_has_policy_id(input: TokenStream) -> TokenStream {
    ...
    let id_field_type = fields.iter().find_map(|f| {
        if f.ident.as_ref().map_or(false, |ident| ident == "id") {
            Some(&f.ty)
        } else {
            None
        }
