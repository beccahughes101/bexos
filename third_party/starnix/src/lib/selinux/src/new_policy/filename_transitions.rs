...
impl FilenameTransitions {
    ...
    pub fn compute_filename_transition(
        ...
        let item = transition
            .items
            .iter()
            .find(|item| item.stypes.contains(source_type))?;
        Some(item.out_type)
    }
...
mod tests {
    ...
    fn test_filename_transition_geq33_parse_and_serialize() {
        ...
        assert_eq!(
            transitions.compute_filename_transition(s_id, t_id, c_id, b"other"),
            None
        );
    ...
    fn test_filename_transition_leq32_parse_and_convert() {
        ...
        assert_eq!(
            transitions.compute_filename_transition(s9_id, t_id, c_id, b"test"),
            None
        );
    ...
    fn test_filename_transition_policy_elements() {
        ...
        new_policy
            .validate()
            .expect("validate type_transition policy");

        let transitions = new_policy.filename_transitions();
        assert_eq!(transitions.transitions.len(), 1);

        let source_type = new_policy
            .types()
            .get_by_name(b"source_t")
            .expect("look up source_t")
            .id();
        let target_type = new_policy
            .types()
            .get_by_name(b"target_t")
            .expect("look up target_t")
            .id();
        let class = new_policy
            .classes()
            .get_by_name(b"file")
            .expect("look up file")
            .id();
    ...
    fn test_filename_transition_duplicate_key_error() {
        ...
        assert!(matches!(
            result,
            Err(ParseError::DuplicateFilenameTransition { .. })
        ));
    }
}
