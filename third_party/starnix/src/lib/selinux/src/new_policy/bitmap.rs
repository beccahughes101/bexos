...
impl ExtensibleBitmap {
    ...
    pub fn high_bit(&self) -> u32 {
        self.items
            .last()
            .map_or(0, |item| item.start_bit + MAP_NODE_BITS)
    }
    ...
    pub fn is_set(&self, index: u32) -> bool {
        ...
        match self
            .items
            .binary_search_by_key(&start_bit, |item| item.start_bit)
        {
            Ok(idx) => {
    ...
    pub fn count(&self) -> usize {
        self.items
            .iter()
            .map(|item| item.map.count_ones() as usize)
            .sum()
    }
...
impl<T: PolicyId, const WITH_ID_ZERO: bool> Default for IdSetBuilder<T, WITH_ID_ZERO> {
    fn default() -> Self {
        Self {
            builder: ExtensibleBitmapBuilder::new(),
            _phantom: PhantomData,
        }
    }
}
...
impl<T: PolicyId, const WITH_ID_ZERO: bool> IdSetBuilder<T, WITH_ID_ZERO> {
    ...
    pub fn insert(&mut self, id: T) {
        self.builder
            .insert(IdSet::<T, WITH_ID_ZERO>::id_to_index(id));
    }
...
mod tests {
    ...
    fn test_id_set() {
        ...
        assert_eq!(
            ids,
            vec![TypeId::from_u32(1).unwrap(), TypeId::from_u32(3).unwrap()]
        );
    }
    ...
    fn test_bitmap_compare() {
        ...
        assert_eq!(
            b_node0.compare(&b_node0_and_128),
            Some(std::cmp::Ordering::Less)
        );
        assert_eq!(
            b_node0_and_128.compare(&b_node0),
        ...
        assert_eq!(
            b_range_narrow.compare(&b_range_wide),
            Some(std::cmp::Ordering::Less)
        );
        assert_eq!(
            b_range_wide.compare(&b_range_narrow),
            Some(std::cmp::Ordering::Greater)
        );
    ...
    fn test_bitmap_set_range() {
        ...
        for i in 0..128 {
            assert_eq!(
                b1.is_set(i),
                (10..=20).contains(&i),
                "bit {i} mismatch for range [10, 20]"
            );
        }
        ...
        for i in 0..128 {
            assert_eq!(
                b3.is_set(i),
                (60..=70).contains(&i),
                "bit {i} mismatch for range [60, 70]"
            );
        }
