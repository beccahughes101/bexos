...
impl Parse for Type {
    fn parse(cursor: &mut PolicyCursor<'_>) -> Result<Self, ParseError> {
        ...
        let name = cursor
            .read_bytes(metadata.length as usize)?
            .to_vec()
            .into_boxed_slice();
...
impl Types {
    ...
    pub fn get_by_name(&self, name: &[u8]) -> Option<&Type> {
        ...
        let idx = self
            .by_name
            .find(hash, |&idx| self.ordered[idx].name.as_ref() == name)?;
...
mod tests {
    ...
    fn test_types_lookup() {
        ...
        assert_eq!(
            types
                .get_by_id(TypeId::new(NonZeroU16::new(1).unwrap()))
                .map(|t| t.name()),
            Some(b"foo".as_slice())
        );
        assert_eq!(
            types
                .get_by_id(TypeId::new(NonZeroU16::new(2).unwrap()))
                .map(|t| t.name()),
            Some(b"bar".as_slice())
        );
        assert!(
            types
                .get_by_id(TypeId::new(NonZeroU16::new(3).unwrap()))
                .is_none()
        );
