    ...
    pub fn parse(data: &[u8]) -> Result<Self, ParseError> {
        ...
        if cursor.offset() < data.len() {
            return Err(ParseError::TrailingBytes {
                num_bytes: data.len() - cursor.offset(),
            });
        }
        Ok(policy)
    }
...
mod tests {
    ...
    const TEST_POLICIES: &[(&str, &[u8])] = &[
        (
            "selinux_testsuite",
            include_bytes!("../../testdata/policies/selinux_testsuite"),
        ),
        (
            "emulator",
            include_bytes!("../../testdata/policies/emulator"),
        ),
        (
            "conditional_policy",
    ...
    fn test_initial_sids_policy_elements() {
        ...
        let kernel_context = new_policy
            .initial_sids()
            .get_by_id(crate::InitialSid::Kernel as u32)
            .unwrap();
        assert!(
            new_policy
                .users()
                .get_by_id(kernel_context.user())
                .is_some()
        );
        assert!(
            new_policy
                .roles()
                .get_by_id(kernel_context.role())
                .is_some()
        );

        let unlabeled_context = new_policy
            .initial_sids()
            .get_by_id(crate::InitialSid::Unlabeled as u32)
            .unwrap();
        assert!(
            new_policy
                .users()
                .get_by_id(unlabeled_context.user())
                .is_some()
        );
        assert!(
            new_policy
                .roles()
                .get_by_id(unlabeled_context.role())
                .is_some()
        );
    }
}
