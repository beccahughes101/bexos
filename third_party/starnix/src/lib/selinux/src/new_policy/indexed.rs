    ...
    T: HasPolicyId + HasName,
{
    ...
    pub fn new(container: C) -> Result<Self, ParseError> {
        ...
        Ok(Self {
            container,
            id_to_index: id_to_index.into_boxed_slice(),
            name_to_index,
            hasher,
        })
    }
    ...
    pub fn get_by_name(&self, name: &[u8]) -> Option<&T> {
        ...
        let idx = self
            .name_to_index
            .find(hash, |&idx| self.container[idx].name() == name)?;
...
impl<C: Debug> Debug for IdAndNameIndexed<C> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IdAndNameIndexed")
...
mod tests {
    ...
    fn test_indexed_lookup() {
        let items: &[TestItem] = &[
            TestItem {
                id: TestId::for_test(1),
                name: "foo",
            },
            TestItem {
                id: TestId::for_test(2),
                name: "bar",
            },
        ];
        let indexed = IdAndNameIndexed::new(items).unwrap();
