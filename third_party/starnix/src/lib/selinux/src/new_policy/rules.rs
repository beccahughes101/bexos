...
impl Parse for AccessVectorRules {
    fn parse(cursor: &mut PolicyCursor<'_>) -> Result<Self, ParseError> {
        ...
        for _ in 0..count {
            ...
            let source_type = TypeId::from_u16(source_val).ok_or(ParseError::InvalidId {
                value: source_val as u32,
            })?;

            let target_val = header.target_type;
            let target_type = TypeId::from_u16(target_val).ok_or(ParseError::InvalidId {
                value: target_val as u32,
            })?;

            let class_val = header.class;
            let class = ClassId::from_u16(class_val).ok_or(ParseError::InvalidId {
                value: class_val as u32,
            })?;
...
fn build_index<R: HasRuleKey>(
    ...
    for chunk in rules.chunk_by(|a, b| a.key() == b.key()) {
        ...
        let Entry::Vacant(entry) = table.entry(
            ...
            return Err(ParseError::DuplicateAccessVectorRule {
                key,
                kind: chunk[0].kind(),
            });
        };
...
mod tests {
    ...
    fn test_av_rule_parse_and_serialize() {
        ...
        assert_eq!(
            av_rules.av_rules[0].key.source_type,
            TypeId::from_u32(1).unwrap()
        );
        assert_eq!(
            av_rules.av_rules[0].key.target_type,
            TypeId::from_u32(2).unwrap()
        );
        assert_eq!(
            av_rules.av_rules[0].key.class,
            ClassId::from_u32(3).unwrap()
        );
        ...
        av_rules
            .serialize(&mut policy_writer)
            .expect("serialize rules table");
    ...
    fn test_av_rule_type_transition_parse_and_serialize() {
        ...
        assert_eq!(
            av_rules.type_rules[0].key.source_type,
            TypeId::from_u32(1).unwrap()
        );
        assert_eq!(
            av_rules.type_rules[0].key.target_type,
            TypeId::from_u32(2).unwrap()
        );
        assert_eq!(
            av_rules.type_rules[0].key.class,
            ClassId::from_u32(3).unwrap()
        );
        assert_eq!(
            av_rules.type_rules[0].new_type,
            TypeId::from_u32(10).unwrap()
        );
    ...
    fn test_unindexed_access_vector_rules_allows_duplicate_keys() {
        ...
        assert!(matches!(
            err,
            ParseError::DuplicateAccessVectorRule {
                key: _,
                kind: RuleKind::Allow
            }
        ));
