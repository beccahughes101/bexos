...
impl Validate for SensitivityId {
    fn validate(&self, policy: &super::NewPolicy) -> Result<(), ValidateError> {
        policy
            ...
            .get_by_id(*self)
            .map(|_| ())
            .ok_or_else(|| ValidateError::UnknownId {
                kind: "sensitivity",
                id: self.as_u32(),
            })
    }
...
mod tests {
    ...
    fn test_category_invalid_id() {
        ...
        let mut cursor = PolicyCursor::new(&data);
        assert!(matches!(
            Category::parse(&mut cursor),
            Err(ParseError::InvalidId { value: 0 })
        ));
    }
    ...
    fn test_sensitivity_parse_and_serialize() {
        ...
        assert_eq!(
            ...
            SensitivityId::from_u32(2).unwrap()
        );
    ...
    fn test_range_transition_parse_and_serialize() {
        ...
        assert_eq!(
            transition.mls_range().low().sensitivity(),
            SensitivityId::from_u32(1).unwrap()
        );
        ...
        transition
            .serialize(&mut policy_writer)
            .expect("serialize RangeTransition");
        assert_eq!(writer.as_slice(), &data);
    }
}
