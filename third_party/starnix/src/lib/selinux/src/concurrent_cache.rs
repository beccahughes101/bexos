    ...
    >
{
    ...
    pub fn get_or_insert(&self, key: &S::Key, callback: impl FnOnce() -> S::Value) -> S::Value {
        self.get_or_try_insert::<()>(key, || Ok(callback()))
    ...
    pub fn get_or_try_insert<E>(
        ...
        bucket
            .seqlock_state
            .fetch_add((1 << 16) - (1 << way_to_write) as u64, Ordering::Release);
        Ok(result)
    }
...
mod tests {
    ...
    impl Default for BlockingStorage {
        fn default() -> Self {
            Self {
                state: Mutex::new(BlockingStorageState::default()),
                condvar: Condvar::new(),
            }
        }
    }
    ...
    impl StorageStrategy<2, 0, 0, 0, 0> for BlockingStorage {
        ...
        fn read_value(
            ...
            self.condvar
                .wait_while(&mut state, |state| state.blocked_reads.contains(&key));
        ...
        fn write_key_value(
            ...
            self.condvar.wait_while(&mut state, |state| {
                state.blocked_writes.contains(&(*key, *value))
            });
    ...
    fn test_skip_cache_when_all_ways_writing() {
        ...
        // Wait until all writers are blocked in the write phase.
        {
            ...
            cache
                .storage
                .condvar
                .wait_while(&mut state, |state| state.blocked_writes.len() < 4);
        }
    ...
    fn test_read_retries_on_concurrent_write_start() {
        ...
        cache
            .storage
            .condvar
            .wait_while(&mut state, |state| state.blocked_writes.len() < 3);
        drop(state);
        ...
        cache
            .storage
            .condvar
            .wait_while(&mut state, |state| !state.blocked_reads.contains(&0));
        drop(state);
    ...
    fn test_read_retries_on_concurrent_write_complete() {
        ...
        let mut state = cache.storage.state.lock();
        cache.storage.condvar.wait_while(&mut state, |state| {
            !state.blocked_writes.iter().any(|&(k, _)| k == 0)
        });
        state.blocked_writes.retain(|&(k, _)| k != 0);
        cache.storage.condvar.notify_all();
        drop(state);
