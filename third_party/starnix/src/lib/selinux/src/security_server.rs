...
impl SecurityServerState {
    ...
    fn reject_unknown(&self) -> bool {
        self.active_policy.as_ref().map_or(false, |p| {
            p.parsed.handle_unknown() == HandleUnknown::Reject
        })
    }
...
impl SecurityServer {
    ...
    pub fn load_policy(&self, binary_policy: Vec<u8>) -> Result<(), anyhow::Error> {
        ...
        self.with_mut_state_and_update_status(|state| {
            ...
            state.active_policy = Some(ActivePolicy {
                parsed,
                sid_table,
                exceptions,
            });
    ...
    pub fn set_enforcing(&self, enforcing: bool) {
        self.with_mut_state_and_update_status(|_| {
            self.backend
                .is_enforcing
                .store(enforcing, Ordering::Release);
        });
    }
    ...
    pub fn class_permissions_by_name(
        ...
        locked_state
            .expect_active_policy()
            .parsed
            .find_class_permissions_by_name(name)
    }
    ...
    pub fn resolve_fs_label(
        ...
        let label = if let Some(mountpoint_sid) = mount_sids.context {
            ...
            FileSystemLabel {
                ...
                scheme: FileSystemLabelingScheme::Mountpoint {
                    sid: mountpoint_sid,
                },
                mount_sids,
            }
            ...
            active_policy.parsed.fs_use_label_and_type(fs_type)
        {
            ...
            let fs_sid_from_policy = active_policy
                .sid_table
            ...
            active_policy
                .parsed
                .genfscon_label_for_fs_and_path(fs_type, ROOT_PATH.into(), None)
        {
            // There is a `genfscon` statement for this file-system type in the policy.
            let genfscon_sid = active_policy
                .sid_table
                .security_context_to_sid(&context)
                .unwrap();
            ...
            supports_seclabel |= matches!(fs_type, b"cgroup" | b"cgroup2")
                && active_policy
                    .parsed
                    .has_policycap(PolicyCap::CgroupSeclabel);
            supports_seclabel |= fs_type == b"functionfs"
                && active_policy
                    .parsed
                    .has_policycap(PolicyCap::FunctionfsSeclabel);
    ...
    pub fn is_bounded_by(&self, bounded_sid: SecurityId, parent_sid: SecurityId) -> bool {
        ...
        let bounded_type = active_policy
            .sid_table
            .sid_to_security_context(bounded_sid)
            .type_();
        let parent_type = active_policy
            .sid_table
            .sid_to_security_context(parent_sid)
            .type_();
        active_policy
            .parsed
            .is_bounded_by(bounded_type, parent_type)
    }
    ...
    /// Returns the security identifier ([`SecurityId`]) with which to label a new object of `target_class`,
    /// based on the specified source & target security SIDs and optional `name`.
    ...
    pub fn compute_create_sid_raw(
        ...
        self.backend
            .compute_create_sid_raw(source_sid, target_sid, target_class.into(), name)
    }
    ...
    pub fn compute_access_decision_raw(
        ...
        self.backend
            .compute_access_decision_raw(source_sid, target_sid, target_class.into())
    }
}
...
impl SecurityServerBackend {
    ...
    fn compute_sid(
        ...
        if let Some(sid) = policy_state
            .sid_table
            .security_context_to_existing_sid(&context)
        {
            return Ok(sid);
        }
        ...
        let context = if needs_recompute {
            compute_context(policy_state)?
        } else {
            context
        };
        policy_state
...
impl Query for SecurityServerBackend {
    ...
    fn compute_create_sid(
        ...
        self.compute_create_sid_raw(source_sid, target_sid, target_class.into(), name)
    }
    ...
    fn compute_xperms_access_decision(
        ...
        let allow = if !base_permit {
            XpermsBitmap::NONE
        } else {
            xperms_decision.allow
        };
...
fn check_policy_feature_is_empty<T>(name: &'static str, items: &[T]) -> Result<(), anyhow::Error> {
    if !items.is_empty() {
        anyhow::bail!(
            "Policy contains unsupported feature: {name} ({})",
            items.len()
        );
    }
    Ok(())
...
mod tests {
    ...
    fn test_unsupported_policy_features_check() {
        ...
        assert!(
            default_server
                .load_policy(MINIMAL_BINARY_POLICY.to_vec())
                .is_ok()
        );
        assert!(
            testsuite_server
        ...
        assert!(
            testsuite_server
                .load_policy(TESTSUITE_BINARY_POLICY.to_vec())
                .is_ok()
        );
    }
    ...
    fn loaded_policy_can_be_retrieved() {
        ...
        assert_eq!(
            TESTS_BINARY_POLICY,
            security_server.get_binary_policy().unwrap().as_ref()
        );
    }
    ...
    fn loaded_policy_is_validated() {
        ...
        assert!(
            security_server
                .load_policy(not_really_a_policy.clone())
                .is_err()
        );
    }
    ...
    fn conditional_booleans_can_be_queried() {
        ...
        assert!(
            security_server
                .get_boolean("this_is_not_a_valid_boolean_name")
                .is_err()
        );
    ...
    fn conditional_booleans_can_be_changed() {
        ...
        assert_eq!(
            active, pending,
            "Initially active and pending values should match"
        );

        security_server
            .set_pending_boolean(boolean, !active)
            .unwrap();
        let (active, pending) = security_server.get_boolean(boolean).unwrap();
        assert!(
            active != pending,
            "Before commit pending should differ from active"
        );

        security_server.commit_pending_booleans();
        let (final_active, final_pending) = security_server.get_boolean(boolean).unwrap();
        assert_eq!(
            final_active, pending,
            "Pending value should be active after commit"
        );
        assert_eq!(
            final_active, final_pending,
            "Active and pending are the same after commit"
        );
    }
    ...
    fn compute_create_sid_no_defaults() {
        ...
        let computed_sid = security_server
            ...
            .compute_create_sid(source_sid, target_sid, FileClass::File.into(), &[])
    ...
    #[test]
    fn compute_create_sid_source_defaults() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_target_defaults() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_source_low_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_source_low_high_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_source_high_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_target_low_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_target_low_high_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_range_target_high_default() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
    ...
    #[test]
    fn compute_create_sid_with_name() {
        let security_server = SecurityServer::new_default();
        let policy_bytes =
        ...
        assert_eq!(
            computed_context,
            b"source_u:object_r:special_transition_t:s0"
        );
    ...
    fn permissions_are_fresh_after_different_policy_load() {
        ...
        assert!(
            !permission_check
                .has_permission(sid, sid, ProcessPermission::Fork)
                .granted
        );
    ...
    fn unknown_sids_are_effectively_unlabeled() {
        ...
        let allowed_type_sid = security_server
            .security_context_to_sid(allowed_type_context.into())
            .unwrap();
        assert!(
            security_server
                .security_context_to_sid(additional_type_context.into())
                .is_err()
        );
        ...
        let additional_type_sid = security_server
            .security_context_to_sid(additional_type_context.into())
            .unwrap();
        assert_eq!(
            allowed_type_sid,
        ...
        let unlabeled_context = security_server
            .sid_to_security_context(InitialSid::Unlabeled.into())
            .unwrap();
        assert_eq!(
            unlabeled_context,
            security_server
                .sid_to_security_context(additional_type_sid)
                .unwrap()
        );
    ...
    fn access_checks_with_exceptions_config() {
        ...
        assert!(
            security_server
                .load_policy(EXCEPTIONS_POLICY.into())
                .is_ok()
        );
