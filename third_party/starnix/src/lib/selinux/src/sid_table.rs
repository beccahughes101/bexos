...
impl SidTable {
    ...
    pub fn new_from_previous(policy: Arc<Policy>, previous: &Self) -> Self {
        ...
        new_entries.extend(
            previous.entries[ReferenceInitialSid::FirstUnused as usize..]
                .iter()
                .map(|previous_entry| {
                    let serialized_context = match previous_entry {
                        Entry::Valid { security_context } => previous
                            .policy
                            .serialize_security_context(&security_context),
                    ...
                    if let Ok(context) = context {
                        ...
                        Entry::Invalid {
                            context_string: serialized_context,
                        }
                    }
                }),
        );
    ...
    fn new_from(policy: Arc<Policy>, mut new_entries: Vec<Entry>) -> Self {
        for initial_sid in InitialSid::all_variants() {
            ...
            new_entries[*initial_sid as usize] = Entry::Valid {
                security_context: initial_context,
            };
        }
...
mod tests {
    ...
    fn initial_sids_remapped_to_dynamic_sids() {
        ...
        let file_dynamic_sid = sid_table
            .security_context_to_sid(&file_initial_security_context.clone())
            .unwrap();
