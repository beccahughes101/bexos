            ...
            match statement {
                "todo_deny" => {
                    ...
                    let bug_id = bug_ref_to_id(
                        parts
                            .next()
                            .ok_or_else(|| anyhow!("Expected bug identifier"))?,
                    )?;
                    ...
                    let stype = policy
                        ...
                        .get_by_name(
                            parts
                                .next()
                                .ok_or_else(|| anyhow!("Expected source type"))?
                                .as_bytes(),
                        )
                        .map(|t| t.id());
                    let ttype = policy
                        ...
                        .get_by_name(
                            parts
                                .next()
                                .ok_or_else(|| anyhow!("Expected target type"))?
                                .as_bytes(),
                        )
                        .map(|t| t.id());
                    ...
                    let class_name = parts
                        .next()
                        .ok_or_else(|| anyhow!("Target class missing"))?;
                    ...
                    let policy_class = policy
                        .classes()
                        .get_by_name(class_name.as_bytes())
                        .map(|x| x.id());
...
mod tests {
    ...
    fn test_data() -> TestData {
        ...
        let defined_source = policy
            .types()
            .get_by_name(EXCEPTION_SOURCE_TYPE)
            .unwrap()
            .id();
        let defined_target = policy
            .types()
            .get_by_name(EXCEPTION_TARGET_TYPE)
            .unwrap()
            .id();
        ...
        assert!(
            policy
                .types()
                .get_by_name(b"test_undefined_source_t")
                .is_none()
        );
        assert!(
            policy
                .types()
                .get_by_name(b"test_undefined_target_t")
                .is_none()
        );
