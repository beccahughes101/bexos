...
impl PolicyIndex {
            ...
            match self.range_transition_new_range(source.type_(), target.type_(), &policy_class) {
                ...
                None => match class_defaults.range() {
                    ...
                    ClassDefaultRange::SourceHigh => (
                        source
                            .high_level()
                            .unwrap_or_else(|| source.low_level())
                            .clone(),
                        None,
                    ),
                    ...
                    ClassDefaultRange::TargetHigh => (
                        target
                            .high_level()
                            .unwrap_or_else(|| target.low_level())
                            .clone(),
                        None,
                    ),
    ...
    pub(super) fn genfscon_label_for_fs_and_path(
        ...
        let class_id = class
            .and_then(|class| self.class(class.into()))
            .map(|class| class.id());
        ...
        for fs_context in fs_contexts {
            ...
            if node_path.0.starts_with(fs_context.partial_path()) {
                if result.as_ref().map_or(true, |c| {
                    c.partial_path().len() < fs_context.partial_path().len()
                }) {
...
fn intersect_categories(left: &MlsLevel, right: &MlsLevel) -> CategorySet {
    ...
    CategorySet::from_ids(
        left.category_ids()
            .filter(|id| right_categories.contains(*id)),
    )
}
