...
impl SecurityContext {
    ...
    pub(super) fn from_string(
        ...
        let mut levels = items
            .next()
            .ok_or(SecurityContextError::InvalidSyntax)?
            .split("-");
        ...
        let high_level = high_level
            .map(|x| MlsLevel::from_string(policy_index, x))
            .transpose()?;
...
impl IdSpan<CategoryId> {
    ...
    fn to_string(&self, policy: &NewPolicy) -> Vec<u8> {
        ...
        // than as a range.
        let separator: &[u8] = if self.high().as_u32() == self.low().as_u32() + 1 {
            b","
        } else {
            b"."
        };
...
mod tests {
    ...
    fn test_policy() -> Policy {
        ...
        parse_policy_by_value(TEST_POLICY.to_vec())
            .unwrap()
            .validate()
            .unwrap()
    }
    ...
    fn category_compare() {
        ...
        let cat_3 = cat(2, 3);
        assert_eq!(
            compare(&[cat_1.clone()], &[cat_1.clone()]),
            Some(Ordering::Equal)
        );
        assert_eq!(
            compare(&[cat_1.clone()], &[cat_2.clone()]),
            Some(Ordering::Less)
        );
        assert_eq!(compare(&[cat_1.clone()], &[cat_3.clone()]), None);
        assert_eq!(
            compare(&[cat_2.clone()], &[cat_1.clone()]),
            Some(Ordering::Greater)
        );
        assert_eq!(
            compare(&[cat_2.clone()], &[cat_3.clone()]),
            Some(Ordering::Greater)
        );
    }
    ...
    fn parse_security_context_single_sensitivity() {
        ...
        assert_eq!(
            sensitivity_name(&policy, security_context.low_level().sensitivity()),
            "s0"
        );
    ...
    fn parse_security_context_and_normalize_categories() {
        ...
        assert_eq!(
            normalize("user0:object_r:type0:s1:c0.c1,c1"),
            "user0:object_r:type0:s1:c0,c1"
        );
        assert_eq!(
        ...
        assert_eq!(
            normalize("user0:object_r:type0:s1:c0.c1,c2"),
            "user0:object_r:type0:s1:c0.c2"
        );
    ...
    fn parse_invalid_sensitivity() {
        let policy = test_policy();
        for invalid_label in [
            "user0:object_r:type0:s_invalid",
            "user0:object_r:type0:s0-s_invalid",
        ] {
            assert_eq!(
                policy.parse_security_context(invalid_label.as_bytes().into()),
                Err(SecurityContextError::UnknownSensitivity {
                    name: "s_invalid".into()
                }),
                "validating {:?}",
                invalid_label
    ...
    fn parse_invalid_category() {
        let policy = test_policy();
        for invalid_label in [
            "user0:object_r:type0:s1:c_invalid",
            "user0:object_r:type0:s1:c0.c_invalid",
        ] {
            assert_eq!(
                policy.parse_security_context(invalid_label.as_bytes().into()),
                Err(SecurityContextError::UnknownCategory {
                    name: "c_invalid".into()
                }),
                "validating {:?}",
                invalid_label
    ...
    fn format_security_contexts() {
        ...
        for label in [
            ...
            let security_context = policy
                .parse_security_context(label.as_bytes().into())
                .expect("should succeed");
            assert_eq!(
                policy.serialize_security_context(&security_context),
                label.as_bytes()
            );
        }
    }
