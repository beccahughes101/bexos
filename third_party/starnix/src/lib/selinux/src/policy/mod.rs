...
impl Policy {
    ...
    pub fn genfscon_label_for_fs_and_path(
        ...
        self.0
            .genfscon_label_for_fs_and_path(fs_type, node_path, class_id)
    }
    ...
    pub fn compute_create_context(
        ...
        self.0
            .compute_create_context(source, target, class.into(), name)
    }
    ...
    pub fn compute_access_decision(
        ...
        if let Some(target_class) = self.0.class(object_class.into()) {
            self.0
                .compute_access_decision(source_context, target_context, &target_class)
        } else {
...
impl AccessVectorComputer for Policy {
    fn access_decision_to_kernel_access_decision(
        ...
        for permission in class.permissions() {
            if let Some(permission_access_vector) = self
                .0
                .kernel_permission_to_access_vector(permission.clone())
            {
...
pub(super) mod tests {
    ...
    fn is_explicitly_allowed(
        ...
        let class = classes
            .get_by_name(target_class.as_bytes())
        ...
        let access_decision = policy
            .0
            .compute_explicitly_allowed(source_type, target_type, class);
    ...
    fn known_policies() {
        ...
        for [policy_path, policy_bytes, expectations_bytes] in policies_and_expectations {
            ...
            assert_eq!(
                expectations.expected_handle_unknown,
                policy.handle_unknown()
            );
    ...
    fn policy_lookup() {
        ...
        let policy = policy
            .validate()
            .expect("validate selinux testsuite policy");

        let unconfined_t = policy
            .types()
            .get_by_name(b"unconfined_t")
            .expect("look up type")
            .id();
    ...
    fn compute_explicitly_allowed_multiple_attributes() {
        ...
        let raw_access_vector = policy
            .0
            .compute_explicitly_allowed(a_t, a_t, class)
            .allow
            .value();
    ...
    fn access_vector_formats() {
        ...
        assert_eq!(
            format!("{:?}", AccessVector::NONE),
            "AccessVector(00000000)"
        );
    ...
    fn policy_genfscon_subpaths() {
        ...
        let path_label_expectations = [
            ...
            (
                "/a1/b/c",
                "system_u:object_r:fs_with_path_rules_a1_b_c_t:s0",
            ),
            // Matching paths defined in the policy:
            ...
            (
                "/a2/b/c/d",
                "system_u:object_r:fs_with_path_rules_a2_b_t:s0",
            ),
            // Matching paths defined in the policy:
