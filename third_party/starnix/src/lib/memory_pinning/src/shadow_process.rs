...
impl Drop for ShadowProcess {
    fn drop(&mut self) {
        ...
        self.process
            .kill()
            .expect("must be able to kill process we created");
    }
}
...
mod tests {
    ...
    async fn creation_sets_role() {
        ...
        std::thread::spawn(move || {
            ...
            .unwrap();
            send_vmar_koid
                .send(shadow_process.vmar.koid().unwrap())
                .unwrap();
        });
        ...
        match role_manager_server.next().await.unwrap().unwrap() {
            RoleManagerRequest::SetRole { payload, responder } => {
                responder
                    .send(Ok(RoleManagerSetRoleResponse::default()))
                    .unwrap();
    ...
    fn vmo_is_mapped_in_shadow_vmar() {
        ...
        assert_eq!(
            get_shadow_mappings(),
            &[],
            "VMAR should be empty before any pinning"
        );
        ...
        assert_eq!(
            mappings_after_pinning.len(),
            1,
            "there should only be one mapping in VMAR"
        );
        ...
        assert_eq!(
            get_shadow_mappings(),
            &[],
            "dropping PinnedMap must clean up"
        );
    }
}
