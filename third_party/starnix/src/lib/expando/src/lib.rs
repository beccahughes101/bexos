...
impl Expando {
    ...
    pub fn get<T: Any + Send + Sync + Default + 'static>(&self) -> Arc<T> {
        ...
        let slot = properties
            .entry(type_id)
            .or_insert_with(|| ExpandoSlot::new(Arc::new(T::default())));
        assert_eq!(type_id, slot.value.deref().type_id());
        slot.downcast()
            .expect("downcast of expando slot was successful")
    }
    ...
    pub fn get_or_init<T: Any + Send + Sync + 'static>(&self, init: impl FnOnce() -> T) -> Arc<T> {
        self.get_or_try_init::<T, ()>(|| Ok(init()))
    ...
    pub fn get_or_try_init<T: Any + Send + Sync + 'static, E>(
        ...
        let slot = properties
            .entry(type_id)
            .or_insert_with(|| ExpandoSlot::new(newly_init));
        assert_eq!(type_id, slot.value.deref().type_id());
        Ok(slot
            .downcast()
            .expect("downcast of expando slot was successful"))
