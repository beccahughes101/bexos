...
impl LineDiscipline {
    ...
    pub fn flush(&mut self, is_main: bool, queue_selector: u32) -> Result<(), Errno> {
        ...
        let (input_queue, output_queue) = if is_main {
            (
                self.output_queue.as_mut().unwrap(),
                self.input_queue.as_mut().unwrap(),
            )
        } else {
            (
                self.input_queue.as_mut().unwrap(),
            ...
            match character_bytes[0] {
                ...
                b'\t' => {
                    ...
                    if self.termios.c_oflag & TABDLY == XTABS {
                        ...
                        queue
                            .line_buffer
                            .extend(std::iter::repeat(b' ').take(spaces));
                        continue;
                    }
                    self.column += spaces;
                }
            ...
            && queue.readable_size() + queue.line_buffer.len() < CANON_MAX_BYTES
        {
            ...
            if self.lnext {
                ...
                if self.termios.has_local_flags(ECHO) {
                    ...
                    signals.append(with_queue!(
                        self.output_queue.write_bytes(self, &echo_bytes)
                    ));
                }
            ...
            if self.termios.has_local_flags(ICANON) {
                if self.termios.is_erase(first_byte) {
                    ...
                    maybe_erase_span = Some(compute_last_word_span(
                        &queue.line_buffer[..],
                        &self.termios,
                    ));
                ...
                if self.termios.is_kill(first_byte) {
                    maybe_erase_span = Some(compute_last_line_span(
                        &queue.line_buffer[..],
                        &self.termios,
                    ));
            ...
            if let Some(erase_span) = maybe_erase_span {
                ...
                queue
                    .line_buffer
                    .truncate(queue.line_buffer.len() - erase_span.bytes);
            ...
            if self.termios.has_local_flags(ECHO) {
                ...
                let needs_normal_echo = if maybe_erase_span.is_some() {
                    echo_bytes.is_empty()
                } else {
                    true
                };
...
impl Queue {
    ...
    fn flush_line_buffer(&mut self) {
        self.read_queue
            .push_back(std::mem::take(&mut self.line_buffer));
    }
