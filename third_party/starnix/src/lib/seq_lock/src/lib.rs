...
const fn sequence_offset<H>() -> usize {
    ...
    assert!(
        offset % align_of::<AtomicU32>() == 0,
        "Sequence must be correctly aligned"
    );
    offset
}
...
impl<H: IntoBytes + Immutable, T: SeqLockable> SeqLock<H, T> {
    ...
    pub unsafe fn new_from_vmo(
        ...
        const {
            ...
            assert!(
                align_of::<T>() >= write_size,
                "T must be aligned to the write size"
            );
            assert!(
                size_of::<T>() % write_size == 0,
                "size of T must be a multiple of write size"
            );
            assert!(
