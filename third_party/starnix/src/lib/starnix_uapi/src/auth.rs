...
impl Credentials {
    ...
    pub fn set_securebits(&mut self, securebits: SecureBits) -> Result<(), Errno> {
        ...
        if self
            .securebits
            .contains(SecureBits::NO_CAP_AMBIENT_RAISE_LOCKED)
        {
            locked_bits |=
...
mod tests {
    ...
    fn test_union() {
        ...
        assert_eq!(
            CAP_BLOCK_SUSPEND.union(CAP_BLOCK_SUSPEND),
            CAP_BLOCK_SUSPEND
        );
    }
    ...
    fn test_difference() {
        ...
        assert_eq!(
            base.difference(CAP_AUDIT_WRITE | CAP_BPF),
            Capabilities::empty()
        );
    }
    ...
    fn test_try_from() {
        ...
        assert_eq!(
            Capabilities::try_from(uapi::CAP_BLOCK_SUSPEND as u64),
            Ok(capabilities)
        );
