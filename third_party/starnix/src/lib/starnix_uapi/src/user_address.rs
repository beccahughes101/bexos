...
impl UserAddress {
    ...
    pub fn round_up(&self, increment: u64) -> Result<UserAddress, Errno> {
        Ok(UserAddress(
            round_up_to_increment(self.0 as usize, increment as usize)? as u64,
        ))
    }
...
impl TryFrom<UserAddress> for uapi::uaddr32 {
    ...
    fn try_from(value: UserAddress) -> Result<Self, ()> {
        Ok(Self {
            addr: value.0.try_into().map_err(|_| ())?,
        })
    }
}
...
impl ops::Add<u32> for UserAddress {
    ...
    fn add(self, rhs: u32) -> Result<UserAddress, Errno> {
        self.checked_add(rhs as usize)
            .map_or_else(|| error!(EFAULT), |res| Ok(res))
    }
}
...
impl<T, T64, T32> MappingMultiArchUserRef<T, T64, T32> {
    ...
    >(
        ...
        if arch.is_arch32() {
            ...
            user_ref
                .try_into()
                .map(|r| Self::Arch64(r, Default::default()))
        }
    }
    ...
    MappingMultiArchUserRef<T, T64, T32>
{
    pub fn into_bytes<Arch: ArchSpecific>(arch: &Arch, value: T) -> Result<Vec<u8>, ()> {
        if arch.is_arch32() {
            TryInto::<T32>::try_into(value)
                .map(|v| v.as_bytes().to_owned())
                .map_err(|_| ())
        } else {
            TryInto::<T64>::try_into(value)
                .map(|v| v.as_bytes().to_owned())
                .map_err(|_| ())
        }
    }
    ...
    MappingMultiArchUserRef<T, T64, T32>
{
    pub fn read_from_prefix<A: ArchSpecific>(a: &A, bytes: &[u8]) -> Result<T, ()> {
        if a.is_arch32() {
            T32::read_from_prefix(bytes)
                .map_err(|_| ())?
                .0
                .try_into()
                .map_err(|_| ())
        } else {
            T64::read_from_prefix(bytes)
                .map_err(|_| ())?
                .0
                .try_into()
                .map_err(|_| ())
    ...
    >
{
    pub fn next(&self) -> Result<Self, Errno> {
        ...
        self.addr().checked_add(offset).map_or_else(
            || error!(EFAULT),
            |user_address| Ok(Self::new(self, user_address)),
        )
    }
}
