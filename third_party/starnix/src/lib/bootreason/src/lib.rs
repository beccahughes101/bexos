...
pub async fn update_android_bootreason(
    ...
    if *HAS_STARNIX_SESSION_RESTARTED
        .get_or_init(async || has_session_restarted(dir).await)
        .await
    {
...
pub fn get_console_ramoops() -> Option<Vec<u8>> {
    ...
    {
        return Some(
            format!("Last Reboot Reason: Starnix Crash\n")
                .as_bytes()
                .to_vec(),
        );
    }
    match ANDROID_BOOTREASON.get() {
        ...
        Some(Err(e)) => {
            info!(
                "Failed to get android bootreason for console_ramoops: {:?}",
                e
            );
            None
        }
