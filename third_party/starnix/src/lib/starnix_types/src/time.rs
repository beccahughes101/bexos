...
pub fn timeval_from_time<T: zx::Timeline>(time: zx::Instant<T>) -> timeval {
    ...
    timeval {
        ...
        tv_usec: (nanos % NANOS_PER_SECOND) / 1000,
    }
}
...
mod test {
    ...
    fn test_invalid_time_from_timespec() {
        let time_spec = timespec {
            tv_sec: 100,
            tv_nsec: NANOS_PER_SECOND * 2,
        };
        assert_eq!(
            time_from_timespec::<zx::MonotonicTimeline>(time_spec),
            error!(EINVAL)
        );
    ...
    fn test_time_from_timeval() {
        ...
        assert_eq!(
            time.into_nanos(),
            100 * NANOS_PER_SECOND + 100 * NANOS_PER_MICRO
        );
    }
    ...
    fn test_invalid_time_from_timeval() {
        ...
        assert_eq!(
            time_from_timeval::<zx::MonotonicTimeline>(tv),
            error!(EINVAL)
        );
    }
}
