...
impl<T> OwnedRef<T> {
    pub fn new(value: T) -> Self {
        Self {
            inner: Some(Arc::new(RefInner::new(value))),
...
impl<T: Releasable> Share for OwnedRef<T> {
    ...
    fn share(&self) -> Self {
        ...
        debug_assert!(
            previous_count > 0,
            "OwnedRef should not be used after being released."
        );
        Self {
...
impl<T> std::fmt::Debug for WeakRefKey<T> {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        fmt.debug_tuple(std::any::type_name::<Self>())
            .field(&self.0.as_ptr())
            .finish()
    }
}
...
impl<T> RefInner<T> {
    ...
    fn wait_for_no_ref_once(self: &Arc<Self>) {
        ...
        let result = self
            .temp_refs_count
            .wait(current_value, None, zx::MonotonicInstant::INFINITE);
        debug_assert!(
