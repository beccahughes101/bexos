...
impl PortWaitResult {
    const NOTIFY_REGULAR: Self = Self::Notification {
        kind: NotifyKind::Regular,
    };
    const NOTIFY_INTERRUPT: Self = Self::Notification {
...
impl PortEvent {
    ...
    pub fn wait(&self, deadline: zx::MonotonicInstant) -> PortWaitResult {
        let mut state = self
            .futex
            .load(ORDERING_FOR_ATOMICS_BETWEEN_NOTIFIER_AND_NOTIFEE);
        loop {
            match state {
    ...
    pub fn object_wait_async(
        ...
        handle
            .as_handle_ref()
            .wait_async(&self.port, key, signals, opts)
    }
    ...
    fn test_signal_and_wait_block() {
        ...
        let thread = std::thread::spawn(move || {
            assert_eq!(
                ...
                PortWaitResult::Signal {
                    key: KEY,
                    observed: ASSERTED_SIGNAL
                }
            );
        });
