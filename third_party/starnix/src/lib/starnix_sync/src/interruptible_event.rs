...
impl Default for InterruptibleEvent {
    fn default() -> Self {
        InterruptibleEvent {
            futex: zx::Futex::new(READY),
            requeue_target: zx::Futex::new(READY),
        }
    }
}
...
impl InterruptibleEvent {
    ...
    pub fn get_owner(&self) -> Option<zx::Koid> {
        self.futex
            .get_owner()
            .or_else(|| self.requeue_target.get_owner())
    }
    ...
    pub fn assign_new_owner(&self, new_owner: &zx::Thread) -> Result<(), zx::Status> {
            ...
            .is_ok()
        {
            let res =
                self.futex
                    .requeue(0, REQUEUED, &self.requeue_target, u32::MAX, Some(new_owner));
    ...
    fn block_until(
        ...
        loop {
            ...
            let target_futex = if is_requeued {
                &self.requeue_target
            } else {
                &self.futex
            };
...
mod test {
    ...
    fn test_wait_block_and_notify() {
        ...
        guard
            .block_until(None, zx::MonotonicInstant::INFINITE)
            .expect("failed to be notified");
    ...
    fn futex_ownership_is_transferred() {
        ...
        let blocked_thread = std::thread::spawn(move || {
            ...
            let guard = event.begin_wait();
            guard
                .block_until(Some(&root_thread_handle), zx::MonotonicInstant::INFINITE)
                .unwrap();
        });
    ...
    fn concurrent_wait_wake_assign_new_owner_stress_test() {
        ...
        for _ in 0..200 {
            ...
            let thread_handle = root_thread_handle
