...
mod tracking {
    ...
    fn pop_lock(target_value: usize) -> Option<HeldLock> {
        let held_lock = STATE.try_with(|state| {
            ...
            let Some(pos) = s
                .held_locks
                .iter()
                .rposition(|v| v.encoded_value == target_value)
            else {
                return Err(format!(
    ...
    ///
    /// Returns the lock ID and the new subclass level, or 0 if no subclass is
    ...
    fn disable_subclass(encoded_value: usize) {
        ...
        let panic_message = STATE.try_with(|state| {
            ...
            let Some(pos) = s
                .held_locks
                .iter()
                .rposition(|v| v.encoded_value == encoded_value)
            else {
                return Err(format!(
    ...
    impl LockLevelToken {
        ...
        pub(super) fn new(lock_id: usize, name: &'static str) -> Self {
            ...
            Self {
                inner: Rc::new(InternalLockLevelToken { target_value }),
            }
        }
...
impl<T, L: crate::LockLevel> LockDepMutex<T, L> {
    pub const fn new(value: T) -> Self {
        Self {
            inner: DynamicLockDepMutex::new::<L>(value),
            _level: PhantomData,
        }
    }
...
impl<T, L: crate::LockLevel> LockDepRwLock<T, L> {
    pub const fn new(value: T) -> Self {
        Self {
            inner: DynamicLockDepRwLock::new::<L>(value),
            _level: PhantomData,
        }
    }
