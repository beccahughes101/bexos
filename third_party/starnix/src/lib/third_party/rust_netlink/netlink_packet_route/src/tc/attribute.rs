...
impl Nla for TcAttribute {
    fn value_len(&self) -> usize {
        match *self {
            Self::Rate(ref bytes) | Self::Fcnt(ref bytes) | Self::Stab(ref bytes) => bytes.len(),
            Self::Chain(_) => 4,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match *self {
            Self::Rate(ref bytes) | Self::Fcnt(ref bytes) | Self::Stab(ref bytes) => {
...
impl<'a, T: AsRef<[u8]> + ?Sized> ParseableParametrized<NlaBuffer<&'a T>, &str> for TcAttribute {
    type Error = TcError;
    fn parse_with_param(buf: &NlaBuffer<&'a T>, kind: &str) -> Result<Self, Self::Error> {
        let payload = buf.value();
        Ok(match buf.kind() {
            TCA_KIND => {
                TcAttribute::Kind(
                    parse_string(payload).map_err(|error| TcError::InvalidValue {
                        kind: "TCA_KIND",
                        error,
                    })?,
                )
            ...
            TCA_STATS => TcAttribute::Stats(
                TcStats::parse(&TcStatsBuffer::new(payload).map_err(|error| {
            ...
            TCA_XSTATS => TcAttribute::Xstats(TcXstats::parse_with_param(buf, kind)?),
            ...
            TCA_CHAIN => {
                TcAttribute::Chain(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_CHAIN",
                    error,
                })?)
            }
            TCA_HW_OFFLOAD => TcAttribute::HwOffload(parse_u8(payload).map_err(|error| {
                TcError::InvalidValue {
                    kind: "TCA_HW_OFFLOAD",
                    error,
                }
