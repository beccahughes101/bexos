            ...
            match nla.kind() {
                IFLA_INFO_XSTATS => {
                    if let Some(link_info_kind) = &link_info_kind {
                        nlas.push(LinkInfo::Xstats(LinkXstats::parse_with_param(
                            &nla,
                            link_info_kind,
                        )?));
                    } else {
                ...
                IFLA_INFO_PORT_DATA => {
                    if let Some(link_info_port_kind) = link_info_port_kind {
                        nlas.push(LinkInfo::PortData(InfoPortData::parse_with_param(
                            nla.value(),
                            link_info_port_kind,
                        )?));
                    } else {
                ...
                _kind => nlas
                    .push(LinkInfo::Other(DefaultNla::parse(&nla).context(
