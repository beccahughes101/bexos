...
use super::lwtunnel::VecRouteLwTunnelEncap;
use super::metrics::VecRouteMetric;
use super::mpls::VecMplsLabel;
use super::{
    MplsLabel, RouteAddress, RouteCacheInfo, RouteCacheInfoBuffer, RouteError, RouteLwEnCapType,
    RouteLwTunnelEncap, RouteMetric, RouteMfcStats, RouteMfcStatsBuffer, RouteMplsTtlPropagation,
    RouteNextHop, RouteNextHopBuffer, RoutePreference, RouteRealm, RouteType, RouteVia,
    RouteViaBuffer,
};
...
impl Nla for RouteAttribute {
    fn value_len(&self) -> usize {
        match self {
            ...
            Self::MultiPath(next_hops) => next_hops.iter().map(|nh| nh.buffer_len()).sum(),
            Self::Preference(_) => 1,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::MulticastExpires(value) => NativeEndian::write_u64(buffer, *value),
...
impl<'a, T: AsRef<[u8]> + ?Sized>
    ParseableParametrized<NlaBuffer<&'a T>, (AddressFamily, RouteType, RouteLwEnCapType)>
    for RouteAttribute
{
    ...
    fn parse_with_param(
        ...
        Ok(match buf.kind() {
            RTA_DST => Self::Destination(RouteAddress::parse(address_family, payload).map_err(
                |error| RouteError::InvalidValue {
                    kind: "RTA_DST",
                    error,
                },
            )?),
            RTA_SRC => Self::Source(RouteAddress::parse(address_family, payload).map_err(
                |error| RouteError::InvalidValue {
                    kind: "RTA_SRC",
                    error,
                },
            )?),
            RTA_GATEWAY => Self::Gateway(RouteAddress::parse(address_family, payload).map_err(
                |error| RouteError::InvalidValue {
                    kind: "RTA_GATEWAY",
                    error,
                },
            )?),
            RTA_PREFSRC => Self::PrefSource(RouteAddress::parse(address_family, payload).map_err(
                |error| RouteError::InvalidValue {
                    kind: "RTA_PREFSRC",
                    error,
                },
            ...
            RTA_VIA => Self::Via(
                RouteVia::parse(&RouteViaBuffer::new(payload).map_err(|error| {
            ...
            RTA_ENCAP => Self::Encap(VecRouteLwTunnelEncap::parse_with_param(buf, encap_type)?.0),
            RTA_EXPIRES => {
                if route_type == RouteType::Multicast {
                    Self::MulticastExpires(parse_u64(payload).map_err(|error| {
                        RouteError::InvalidValue {
                    ...
                    Self::Expires(
                        parse_u32(payload).map_err(|error| RouteError::InvalidValue {
                            kind: "RTA_EXPIRES",
                            error,
                        })?,
                    )
            ...
            RTA_TTL_PROPAGATE => Self::TtlPropagate(RouteMplsTtlPropagation::from(
                parse_u8(payload).map_err(|error| RouteError::InvalidValue {
                    kind: "RTA_TTL_PROPAGATE",
                    error,
                })?,
            )),
            RTA_ENCAP_TYPE => Self::EncapType(RouteLwEnCapType::from(parse_u16(payload).map_err(
                |error| RouteError::InvalidValue {
                    kind: "RTA_ENCAP_TYPE",
                    error,
                },
            ...
            RTA_CACHEINFO => Self::CacheInfo(
                RouteCacheInfo::parse(&RouteCacheInfoBuffer::new(payload).map_err(|error| {
                    RouteError::InvalidValue {
                        kind: "RTA_CACHEINFO",
                        error,
                    }
            ...
            _ => Self::Other(
                DefaultNla::parse(buf).map_err(|error| RouteError::InvalidValue {
