...
impl Nla for InfoMacVlan {
    fn value_len(&self) -> usize {
        match self {
            ...
            Self::MacAddr(_) => 6,
            Self::MacAddrData(nlas) => nlas.as_slice().buffer_len(),
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            Self::Mode(value) => NativeEndian::write_u32(buffer, (*value).into()),
            ...
            Self::MacAddrData(nlas) => nlas.as_slice().emit(buffer),
            Self::MacAddrCount(value) => NativeEndian::write_u32(buffer, *value),
            Self::BcQueueLen(value) => NativeEndian::write_u32(buffer, *value),
            Self::BcQueueLenUsed(value) => NativeEndian::write_u32(buffer, *value),
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_MACVLAN_BC_QUEUE_LEN => {
                BcQueueLen(parse_u32(payload).context("invalid IFLA_MACVLAN_BC_QUEUE_LEN value")?)
            }
            IFLA_MACVLAN_BC_QUEUE_LEN_USED => BcQueueLenUsed(
                parse_u32(payload).context("invalid IFLA_MACVLAN_BC_QUEUE_LEN_USED value")?,
            ),
            IFLA_MACVLAN_BC_CUTOFF => {
                BcCutoff(parse_i32(payload).context("invalid IFLA_MACVLAN_BC_CUTOFF value")?)
            }
...
impl Nla for InfoMacVtap {
    fn value_len(&self) -> usize {
        ...
        match self {
            ...
            MacAddrMode(_) => 4,
            MacAddr(_) => 6,
            MacAddrData(nlas) => nlas.as_slice().buffer_len(),
            MacAddrCount(_) => 4,
            BcQueueLen(_) => 4,
            BcQueueLenUsed(_) => 4,
            BcCutoff(_) => 4,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        ...
        match self {
            ...
            MacAddrData(nlas) => nlas.as_slice().emit(buffer),
