...
use super::{
    TcError, TcFilterMatchAll, TcFilterMatchAllOption, TcFilterU32, TcFilterU32Option,
...
{
    type Error = TcError;
    fn parse_with_param(buf: &NlaBuffer<&'a T>, kind: &str) -> Result<Self, TcError> {
        Ok(match kind {
            TcQdiscIngress::KIND => {
                Self::Ingress(TcQdiscIngressOption::parse(buf).map_err(|error| {
                    TcError::ParseTcaOptionAttributes {
                        kind: "ingress",
                        error,
                    }
                })?)
            }
            TcQdiscFqCodel::KIND => Self::FqCodel(TcQdiscFqCodelOption::parse(buf)?),
            TcFilterU32::KIND => Self::U32(TcFilterU32Option::parse(buf)?),
            TcFilterMatchAll::KIND => Self::MatchAll(TcFilterMatchAllOption::parse(buf)?),
            kind => {
                Self::Other(
                    DefaultNla::parse(buf).map_err(|error| TcError::UnknownOption {
                        kind: kind.to_string(),
                        error,
                    })?,
                )
...
{
    type Error = TcError;
    fn parse_with_param(buf: &NlaBuffer<&'a T>, kind: &str) -> Result<VecTcOption, TcError> {
        Ok(match kind {
            TcFilterU32::KIND
            ...
            kind => Self(vec![TcOption::Other(DefaultNla::parse(buf).map_err(
