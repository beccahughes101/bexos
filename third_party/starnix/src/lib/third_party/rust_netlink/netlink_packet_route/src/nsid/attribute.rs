...
use netlink_packet_utils::parsers::{parse_i32, parse_u32};
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for NsidAttribute {
    type Error = NsidError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, NsidError> {
        let payload = buf.value();
        Ok(match buf.kind() {
            NETNSA_NSID => {
                Self::Id(parse_i32(payload).map_err(|error| NsidError::InvalidValue {
                    kind: "NETNSA_NSID",
                    error,
                })?)
            }
            NETNSA_PID => {
                Self::Pid(parse_u32(payload).map_err(|error| NsidError::InvalidValue {
                    kind: "NETNSA_PID",
                    error,
                })?)
            }
            NETNSA_FD => Self::Fd(parse_u32(payload).map_err(|error| NsidError::InvalidValue {
                kind: "NETNSA_FD",
                error,
            })?),
            NETNSA_TARGET_NSID => {
                Self::TargetNsid(parse_i32(payload).map_err(|error| NsidError::InvalidValue {
                    kind: "NETNSA_TARGET_NSID",
                    error,
                })?)
            }
            NETNSA_CURRENT_NSID => {
                Self::CurrentNsid(parse_i32(payload).map_err(|error| NsidError::InvalidValue {
                    kind: "NETNSA_CURRENT_NSID",
                    error,
                })?)
            }
            kind => Self::Other(
                DefaultNla::parse(buf).map_err(|error| NsidError::UnknownNLA { kind, error })?,
            ),
        })
    }
}
