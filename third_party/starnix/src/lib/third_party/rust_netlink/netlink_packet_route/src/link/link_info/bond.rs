...
impl Nla for BondAdInfo {
    fn value_len(&self) -> usize {
        match self {
            Self::Aggregator(_) | Self::NumPorts(_) | Self::ActorKey(_) | Self::PartnerKey(_) => 2,
            Self::PartnerMac(_) => 6,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            Self::Aggregator(d) | Self::NumPorts(d) | Self::ActorKey(d) | Self::PartnerKey(d) => {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_BOND_AD_INFO_NUM_PORTS => Self::NumPorts(
                parse_u16(payload).context("invalid IFLA_BOND_AD_INFO_NUM_PORTS value")?,
            ),
            IFLA_BOND_AD_INFO_ACTOR_KEY => Self::ActorKey(
                parse_u16(payload).context("invalid IFLA_BOND_AD_INFO_ACTOR_KEY value")?,
            ),
            ...
            _ => Self::Other(
                DefaultNla::parse(buf)
                    .context(format!("invalid NLA for {}: {payload:?}", buf.kind()))?,
            ),
        })
    }
}
...
impl Nla for InfoBond {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            | Self::PeerNotifDelay(value) => NativeEndian::write_u32(buffer, *value),
            Self::AdActorSystem(bytes) => buffer.copy_from_slice(bytes),
            Self::ArpIpTarget(addrs) => BondIpAddrNlaList::from(addrs).as_slice().emit(buffer),
            Self::NsIp6Target(addrs) => BondIpAddrNlaList::from(addrs).as_slice().emit(buffer),
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoBond {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_BOND_ACTIVE_PORT => {
                Self::ActivePort(parse_u32(payload).context("invalid IFLA_BOND_ACTIVE_PORT value")?)
            }
            IFLA_BOND_MIIMON => {
                Self::MiiMon(parse_u32(payload).context("invalid IFLA_BOND_MIIMON value")?)
            }
            IFLA_BOND_UPDELAY => {
                Self::UpDelay(parse_u32(payload).context("invalid IFLA_BOND_UPDELAY value")?)
            }
            IFLA_BOND_DOWNDELAY => {
                Self::DownDelay(parse_u32(payload).context("invalid IFLA_BOND_DOWNDELAY value")?)
            }
            IFLA_BOND_USE_CARRIER => {
                Self::UseCarrier(parse_u8(payload).context("invalid IFLA_BOND_USE_CARRIER value")?)
            }
            IFLA_BOND_ARP_INTERVAL => Self::ArpInterval(
                parse_u32(payload).context("invalid IFLA_BOND_ARP_INTERVAL value")?,
            ),
            ...
            IFLA_BOND_ARP_VALIDATE => Self::ArpValidate(
                parse_u32(payload).context("invalid IFLA_BOND_ARP_VALIDATE value")?,
            ),
            IFLA_BOND_ARP_ALL_TARGETS => Self::ArpAllTargets(
                parse_u32(payload).context("invalid IFLA_BOND_ARP_ALL_TARGETS value")?,
            ),
            IFLA_BOND_PRIMARY => {
                Self::Primary(parse_u32(payload).context("invalid IFLA_BOND_PRIMARY value")?)
            }
            ...
            IFLA_BOND_FAIL_OVER_MAC => Self::FailOverMac(
                parse_u8(payload).context("invalid IFLA_BOND_FAIL_OVER_MAC value")?,
            ),
            IFLA_BOND_XMIT_HASH_POLICY => Self::XmitHashPolicy(
                parse_u8(payload).context("invalid IFLA_BOND_XMIT_HASH_POLICY value")?,
            ),
            IFLA_BOND_RESEND_IGMP => {
                Self::ResendIgmp(parse_u32(payload).context("invalid IFLA_BOND_RESEND_IGMP value")?)
            }
            IFLA_BOND_NUM_PEER_NOTIF => Self::NumPeerNotif(
                parse_u8(payload).context("invalid IFLA_BOND_NUM_PEER_NOTIF value")?,
            ),
            IFLA_BOND_ALL_PORTS_ACTIVE => Self::AllPortsActive(
                parse_u8(payload).context("invalid IFLA_BOND_ALL_PORTS_ACTIVE value")?,
            ),
            IFLA_BOND_MIN_LINKS => {
                Self::MinLinks(parse_u32(payload).context("invalid IFLA_BOND_MIN_LINKS value")?)
            }
            IFLA_BOND_LP_INTERVAL => {
                Self::LpInterval(parse_u32(payload).context("invalid IFLA_BOND_LP_INTERVAL value")?)
            }
            IFLA_BOND_PACKETS_PER_PORT => Self::PacketsPerPort(
                parse_u32(payload).context("invalid IFLA_BOND_PACKETS_PER_PORT value")?,
            ),
            IFLA_BOND_AD_LACP_RATE => {
                Self::AdLacpRate(parse_u8(payload).context("invalid IFLA_BOND_AD_LACP_RATE value")?)
            }
            IFLA_BOND_AD_SELECT => {
                Self::AdSelect(parse_u8(payload).context("invalid IFLA_BOND_AD_SELECT value")?)
            }
            IFLA_BOND_AD_INFO => {
            ...
            IFLA_BOND_TLB_DYNAMIC_LB => Self::TlbDynamicLb(
                parse_u8(payload).context("invalid IFLA_BOND_TLB_DYNAMIC_LB value")?,
            ),
            ...
            IFLA_BOND_AD_LACP_ACTIVE => Self::AdLacpActive(
                parse_u8(payload).context("invalid IFLA_BOND_AD_LACP_ACTIVE value")?,
            ),
            IFLA_BOND_MISSED_MAX => {
                Self::MissedMax(parse_u8(payload).context("invalid IFLA_BOND_MISSED_MAX value")?)
            }
