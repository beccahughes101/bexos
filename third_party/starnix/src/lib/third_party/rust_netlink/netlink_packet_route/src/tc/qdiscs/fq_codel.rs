...
impl Emitable for TcFqCodelXstats {
    fn buffer_len(&self) -> usize {
        match self {
            Self::Qdisc(_) => TC_FQ_CODEL_QD_STATS_LEN + std::mem::size_of::<u32>(),
            Self::Class(_) => TC_FQ_CODEL_CL_STATS_LEN + std::mem::size_of::<u32>(),
            Self::Other(v) => v.len(),
        }
    }
    ...
    fn emit(&self, buffer: &mut [u8]) {
        match self {
            Self::Qdisc(v) => {
                buffer[0..4].copy_from_slice(&TCA_FQ_CODEL_XSTATS_QDISC.to_ne_bytes());
                v.emit(&mut buffer[4..]);
            }
            Self::Class(v) => {
                buffer[0..4].copy_from_slice(&TCA_FQ_CODEL_XSTATS_CLASS.to_ne_bytes());
                v.emit(&mut buffer[4..]);
            }
...
impl Nla for TcQdiscFqCodelOption {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::CeThresholdSelector(d) | Self::CeThresholdMask(d) => buffer[0] = *d,
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for TcQdiscFqCodelOption {
    type Error = TcError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
        let payload = buf.value();
        Ok(match buf.kind() {
            TCA_FQ_CODEL_TARGET => {
                Self::Target(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_TARGET",
                    error,
                })?)
            }
            TCA_FQ_CODEL_LIMIT => {
                Self::Limit(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_LIMIT",
                    error,
                })?)
            }
            TCA_FQ_CODEL_INTERVAL => {
                Self::Interval(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_INTERVAL",
                    error,
                })?)
            }
            TCA_FQ_CODEL_ECN => {
                Self::Ecn(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_ECN",
                    error,
                })?)
            }
            TCA_FQ_CODEL_FLOWS => {
                Self::Flows(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_FLOWS",
                    error,
                })?)
            }
            TCA_FQ_CODEL_QUANTUM => {
                Self::Quantum(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_QUANTUM",
                    error,
                })?)
            }
            TCA_FQ_CODEL_CE_THRESHOLD => {
                Self::CeThreshold(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_CE_THRESHOLD",
                    error,
                })?)
            }
            TCA_FQ_CODEL_DROP_BATCH_SIZE => {
                Self::DropBatchSize(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_DROP_BATCH_SIZE",
                    error,
                })?)
            }
            TCA_FQ_CODEL_MEMORY_LIMIT => {
                Self::MemoryLimit(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_FQ_CODEL_MEMORY_LIMIT",
                    error,
                })?)
            }
            TCA_FQ_CODEL_CE_THRESHOLD_SELECTOR => {
                Self::CeThresholdSelector(parse_u8(payload).map_err(|error| {
            ...
            TCA_FQ_CODEL_CE_THRESHOLD_MASK => {
                Self::CeThresholdMask(parse_u8(payload).map_err(|error| TcError::InvalidValue {
