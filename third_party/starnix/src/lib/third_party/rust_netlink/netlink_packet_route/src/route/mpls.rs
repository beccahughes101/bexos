...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for RouteMplsIpTunnel {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, RouteError> {
        ...
        Ok(match buf.kind() {
            MPLS_IPTUNNEL_DST => Self::Destination(VecMplsLabel::parse(payload)?.0),
            MPLS_IPTUNNEL_TTL => {
                Self::Ttl(parse_u8(payload).map_err(RouteError::InvalidMplsIpTunnelTtl)?)
            }
            _ => Self::Other(DefaultNla::parse(buf).map_err(RouteError::InvalidMplsIpTunnelTtl)?),
        })
    }
}
