    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
        ...
        for iter in NlasIterator::new(buf.value()) {
            ...
            attributes.push(match buf.kind() {
                TCA_ACT_KIND => {
                    kind = parse_string(payload).map_err(|error| TcError::ParseAction {
                        kind: "TCA_ACT_KIND",
                        error,
                    })?;
                ...
                TCA_ACT_OPTIONS => {
                    ...
                    for nla in NlasIterator::new(payload) {
                        let nla = nla?;
                        nlas.push(TcActionOption::parse_with_param(&nla, &kind)?);
                    }
                    TcActionAttribute::Options(nlas)
                }
                TCA_ACT_INDEX => TcActionAttribute::Index(parse_u32(payload).map_err(|error| {
                    TcError::ParseAction {
                        kind: "TCA_ACT_INDEX",
                        error,
                    }
                ...
                TCA_ACT_IN_HW_COUNT => {
                    TcActionAttribute::InHwCount(parse_u32(payload).map_err(|error| {
                        TcError::ParseAction {
...
impl Nla for TcActionAttribute {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Index(value) | Self::InHwCount(value) => NativeEndian::write_u32(buffer, *value),
...
{
    type Error = TcError;
    fn parse_with_param(buf: &NlaBuffer<&'a T>, kind: S) -> Result<Self, TcError> {
        Ok(match kind.as_ref() {
            TcActionMirror::KIND => {
                Self::Mirror(TcActionMirrorOption::parse(buf).map_err(TcError::ParseMirrorAction)?)
            }
            TcActionNat::KIND => {
                Self::Nat(TcActionNatOption::parse(buf).map_err(TcError::ParseMirrorAction)?)
            }
            _ => Self::Other(DefaultNla::parse(buf).map_err(TcError::ParseMirrorAction)?),
        })
    }
}
