    ...
    fn parse_with_param(
        ...
        Ok(match buf.kind() {
            NDA_DST => Self::Destination(
                NeighbourAddress::parse_with_param(address_family, payload).map_err(|error| {
            ...
            NDA_PROBES => {
                Self::Probes(
                    parse_u32(payload).map_err(|error| NeighbourError::InvalidValue {
                        kind: "NDA_PROBES",
                        error,
                    })?,
                )
            ...
            NDA_VLAN => {
                Self::Vlan(
                    parse_u16(payload).map_err(|error| NeighbourError::InvalidValue {
                        kind: "NDA_VLAN",
                        error,
                    })?,
                )
            ...
            NDA_PORT => {
                Self::Port(
                    parse_u16_be(payload).map_err(|error| NeighbourError::InvalidValue {
                        kind: "NDA_PORT",
                        error,
                    })?,
                )
            ...
            NDA_CONTROLLER => Self::Controller(parse_u32(payload).map_err(|error| {
                NeighbourError::InvalidValue {
                    kind: "NDA_CONTROLLER",
                    error,
                }
            ...
            NDA_SRC_VNI => Self::SourceVni(parse_u32(payload).map_err(|error| {
                NeighbourError::InvalidValue {
                    kind: "NDA_SRC_VNI",
                    error,
                }
