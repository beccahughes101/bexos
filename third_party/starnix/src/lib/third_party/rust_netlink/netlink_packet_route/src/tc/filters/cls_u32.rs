...
impl Nla for TcFilterU32Option {
    fn value_len(&self) -> usize {
        match self {
            Self::Police(b) | Self::Indev(b) | Self::Pnct(b) | Self::Mark(b) => b.len(),
            Self::Hash(_) | Self::Link(_) | Self::Divisor(_) | Self::Flags(_) => 4,
            Self::ClassId(_) => 4,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            Self::Police(b) | Self::Indev(b) | Self::Pnct(b) | Self::Mark(b) => {
                buffer.copy_from_slice(b.as_slice())
            }
            Self::Hash(i) | Self::Link(i) | Self::Divisor(i) => NativeEndian::write_u32(buffer, *i),
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for TcFilterU32Option {
    type Error = TcError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
        let payload = buf.value();
        Ok(match buf.kind() {
            TCA_U32_CLASSID => {
                Self::ClassId(TcHandle::from(parse_u32(payload).map_err(|error| {
            ...
            TCA_U32_HASH => {
                Self::Hash(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_U32_HASH",
                    error,
                })?)
            }
            TCA_U32_LINK => {
                Self::Link(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_U32_LINK",
                    error,
                })?)
            }
            TCA_U32_DIVISOR => {
                Self::Divisor(parse_u32(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_U32_DIVISOR",
                    error,
                })?)
            }
            TCA_U32_SEL => Self::Selector(TcU32Selector::parse(
                &TcU32SelectorBuffer::new(payload).map_err(|error| TcError::InvalidValue {
                    kind: "TCA_U32_SEL",
                    error,
                })?,
            )?),
...
impl<T: AsRef<[u8]>> TcU32SelectorBuffer<T> {
    ...
    fn check_buffer_length(&self) -> Result<(), DecodeError> {
        ...
        let expected_len = ((self.nkeys() as usize) * TC_U32_KEY_BUF_LEN) + TC_U32_SEL_BUF_LEN;
        if len < expected_len {
            return Err(format!(
...
impl Emitable for TcU32Selector {
    ...
    fn emit(&self, buffer: &mut [u8]) {
        ...
        for (i, k) in self.keys.iter().enumerate() {
            k.emit(&mut key_buf[(i * TC_U32_KEY_BUF_LEN)..((i + 1) * TC_U32_KEY_BUF_LEN)]);
        }
    }
}

impl<T: AsRef<[u8]> + ?Sized> Parseable<TcU32SelectorBuffer<&T>> for TcU32Selector {
    type Error = TcError;
