...
use super::param::VecNeighbourTableParameter;
use super::{
    NeighbourTableConfig, NeighbourTableConfigBuffer, NeighbourTableError, NeighbourTableParameter,
    NeighbourTableStats, NeighbourTableStatsBuffer,
};
use byteorder::{ByteOrder, NativeEndian};
use netlink_packet_utils::nla::{DefaultNla, Nla, NlaBuffer};
use netlink_packet_utils::parsers::{parse_string, parse_u32, parse_u64};
...
impl Nla for NeighbourTableAttribute {
    fn value_len(&self) -> usize {
        match self {
            ...
            Self::Name(s) => s.len() + 1,
            Self::Threshold1(_) | Self::Threshold2(_) | Self::Threshold3(_) => 4,
            Self::GcInterval(_) => 8,
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Threshold1(value) | Self::Threshold2(value) | Self::Threshold3(value) => {
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for NeighbourTableAttribute {
    type Error = NeighbourTableError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, Self::Error> {
        let payload = buf.value();
        Ok(match buf.kind() {
            NDTA_NAME => Self::Name(parse_string(payload).map_err(|error| {
            ...
            NDTA_PARMS => {
                Self::Parms(VecNeighbourTableParameter::parse(&NlaBuffer::new(payload)?)?.0)
            }
            NDTA_GC_INTERVAL => Self::GcInterval(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidValue {
                    kind: "NDTA_GC_INTERVAL",
                    error,
                }
            })?),
            NDTA_THRESH1 => Self::Threshold1(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue {
                    kind: "NDTA_THRESH1",
                    error,
                }
            })?),
            NDTA_THRESH2 => Self::Threshold2(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue {
                    kind: "NDTA_THRESH2",
                    error,
                }
            })?),
            NDTA_THRESH3 => Self::Threshold3(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue {
                    kind: "NDTA_THRESH3",
                    error,
                }
            ...
            kind => Self::Other(
                DefaultNla::parse(buf)
                    .map_err(|error| NeighbourTableError::UnknownNla { kind, error })?,
            ),
        })
    }
}
