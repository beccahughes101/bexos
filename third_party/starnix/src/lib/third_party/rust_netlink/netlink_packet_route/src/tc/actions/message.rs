...
impl<'a, T: AsRef<[u8]> + 'a + ?Sized> Parseable<NlaBuffer<&'a T>> for TcActionMessageAttribute {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        Ok(match buf.kind() {
            ...
            TCA_ROOT_FLAGS => Self::Flags(TcActionMessageFlagsWithSelector::parse(buf)?),
            TCA_ROOT_COUNT => {
                let count = u32::from_ne_bytes(buf.value().try_into().context("invalid length")?);
                Self::RootCount(count)
            }
            TCA_ROOT_TIME_DELTA => {
                let delta = u32::from_be_bytes(buf.value().try_into().context("invalid length")?);
                Self::RootTimeDelta(delta)
            }
            TCA_ROOT_EXT_WARN_MSG => {
                let msg = String::from_utf8(buf.value().to_vec()).context("invalid utf8")?;
...
impl<'a, T: AsRef<[u8]> + 'a + ?Sized> Parseable<TcActionMessageBuffer<&'a T>> for TcActionMessage {
    type Error = DecodeError;
