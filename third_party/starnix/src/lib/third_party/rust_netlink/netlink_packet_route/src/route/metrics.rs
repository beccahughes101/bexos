...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for RouteMetric {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, RouteError> {
        ...
        Ok(match buf.kind() {
            RTAX_LOCK => {
                Self::Lock(
                    parse_u32(payload).map_err(|error| RouteError::InvalidRouteMetric {
                        kind: "RTAX_LOCK",
                        error,
                    })?,
                )
            ...
            RTAX_WINDOW => Self::Window(parse_u32(payload).map_err(|error| {
            ...
            RTAX_RTTVAR => Self::RttVar(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_RTTVAR",
                    error,
                }
            })?),
            RTAX_SSTHRESH => Self::SsThresh(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_SSHTHRESH",
                    error,
                }
            ...
            RTAX_ADVMSS => Self::Advmss(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_ADVMSS",
                    error,
                }
            })?),
            RTAX_REORDERING => Self::Reordering(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_REORDERING",
                    error,
                }
            })?),
            RTAX_HOPLIMIT => Self::Hoplimit(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_HOPLIMIT",
                    error,
                }
            })?),
            RTAX_INITCWND => Self::InitCwnd(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_INITCWND",
                    error,
                }
            })?),
            RTAX_FEATURES => Self::Features(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_FEATURES",
                    error,
                }
            })?),
            RTAX_RTO_MIN => Self::RtoMin(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_RTO_MIN",
                    error,
                }
            })?),
            RTAX_INITRWND => Self::InitRwnd(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_INITRWND",
                    error,
                }
            })?),
            RTAX_QUICKACK => Self::QuickAck(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_QUICKACK",
                    error,
                }
            })?),
            RTAX_CC_ALGO => Self::CcAlgo(parse_u32(payload).map_err(|error| {
                RouteError::InvalidRouteMetric {
                    kind: "RTAX_CC_ALGO",
                    error,
                }
