...
impl Nla for RuleAttribute {
    fn value_len(&self) -> usize {
        match self {
            ...
            Self::SourcePortRange(v) | Self::DestinationPortRange(v) => v.buffer_len(),
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            Self::Destination(ip) | Self::Source(ip) => emit_ip_addr(ip, buffer),
            Self::SourcePortRange(v) | Self::DestinationPortRange(v) => v.emit(buffer),
            Self::UidRange(v) => v.emit(buffer),
            Self::Iifname(s) | Self::Oifname(s) => buffer[..s.len()].copy_from_slice(s.as_bytes()),
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for RuleAttribute {
    type Error = RuleError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, RuleError> {
        let payload = buf.value();

        Ok(match buf.kind() {
            FRA_DST => Self::Destination(parse_ip_addr(payload).map_err(|error| {
            ...
            FRA_SRC => {
                Self::Source(
                    parse_ip_addr(payload).map_err(|error| RuleError::InvalidValue {
                        kind: "FRA_DST",
                        error,
                    })?,
                )
            ...
            FRA_IIFNAME => {
                Self::Iifname(
                    parse_string(payload).map_err(|error| RuleError::InvalidValue {
                        kind: "FRA_IIFNAME",
                        error,
                    })?,
                )
            ...
            FRA_GOTO => {
                Self::Goto(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_GOTO",
                    error,
                })?)
            }
            FRA_PRIORITY => {
                Self::Priority(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_PRIORITY",
                    error,
                })?)
            }
            FRA_FWMARK => {
                Self::FwMark(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_FWMARK",
                    error,
                })?)
            }
            ...
            FRA_TUN_ID => {
                Self::TunId(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_TUN_ID",
                    error,
                })?)
            }
            FRA_SUPPRESS_IFGROUP => Self::SuppressIfGroup(parse_u32(payload).map_err(|error| {
                RuleError::InvalidValue {
            ...
            FRA_SUPPRESS_PREFIXLEN => {
                Self::SuppressPrefixLen(parse_u32(payload).map_err(|error| {
                    RuleError::InvalidValue {
            ...
            FRA_TABLE => {
                Self::Table(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_TABLE",
                    error,
                })?)
            }
            FRA_FWMASK => {
                Self::FwMask(parse_u32(payload).map_err(|error| RuleError::InvalidValue {
                    kind: "FRA_FWMASK",
                    error,
                })?)
            }
            ...
            FRA_IP_PROTO => {
                Self::IpProtocol(IpProtocol::from(parse_u8(payload).map_err(|error| {
                    RuleError::InvalidValue {
                        kind: "FRA_IP_PROTO",
                        error,
                    }
            ...
            FRA_SPORT_RANGE => Self::SourcePortRange(RulePortRange::parse(payload)?),
            FRA_DPORT_RANGE => Self::DestinationPortRange(RulePortRange::parse(payload)?),
            kind => Self::Other(
                DefaultNla::parse(buf).map_err(|error| RuleError::UnknownNLA { kind, error })?,
            ),
        })
    }
}
