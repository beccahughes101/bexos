...
use crate::address::{AddressError, AddressFlags, CacheInfo, CacheInfoBuffer};
...
impl Nla for AddressAttribute {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match *self {
            ...
            Self::Flags(ref value) => NativeEndian::write_u32(buffer, value.bits()),
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for AddressAttribute {
    type Error = AddressError;

    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, AddressError> {
        ...
        Ok(match buf.kind() {
            IFA_ADDRESS => {
                if payload.len() == IPV4_ADDR_LEN {
                    ...
                    return Err(AddressError::ParseAttributeInvalidPayload {
                        kind: "IFA_ADDRESS",
            ...
            IFA_LABEL => {
                Self::Label(
                    parse_string(payload).map_err(|err| AddressError::ParseAttribute {
                        kind: "IFA_LABEL",
                        err,
                    })?,
                )
            ...
            IFA_CACHEINFO => Self::CacheInfo(
                CacheInfo::parse(&CacheInfoBuffer::new(payload).map_err(|err| {
                    AddressError::ParseAttribute {
                        kind: "IFA_CACHEINFO",
                        err,
                    }
                })?)
                .map_err(|err| AddressError::ParseAttribute {
                    kind: "IFA_CACHEINFO",
                    err,
                })?,
            ),
            ...
            IFA_FLAGS => Self::Flags(AddressFlags::from_bits_retain(parse_u32(payload).map_err(
                |err| AddressError::ParseAttribute {
                    kind: "IFA_FLAGS",
                    err,
                },
            )?)),
            kind => Self::Other(
                DefaultNla::parse(buf)
                    .map_err(|err| AddressError::ParseUnknownNLA { kind, err })?,
            ),
        })
    }
}
