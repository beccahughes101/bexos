...
use crate::link::{
    VfInfoBroadcast, VfInfoBroadcastBuffer, VfInfoGuid, VfInfoGuidBuffer, VfInfoLinkState,
    VfInfoLinkStateBuffer, VfInfoMac, VfInfoMacBuffer, VfInfoRate, VfInfoRateBuffer,
    VfInfoRssQueryEn, VfInfoRssQueryEnBuffer, VfInfoSpoofCheck, VfInfoSpoofCheckBuffer,
    VfInfoTrust, VfInfoTrustBuffer, VfInfoTxRate, VfInfoTxRateBuffer, VfInfoVlan, VfInfoVlanBuffer,
    VfStats, VfVlan,
};
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for VecLinkVfInfo {
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for LinkVfInfo {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        let mut nlas = vec![];
        for nla in NlasIterator::new(buf.into_inner()) {
            let nla = &nla.context(format!("invalid IFLA_VF_INFO value {:?}", buf.value()))?;
        ...
        Ok(match buf.kind() {
            ...
            IFLA_VF_BROADCAST => Self::Broadcast(
                VfInfoBroadcast::parse(&VfInfoBroadcastBuffer::new(payload)?)
                    .context(format!("invalid IFLA_VF_BROADCAST {payload:?}"))?,
            ),
            ...
            IFLA_VF_LINK_STATE => Self::LinkState(
                VfInfoLinkState::parse(&VfInfoLinkStateBuffer::new(payload)?)
                    .context(format!("invalid IFLA_VF_LINK_STATE {payload:?}"))?,
            ),
            IFLA_VF_RSS_QUERY_EN => Self::RssQueryEn(
                VfInfoRssQueryEn::parse(&VfInfoRssQueryEnBuffer::new(payload)?)
                    .context(format!("invalid IFLA_VF_RSS_QUERY_EN {payload:?}"))?,
            ),
            ...
            IFLA_VF_IB_NODE_GUID => Self::IbNodeGuid(
                VfInfoGuid::parse(&VfInfoGuidBuffer::new(payload)?)
                    .context(format!("invalid IFLA_VF_IB_NODE_GUID {payload:?}"))?,
            ),
            IFLA_VF_IB_PORT_GUID => Self::IbPortGuid(
                VfInfoGuid::parse(&VfInfoGuidBuffer::new(payload)?)
                    .context(format!("invalid IFLA_VF_IB_PORT_GUID {payload:?}"))?,
            ),
            IFLA_VF_VLAN_LIST => {
            ...
            IFLA_VF_STATS => {
                ...
                for nla in NlasIterator::new(payload) {
                    let nla =
                        &nla.context(format!("invalid IFLA_VF_STATS value: {:?}", buf.value()))?;
