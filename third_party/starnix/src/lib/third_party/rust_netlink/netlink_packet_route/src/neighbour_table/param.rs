...
impl Nla for NeighbourTableParameter {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            | Self::ProxyDelay(v)
            | Self::Locktime(v)
            | Self::IntervalProbeTimeMs(v) => NativeEndian::write_u64(buffer, *v),
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for NeighbourTableParameter {
    type Error = NeighbourTableError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, NeighbourTableError> {
        let payload = buf.value();
        Ok(match buf.kind() {
            NDTPA_IFINDEX => Self::Ifindex(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_IFINDEX",
                    error,
                }
            })?),
            NDTPA_REFCNT => Self::ReferenceCount(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_REFCNT",
                    error,
                }
            })?),
            NDTPA_REACHABLE_TIME => Self::ReachableTime(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
            ...
            NDTPA_BASE_REACHABLE_TIME => {
                Self::BaseReachableTime(parse_u64(payload).map_err(|error| {
                    NeighbourTableError::InvalidParameter {
            ...
            NDTPA_RETRANS_TIME => Self::RetransTime(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_RETRANS_TIME",
                    error,
                }
            })?),
            NDTPA_GC_STALETIME => Self::GcStaletime(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
            ...
            NDTPA_QUEUE_LEN => Self::QueueLen(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_QUEUE_LEN",
                    error,
                }
            })?),
            NDTPA_APP_PROBES => Self::AppProbes(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_APP_PROBES",
                    error,
                }
            })?),
            NDTPA_UCAST_PROBES => Self::UcastProbes(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_UCAST_PROBES",
                    error,
                }
            })?),
            NDTPA_MCAST_PROBES => Self::McastProbes(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_MCAST_PROBES",
                    error,
                }
            })?),
            NDTPA_ANYCAST_DELAY => Self::AnycastDelay(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_ANYCAST_DELAY",
                    error,
                }
            })?),
            NDTPA_PROXY_DELAY => Self::ProxyDelay(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_PROXY_DELAY",
                    error,
                }
            })?),
            NDTPA_PROXY_QLEN => Self::ProxyQlen(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_PROXY_QLEN",
                    error,
                }
            })?),
            NDTPA_LOCKTIME => Self::Locktime(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_LOCKTIME",
                    error,
                }
            })?),
            NDTPA_QUEUE_LENBYTES => Self::QueueLenbytes(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
                    kind: "NDTPA_QUEUE_LENBYTES",
                    error,
                }
            })?),
            NDTPA_MCAST_REPROBES => Self::McastReprobes(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidParameter {
            ...
            NDTPA_INTERVAL_PROBE_TIME_MS => {
                Self::IntervalProbeTimeMs(parse_u64(payload).map_err(|error| {
                    NeighbourTableError::InvalidParameter {
...
#[derive(Debug, PartialEq, Eq, Clone)]
pub(crate) struct VecNeighbourTableParameter(pub(crate) Vec<NeighbourTableParameter>);

impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for VecNeighbourTableParameter {
    type Error = NeighbourTableError;
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, NeighbourTableError> {
        let mut nlas = vec![];
