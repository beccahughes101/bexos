...
impl Nla for InfoVlan {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Protocol(value) => BigEndian::write_u16(buffer, (*value).into()),
            Self::Flags(flags) => {
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for VlanQosMapping {
        ...
        Ok(match buf.kind() {
            ...
            kind => Other(
                DefaultNla::parse(buf)
                    .context(format!("unknown NLA type {kind} for VLAN QoS mapping"))?,
            ),
        })
    }
}
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_VLAN_EGRESS_QOS => {
                EgressQos(parse_mappings(payload).context("failed to parse IFLA_VLAN_EGRESS_QOS")?)
            }
            IFLA_VLAN_INGRESS_QOS => IngressQos(
                parse_mappings(payload).context("failed to parse IFLA_VLAN_INGRESS_QOS")?,
            ),
