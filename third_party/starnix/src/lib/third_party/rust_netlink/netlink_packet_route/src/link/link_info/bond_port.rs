...
use netlink_packet_utils::parsers::{parse_i32, parse_u8, parse_u16, parse_u32};
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoBondPort {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_BOND_PORT_LINK_FAILURE_COUNT => LinkFailureCount(
                parse_u32(payload).context("invalid IFLA_BOND_PORT_LINK_FAILURE_COUNT value")?,
            ),
            ...
            IFLA_BOND_PORT_PRIO => {
                Prio(parse_i32(payload).context("invalid IFLA_BOND_PORT_PRIO value")?)
            }
            IFLA_BOND_PORT_QUEUE_ID => {
                QueueId(parse_u16(payload).context("invalid IFLA_BOND_PORT_QUEUE_ID value")?)
            }
