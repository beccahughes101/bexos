...
use crate::link::af_spec::{VecAfSpecInet, VecAfSpecInet6};
use crate::link::{AfSpecInet, AfSpecInet6};
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for VecAfSpecUnspec {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        for nla in NlasIterator::new(buf.into_inner()) {
            ...
            nlas.push(match nla.kind() {
                ...
                kind => AfSpecUnspec::Other(DefaultNla::parse(&nla).context(format!(
