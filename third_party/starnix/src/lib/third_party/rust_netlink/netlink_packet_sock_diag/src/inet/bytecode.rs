...
impl InvalidInstructionError {
    fn at_index(self, index: InstructionIndex) -> SerializationError {
        SerializationError::InvalidInstruction {
            at: index.get(),
            error: self,
        }
    }
...
impl Bytecode {
    ...
    pub fn serialize(self, buf: &mut [u8]) -> Result<(), SerializationError> {
        ...
        instructions
            .into_iter()
            .enumerate()
            .try_for_each(|(curr_inst_index, inst)| {
...
impl RawInstruction {
    ...
    fn parse(buf: &[u8]) -> Result<RawInstruction, ParseErrorCode> {
        ...
        if code == INET_DIAG_BC_NOP {
            return Ok(RawInstruction::Nop(
                yes.ok_or(ParseErrorCode::SelfReference)?,
            ));
        } else if code == INET_DIAG_BC_JMP {
            return Ok(RawInstruction::Jmp(
                no.ok_or(ParseErrorCode::SelfReference)?,
            ));
        }
    ...
    fn serialize(&self, buf: &mut [u8]) -> Result<(), InvalidInstructionError> {
        ...
        let max_prefix_len = match self.addr {
            Some(IpAddr::V4(addr)) => {
                ...
                Ipv4AddrBuffer::new(buf.payload_mut())
                    .unwrap()
                    .set_addr(addr.into());
            ...
            Some(IpAddr::V6(addr)) => {
                ...
                Ipv6AddrBuffer::new(buf.payload_mut())
                    .unwrap()
                    .set_addr(addr.into());
...
mod tests {
    ...
    fn accept_reject_mapping() {
        let bc = Bytecode(vec![
            Instruction::Jmp(Action::Accept),
            Instruction::Jmp(Action::Reject),
        ]);
    ...
    #[test]
    fn buffer_too_small() {
        let bc = Bytecode(vec![Instruction::Nop(Action::AdvanceBy(
    ...
    fn index_too_large_yes() {
        ...
        ops.push(Instruction::Nop(Action::AdvanceBy(
            NonZeroUsize::new(1).unwrap(),
        ))); // Target
    ...
    fn index_too_large_no() {
        ...
        let mut ops = vec![Instruction::Jmp(Action::AdvanceBy(
    ...
    fn parse_errors() {
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 0,
                code: ParseErrorCode::UnknownOpcode
            })
        );
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 4,
                code: ParseErrorCode::InvalidJumpTarget
            })
        );
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 0,
                code: ParseErrorCode::TruncatedInstruction
            })
        );
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 0,
                code: ParseErrorCode::SelfReference
            })
        );
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 0,
                code: ParseErrorCode::InvalidAddressFamily
            })
        );
        ...
        assert_eq!(
            ...
            Err(ParseError {
                index: 0,
                code: ParseErrorCode::PrefixLengthLongerThanAddress
            })
        );
    }
