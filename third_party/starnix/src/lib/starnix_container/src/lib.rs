...
impl StarnixContainerGenerator {
    ...
    fn add_ext4_image(
        ...
        let my_overrides: Vec<StarnixFileOverride> = self
            .file_overrides
            .iter()
            .filter(|o| o.image_name == name.as_str())
            .cloned()
            .collect();
        ...
        if !my_overrides.is_empty() {
            ...
            let skipped_inode_strings: std::collections::HashSet<String> = result
                .skipped_inodes
                .iter()
                .map(|i| i.to_string())
                .collect();
    ...
    pub fn build(self, deps: &mut Depfile) -> Result<()> {
        ...
        let mut builder = self
            .clone()
            .clone_package(&self.base, &self.outdir.to_string(), deps)?;
        ...
        if !self.ramdisk.is_empty() {
            let ramdisk_files =
                self.clone()
                    .add_ramdisks("ramdisk", &self.outdir, &self.ramdisk, &mut builder)?;
        ...
        let system_files =
            self.clone()
                .add_ext4_image("system", &self.outdir, &self.system, &mut builder)?;
        ...
        if let Some(vendor_path) = &self.vendor {
            let vendor_files =
                self.clone()
                    .add_ext4_image("vendor", &self.outdir, &vendor_path, &mut builder)?;
        ...
        builder
            .build(&self.outdir, &metafar_path)
            .context("Building starnix container")?;
        deps.add_outputs(
            [
...
impl StarnixContainerRepackager {
    pub fn build(self, deps: &mut Depfile) -> Result<Utf8PathBuf> {
        ...
        let container_manifest = PackageManifest::try_load_from(&self.container_manifest_path)
            .with_context(|| {
                format!(
                    "Reading existing starnix container: {}",
        ...
        for subpackage in new_base_package_manifest.subpackages() {
            let name = subpackage
                ...
                .context("parsing subpackage name")?;
        ...
        if !self.hals.is_empty() {
            ...
            if !overrides.is_empty() {
                let original_metadata = if let Some(blob) = container_manifest
                    .blobs()
                    .iter()
                    .find(|b| b.path == "data/odm/metadata.v1")
                {
        ...
        let meta_far_bytes = std::fs::read(&meta_far_blob.source_path).with_context(|| {
            format!(
                "Failed to read meta.far blob at {}",
                meta_far_blob.source_path
            )
        })?;
        let mut far_reader =
        ...
        builder
            .build(&self.outdir, &metafar_path)
            .context("Building new starnix container")?;
...
mod tests {
    ...
    const EXT4_IMAGE_PATH: &str = concat!(
        env!("ROOT_OUT_DIR"),
        "/test_data/gen-starnix-container/test.img"
    );
    ...
    fn fake_base(outdir: &Utf8Path) -> Utf8PathBuf {
        ...
        builder
            .add_file_as_blob("data/test", &test_blob_path)
            .unwrap();
        builder.manifest_path(&base_manifest_path);
        let _ = builder
            .build(&outdir, outdir.join("base-meta.far"))
            .unwrap();
        base_manifest_path
    }
    ...
    fn test_generate() -> Result<()> {
        ...
        builder
            .add_contents_as_blob("data/hal", "test-hal-blob", &outdir)
            .unwrap();
    ...
    fn test_generator_override_create() -> Result<()> {
        ...
        let system_metadata_blob = blobs
            .iter()
            .find(|b| b.path == "data/system/metadata.v1")
            .unwrap();
        let m = Metadata::deserialize(&std::fs::read(&system_metadata_blob.source_path).unwrap())
            .unwrap();

        // new_file should be there.
        let new_file_inode = m
            .lookup(ROOT_INODE_NUM, "new_file")
    ...
    fn test_repackage_preserves_custom_files() -> Result<()> {
        ...
        let added_1_inode = m
            .lookup(ROOT_INODE_NUM, "added_1")
            .expect("added_1 missing");
        ...
        let added_2_inode = m
            .lookup(ROOT_INODE_NUM, "added_2")
            .expect("added_2 missing");
    ...
    fn fake_base_with_component(outdir: &Utf8Path, component: &fcdecl::Component) -> Utf8PathBuf {
        ...
        builder
            .add_contents_to_far("meta/test.cm", component_bytes, outdir)
            .unwrap();
    ...
    fn test_hal_init_rc() {
        ...
        builder
            ...
            .unwrap();
        builder
            .add_contents_as_blob("system/init.rc", "service foo bar", &outdir)
            .unwrap();
        builder
