...
pub fn load_from_package(
    ...
    let Some(hal_manifest) = package_manifest
        .blobs()
        .iter()
        .find(|blob| blob.path == HAL_MANIFEST_PATH)
    else {
    ...
    let init_rc = manifest
        .init_rc
        .map(|p| load_blob(package_manifest, &p))
        .transpose()?;
    let vintf_manifest = manifest
        .vintf_manifest
        .map(|p| load_blob(package_manifest, &p))
        .transpose()?;
    Ok((
        ...
        Some(hal_manifest.source_path.clone()),
    ))
}
