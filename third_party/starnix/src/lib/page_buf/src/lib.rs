...
impl<T> PageBuf<T> {
    ...
    fn new_internal(
        ...
        let base = NonNull::new(std::ptr::with_exposed_provenance_mut::<MaybeUninit<u8>>(
            addr,
        ))
        .unwrap();
    ...
    pub fn set_name(&self, name: &zx::Name) {
        self.vmo
            .set_name(name)
            .expect("default vmo rights must include ability to set name");
    }
...
mod tests {
    ...
    fn can_be_used_for_object_info() {
        ...
        assert_eq!(
            maps.len(),
            avail,
            "should have consumed all of the mappings"
        );
    }
