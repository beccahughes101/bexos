...
// Symbols for terminal attributes
...
// Fastrpc ioctls
C(FASTRPC_IOCTL_INVOKE);
C(FASTRPC_IOCTL_INVOKE_FD);
C(FASTRPC_IOCTL_GETINFO);
C(FASTRPC_IOCTL_GET_DSP_INFO);
C(FASTRPC_IOCTL_INVOKE2);
C(FASTRPC_IOCTL_INIT);

// DMA-BUF Heaps
C(DMA_HEAP_IOCTL_ALLOC);
C(DMA_BUF_IOCTL_SYNC);
C(DMA_BUF_SET_NAME);
C(DMA_BUF_SET_NAME_A);
C(DMA_BUF_SET_NAME_B);
