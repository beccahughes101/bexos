    ...
    T: ArgumentTypeChecker<C>,
{
    ...
    for StructAccess {
        ...
        field_offset,
        is_32_bit_ptr_load,
    } in program.struct_access_instructions.iter()
    {
        let field_mapping = struct_mappings
            .iter()
            ...
            .and_then(|struct_map| {
                struct_map
                    .fields
                    .iter()
                    .find(|m| m.source_offset == *field_offset)
            });
        ...
        if let Some(field_mapping) = field_mapping {
            ...
            let new_offset = instruction
                .offset()
                .checked_add(offset_diff)
                .ok_or_else(|| {
                    EbpfError::ProgramLinkError(format!(
                        "Struct field offset overflow at PC {}",
                        *pc
                    ))
                })?;
                ...
                BPF_PSEUDO_MAP_IDX | BPF_PSEUDO_MAP_IDX_VALUE => {
                    ...
                    let value = if instruction.src_reg() == BPF_PSEUDO_MAP_IDX {
                        ...
                        let map_value = map
                            ...
                            .as_u64();
                        map_value
                            .checked_add_signed(next_instruction.imm().into())
                            .ok_or_else(|| {
                                EbpfError::ProgramLinkError(format!(
...
mod test {
    ...
    static TEST_ARG_32_BIT_TYPE: LazyLock<Type> = LazyLock::new(|| {
        ...
        Type::PtrToStruct {
            id: TEST_ARG_32_BIT_MEMORY_ID.clone(),
            offset: 0.into(),
            descriptor,
        }
    });
    ...
    fn initialize_test_program_for_32bit_arg(
        ...
        let verified_program = verify_program(
            code,
            CallingContext {
                args: vec![TEST_ARG_32_BIT_TYPE.clone()],
                ..Default::default()
            },
            &mut NullVerifierLogger,
        )?;
    ...
    fn test_mapping() {
        ...
        assert_eq!(
            program.run_with_1_argument(&mut (), &mut data),
            data.0.mutable_field as u64
        );
    }
