...
/// generated for types while verifying an ebpf program. Because different sources of identifiers
/// use different namespaces, this counter can start at 0.
static BPF_TYPE_IDENTIFIER_COUNTER: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

impl MemoryId {
    /// Build a `MemoryId` from a raw u64.
    ///
    /// This is supposed to be used by the `ebpf_api` crate to build static identifiers (e.g.
    /// for hardcoded program contexts like `SkBuff`), but not elsewhere.
    ...
    fn prepended(&self, other: MemoryId) -> Self {
        match &self.parent {
            ...
            Some(parent) => MemoryId {
                namespace: self.namespace,
                id: self.id,
                parent: Some(Box::new(parent.prepended(other))),
            },
        }
    }
    ...
    fn matches(&self, other: &MemoryId) -> bool {
        if self.namespace != other.namespace || self.id != other.id {
            return false;
        };
...
impl StructDescriptor {
    ...
    fn find_field(&self, base_offset: ScalarValueData, field: Field) -> Option<&FieldDescriptor> {
        ...
        let is_valid_load = match field_desc.field_type {
            ...
            | FieldType::PtrToArray { is_32_bit, .. }
            | FieldType::PtrToEndArray { is_32_bit, .. } => {
                let expected_width = if is_32_bit {
                    DataWidth::U32
                } else {
                    DataWidth::U64
                };
    ...
    fn constraint(
        ...
        let result = match (
            ...
            (JumpWidth::W64, JumpType::Lt, Type::ScalarValue(lhs), Type::ScalarValue(rhs)) => {
                ...
                (
                    lhs.update_range(new_range_lhs).into(),
                    rhs.update_range(new_range_rhs).into(),
                )
            }
    ...
    fn match_parameter_type(
        ...
        match (parameter_type, self) {
            ...
            (Type::MapKeyParameter { map_ptr_index }, Type::PtrToStack { offset }) => {
                ...
                context
                    .stack
                    .read_data_ptr(context.pc, *offset, schema.key_size as u64)
            }
            (
            ...
            (Type::MapValueParameter { map_ptr_index }, Type::PtrToStack { offset }) => {
                ...
                context
                    .stack
                    .read_data_ptr(context.pc, *offset, schema.value_size as u64)
            }
            (
                ...
                if id2.has_parent(id1) =>
            {
                if next.resources.remove(id2) {
                    ...
                    Err(format!(
                        "{id2:?} Resource already released for index {index}"
                    ))
                }
            }
...
pub fn verify_program(
    ...
    while let Some(mut context) = verification_context.states.pop() {
            ...
            verification_context.terminating_contexts.get(&context.pc)
        {
            ...
            // also terminate.
            if let Some(ending_context) = terminating_contexts
                .iter()
                .find(|c| c.computation_context >= context)
            {
                ...
                if let Some(parent) = context.parent.take() {
                    parent
                        .dependencies
                        .lock()
                        .merge(ending_context.dependencies);
        ...
        if verification_context.iteration > 10 * BPF_MAX_INSTS {
            return error_and_log(
                verification_context.logger,
                "bpf byte code does not terminate",
            );
        }
    ...
    let struct_access_instructions = verification_context
        .struct_access_instructions
        .into_values()
        .collect::<Vec<_>>();
    let CallingContext { maps, args, .. } = verification_context.calling_context;
    Ok(VerifiedEbpfProgram {
        code,
        struct_access_instructions,
        maps,
        args,
...
impl ComputationContext {
    ...
    fn resolve_return_value(
        ...
        match return_value {
            ...
            Type::ReleasableParameter { id, inner } => {
                let id = verification_context.next_id().prepended(id.clone());
                if !maybe_null {
    ...
    fn apply_computation(
        ...
        let result: Type = match (alu_type, op1.inner(context)?, op2.inner(context)?) {
            ...
            {
                Type::PtrToStack {
                    offset: run_on_stack_offset(*x, |x| op(x, *data)),
                }
            }
            (
                alu_type,
    ...
    fn conditional_jump(
        ...
        if branch.unwrap_or(true) {
            ...
            verification_context
                .states
                .push(apply_constraints_and_register(
    ...
    fn terminate(self, verification_context: &mut VerificationContext<'_>) -> Result<(), String> {
        ...
        while let Some(mut current) = next.take() {
            ...
            let terminating_contexts = verification_context
...
impl PartialOrd for ComputationContext {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        let result = loop {
            match (
                array_bound_iter1.peek().cloned(),
                array_bound_iter2.peek().cloned(),
            ) {
...
impl BpfVisitor for ComputationContext {
    ...
    fn and<'a>(
        ...
        self.alu(Some("and32"), context, dst, src, AluType::Plain, |x, y| {
            alu32(x, y, |x, y| x & y)
        })
    }
    fn and64<'a>(
    ...
    fn arsh64<'a>(
        ...
        self.alu(Some("arsh"), context, dst, src, AluType::Plain, |x, y| {
            x.ashr(y)
        })
    }
    fn div<'a>(
    ...
    fn lsh64<'a>(
        ...
        self.alu(Some("lsh"), context, dst, src, AluType::Plain, |x, y| {
            x << y
        })
    }
    fn r#mod<'a>(
    ...
    fn or<'a>(
        ...
        self.alu(Some("or32"), context, dst, src, AluType::Plain, |x, y| {
            alu32(x, y, |x, y| x | y)
        })
    }
    fn or64<'a>(
    ...
    fn rsh64<'a>(
        ...
        self.alu(Some("rsh"), context, dst, src, AluType::Plain, |x, y| {
            x >> y
        })
    }
    fn sub<'a>(
    ...
    fn xor<'a>(
        ...
        self.alu(Some("xor32"), context, dst, src, AluType::Plain, |x, y| {
            alu32(x, y, |x, y| x ^ y)
        })
    }
    fn xor64<'a>(
    ...
    fn call_external<'a>(
        ...
        for (arg_index, arg) in signature.args.iter().enumerate() {
            ...
            self.reg(reg)?
                .match_parameter_type(context, self, name, arg, arg_index, &mut next)?
        }
...
mod tests {
    ...
    fn test_type_ordering() {
        ...
        assert_eq!(
            unknown_written.partial_cmp(&unknown_written),
            Some(Ordering::Equal)
        );
        assert_eq!(
            unknown_written.partial_cmp(&unwritten),
            Some(Ordering::Less)
        );
        ...
        assert_eq!(
            unwritten.partial_cmp(&unknown_written),
    ...
    fn test_stack_access() {
        ...
        assert!(
            s.store(StackOffset(8.into()), Type::UNKNOWN_SCALAR, DataWidth::U64)
                .is_ok()
        );
        assert!(
            s.store(StackOffset(16.into()), Type::UNKNOWN_SCALAR, DataWidth::U64)
                .is_ok()
        );
        assert!(
            s.store(StackOffset(24.into()), Type::UNKNOWN_SCALAR, DataWidth::U16)
                .is_ok()
        );
        ...
        for offset in 0..32 {
            for end in (offset + 1)..32 {
                assert_eq!(
                    s.read_data_ptr(2, StackOffset(offset.into()), (end - offset) as u64)
                        .is_ok(),
                    offset >= 8 && end <= 26
                );
            }
        ...
        assert!(
            s.read_data_ptr(2, StackOffset(12.into()), u64::MAX - 2)
                .is_err()
        );
    }
    ...
    fn test_type_constraint_null_or() {
        let mut context = ComputationContext::default();
        let id = MemoryId::new();
        let null_or = Type::NullOr {
            id: id.clone(),
            inner: Box::new(Type::UNKNOWN_SCALAR),
        };
        let (new_lhs, _) = Type::constraint(
            &mut context,
            JumpType::Eq,
    ...
    fn test_resolve_return_value_unique_ids() {
        ...
        let ret_template = Type::PtrToMemory {
            id: static_id.clone(),
            offset: 0.into(),
            buffer_size: 64,
        };
