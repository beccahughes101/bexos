...
impl<C: EbpfProgramContext> BpfVisitor for ComputationContext<'_, C> {
    ...
    fn neg64<'a>(&mut self, _context: &mut Self::Context<'a>, dst: Register) -> Result<(), String> {
        self.alu(dst, Source::Value(0), |x, _y| {
            (x as i64).overflowing_neg().0 as u64
        })
    }
    ...
    fn atomic_add<'a>(
        ...
        self.atomic_operation(fetch, dst, offset, src, |_, a, v| {
    ...
    fn atomic_add64<'a>(
        ...
        self.atomic_operation64(fetch, dst, offset, src, |_, a, v| {
            a.fetch_add(v, Ordering::SeqCst)
        })
    }
    ...
    fn load_from_packet<'a>(
        ...
        let Some(offset) = register_offset
            .map(|r| self.reg(r).as_i32())
            .unwrap_or(0)
