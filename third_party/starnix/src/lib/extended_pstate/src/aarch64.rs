...
mod test {
    ...
    fn test_save_restore_64() {
        ...
        let mut pstate_ptr_struct = crate::ExtendedPstatePointer {
            extended_pstate: &mut state,
        };
        ...
        assert_eq!(
            state.get_arm64_fpcr(),
            sentinel_fpcr as u32,
            "state.fpcr mismatch"
        );
        assert_eq!(
            state.get_arm64_fpsr(),
            sentinel_fpsr as u32,
            "state.fpsr mismatch"
        );

        for i in 0..32 {
            assert_eq!(
                restored_state.q[i], sentinel_q,
                "restored_state.q[{}] mismatch",
                i
            );
        }
        assert_eq!(
            restored_state.fpcr, sentinel_fpcr as u32,
            "restored_state.fpcr mismatch"
        );
        assert_eq!(
            restored_state.fpsr, sentinel_fpsr as u32,
            "restored_state.fpsr mismatch"
        );
    }
    ...
    fn test_save_restore_32() {
        ...
        let mut pstate_ptr_struct = crate::ExtendedPstatePointer {
            extended_aarch32_pstate: &mut state,
        };
        let pstate_ptr = &mut pstate_ptr_struct as *mut crate::ExtendedPstatePointer;
        ...
        assert_eq!(
            state.get_arm32_fpcr(),
            sentinel_fpcr as u32,
            "state.fpcr mismatch"
        );
        assert_eq!(
            state.get_arm32_fpsr(),
            sentinel_fpsr as u32,
            "state.fpsr mismatch"
        );
