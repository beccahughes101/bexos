    ...
    V: Clone,
{
    ...
    fn measure_gap(&self, other: &Self) -> u64 {
        ...
        self.keys[self.keys.len() - 1]
            .end
            .measure_gap(&other.keys[0].start)
    }
    ...
    fn remove(&mut self, cursor: Cursor) -> RemoveResult<K, V> {
        if let Some(index) = self.get_index(cursor) {
            ...
            if self.keys.len() < NODE_CAPACITY / 2 {
                RemoveResult::Underflow(RemoveState {
                    maybe_split_key,
                    removed_value,
                })
            } else {
                RemoveResult::Removed(RemoveState {
                    maybe_split_key,
                    removed_value,
                })
            }
    ...
    V: Clone,
{
    ...
    fn new(keys: ArrayVec<K, NODE_CAPACITY>, children: ChildList<K, V>) -> Self {
        let mut node = Self {
            max_gap: 0,
            subtree_key_range: children.subtree_key_range(),
            keys,
            children,
        };
        node.update_max_gap();
    ...
    fn last_key_value(&self) -> Option<(&Range<K>, &V)> {
        match &self.children {
            ChildList::Leaf(children) => children
                .last()
                .expect("child lists are always non-empty")
                .last_key_value(),
            ChildList::Internal(children) => children
                .last()
                .expect("child lists are always non-empty")
                .last_key_value(),
        }
    ...
    /// Measure the gap between the last key in this node and the first key in the other node.
    fn measure_gap(&self, other: &Self) -> u64 {
        self.subtree_key_range
            .end
            .measure_gap(&other.subtree_key_range.start)
    }
    ...
    fn insert_child(&mut self, index: usize, key: K, child: Node<K, V>) -> InsertResult<K, V> {
        ...
        if n == NODE_CAPACITY {
            ...
            let middle = NODE_CAPACITY / 2;
            assert!(middle > 0);
            let mut internal = Self::new(
                self.keys.drain(middle..).collect(),
                self.children.split_off(middle),
            );
        ...
        match &mut self.children {
            ChildList::Leaf(children) => {
                ...
                if n <= NODE_CAPACITY {
                    ...
                    left_node
                        .keys
                        .extend(right_shared_node.keys.iter().cloned());
                    left_node
                        .values
                        .extend(right_shared_node.values.iter().cloned());
                    ...
                    if left_node.values.len() < split {
                        ...
                        left_node
                            .values
                            .extend(right_node.values.drain(..move_count));
                    } else {
                ...
                if n <= NODE_CAPACITY {
                    ...
                    if left_node.children.len() < split {
                        ...
                        split_key = left_node
                            .keys
                            .pop()
                            .expect("must have moved at least one element");

                        left_node
                            .children
                            .extend(&right_node.children.split_off_front(move_count));
                        debug_assert!(left_node.keys.len() + 1 == left_node.children.len());
                    } else {
                        // Move elements from left to right.
...
#[derive(Clone, Debug)]
pub struct RangeMap<K: Ord + Copy + Gap, V: Clone + Eq> {
    /// The root node of the tree.
    ///
    ...
    V: Clone + Eq,
{
    fn default() -> Self {
        Self {
            node: Node::Leaf(Arc::new(NodeLeaf::empty())),
        }
    }
}
    ...
    V: Clone + Eq,
{
    ...
    pub fn find_at_or_after(&self, key: K) -> Option<(&Range<K>, &V)> {
        let mut iter = self
            .range(key..)
            .filter(move |(range, _)| key <= range.start);
        iter.next()
    }
...
mod test {
    ...
    fn test_iter() {
        ...
        assert_eq!(
            map.range(..75).collect::<Vec<_>>(),
            vec![(&(10..34), &-14), (&(74..92), &-12)]
        );
        assert_eq!(
            map.range(..91).collect::<Vec<_>>(),
            vec![(&(10..34), &-14), (&(74..92), &-12)]
        );
        assert_eq!(
            map.range(..92).collect::<Vec<_>>(),
            vec![(&(10..34), &-14), (&(74..92), &-12)]
        );
    }
    ...
    fn test_find_gap_end_empty() {
        let map = RangeMap::<u32, i32>::default();
    ...
    fn test_insert_merge_does_not_return_values() {
        ...
        assert!(
            removed.is_empty(),
            "merging right should not return removed values"
        );
        ...
        assert!(
            removed.is_empty(),
            "merging left should not return removed values"
        );
