...
impl Usercopy {
    ...
    pub fn new(restricted_address_range: Range<usize>) -> Result<Self, zx::Status> {
        ...
        let shutdown_event_clone = shutdown_event
        ...
        let join_handle = std::thread::spawn(move || {
            ...
            loop {
                ...
                excp.set_exception_state(&zx::sys::ZX_EXCEPTION_STATE_HANDLED)
                    .unwrap();
            }
        });
...
impl Drop for Usercopy {
    fn drop(&mut self) {
        self.shutdown_event
            .signal(zx::Signals::empty(), zx::Signals::USER_0)
            .unwrap();
...
mod test {
    ...
    fn zero_fault(offset: usize, zero_len: usize, ch: u8) {
        ...
        assert_eq!(
            &mapped_bytes[..page_size - offset],
            &vec![ch; page_size - offset][..]
        );
    }
    ...
    fn atomic_compare_exchange_u32_acq_rel_no_fault() {
        ...
        assert_eq!(
            m.usercopy
                .atomic_compare_exchange_u32_acq_rel(m.addr, 0x12345678, 0xffffffff),
            Ok(Ok(0x12345678))
        );
        ...
        assert_eq!(
            m.usercopy
                .atomic_compare_exchange_u32_acq_rel(m.addr, 0x22222222, 0x11111111),
            Ok(Err(0xffffffff))
        );
    ...
    fn atomic_compare_exchange_u32_acq_rel_fault() {
        ...
        let result = m
            .usercopy
            .atomic_compare_exchange_u32_acq_rel(m.addr, 0x00000000, 0x11111111);
        assert_eq!(Err(()), result);
