...
fn test_set_drops_old_payload() {
    ...
    let mut container = ContainerUnsplit {
        data_enum: DataEnum::VariantWithAlloc(alloc.clone()),
    }
    .decompose();
    ...
    assert_eq!(
        container.data_enum(),
        DataEnum::VariantWithAlloc(alloc.clone())
    );
    assert_eq!(
        DataEnum::VariantWithAlloc(alloc.clone()),
        container.data_enum()
    ...
    // This consumes `alloc`.
    assert_eq!(
        container.data_enum().to_owned(),
        DataEnum::VariantWithAlloc(alloc)
    );
