...
The implementation uses an explicit BexOS scene ABI between WASM and the trusted native runner. Native GPU handles, shaders, VMOs, renderer caches, and compositor resources stay in the runner and scened. The guest submits bounded, validated 2D scene batches; the runner renders them with the Vello/Venus path when the granted display endpoint supports GPU transport, and otherwise replays the same scene on the CPU.
...
- `//apps/dioxus_shared`: installable `com.bexos.lib.dioxus` WASM component package. It exports `bexos:wasm/dioxus@1.0.0`, decodes document submissions, resolves the supported CSS subset, computes WASM-side layout and glyph runs, then submits validated scenes to the host UI interface. Raw scene submission remains available for compatibility and focused renderer tests.
- `//lib/dioxus_dom`: value-only document tree, selector/declaration parsing, deterministic flex/grid-style layout, fixed-advance glyph shaping, and projection into the scene ABI. It is the current implemented shared-component body behind the Dioxus WIT boundary.
...
`View::submit_document()` sends an encoded `bexos_dioxus_guest::dom::Document` to the shared component. The current document ABI supports stable node IDs, tags, classes, inline declarations, stylesheet rules, text nodes, image nodes, and listener flags for pointer, keyboard, text, wheel, and focus handling. The shared component resolves tag/class/id selectors and a bounded CSS subset (`display`, `flex-direction`, `grid-template-columns`, `width`, `height`, `x`, `y`, `padding`, `gap`, `background`, `color`, `font-size`, and `scroll-y`) before emitting the validated scene batch. `View::submit()` remains available for direct scene tests.
...
The app must request `bexos.ui.scened.FlatlandSession` Public methods 1, 2, 8, 14, 15, 16, 20, and 24. Methods 1/2/8/14/15/16/20 cover node creation, root/content/presentation/input/status operations used by the runner; method 24 supplies a session-local viewport query so resize and scale changes do not require shell control.
...
The first shipped scene ABI is deliberately 2D and UI-shaped. It supports paths, colors, gradients, transforms, layers/clips, glyph runs, image commands, and shadows. Text shaping, CSS selector resolution, and DOM/document interpretation are kept behind the shared component boundary so the application ABI does not expose Rust pointers or callbacks. The current checked-in shared component implements a bounded BexOS document projection; the design document retains the longer-term Blitz DOM/Stylo/Taffy/Parley implementation behind the same exported interface.
...
The runner migration codec retains dependency identities, component payload identities, UI resource metadata, presentation sequence state, and renderer quiescence requirements. During replacement, appd prepares package resolution, component composition, compilation, and replacement stores before the final quiescence point. The source keeps handling input and presentation until the candidate is ready. The runner refuses cutover while presentation releases or GPU jobs are still unsafe to transfer; failed restore resumes the original app without duplicating input or presentation.
