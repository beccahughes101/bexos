...
pub fn word(out: &mut alloc::vec::Vec<u8>, number: u32, value: u64) {
    put_varint(out, u64::from(number) << 3);
    put_varint(out, value);
}
pub fn bytes(out: &mut alloc::vec::Vec<u8>, number: u32, value: &[u8]) {
    put_varint(out, (u64::from(number) << 3) | 2);
    put_varint(out, value.len() as u64);
    out.extend_from_slice(value);
}
fn put_varint(out: &mut alloc::vec::Vec<u8>, mut value: u64) {
    while value >= 128 {
