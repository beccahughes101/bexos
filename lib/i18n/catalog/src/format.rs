...
impl<'a> Catalog<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, Error> {
        ...
        if entries == 0 || !slots.is_power_of_two()
            || slots > MAX_NODES * 4
            || entries > slots / 2
            || nodes > MAX_NODES
        ...
        for i in 0..entries {
            ...
            if locale.is_empty() || locale.len()>32 || key.is_empty() || key.len()>256 || previous.is_some_and(|p| p >= (locale,key)) {return Err(Error::Malformed);}
