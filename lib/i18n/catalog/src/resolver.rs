...
impl<F: Formatter> Resolver<'_, F> {
    pub fn resolve(&self, key: &str, args: &Arguments) -> Result<String, Error> {
        if key.len() > 256
            ...
            || args.len() > 256
            || args.iter().any(|(k,v)| k.len()>256 || match v {Value::Text(s)|Value::Number(s)=>s.len()>65536,Value::FormattedNumber{raw,text}=>raw.len().saturating_add(text.len())>65536})
        {
    ...
    fn eval(
        ...
        *budget -= 1;
        ...
        if matches!(n.kind, kind::TEXT | kind::NUMBER) && n.b > 65536 {return Err(Error::EvaluationLimit);}
