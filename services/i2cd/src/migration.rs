...
impl Runtime {
    pub fn new(
        ...
        let controllers = topology
            .controllers
            .into_iter()
            .filter(|controller| controller.bus == BusKind::I2c)
    ...
    pub fn controller_for_peripheral_mut(&mut self, node_id: u64) -> Option<&mut BusController> {
        self.controllers.iter_mut().find(|controller| {
            controller
                .config
                .peripherals
                .iter()
...
impl State for Runtime {
    ...
    fn encode_record(&self, key: u64) -> Result<Option<Vec<u8>>, Error> {
        ...
        match key {
            ...
            2 => {
                encode_backend(&mut w, &self.backend);
            }
            3 => {
                w.word(1);
    ...
    fn validate(&self) -> Result<(), Error> {
        ...
        for controller in &self.controllers {
            controller
                .config
                .clone()
                .validate_as_single_for_service()
    ...
    fn activated(&mut self, generation: u64) {
        bexos_userspace::log(&alloc::format!("i2cd: adopted generation={generation}\n"));
    }
}

trait ValidateSingleForService {
    fn validate_as_single_for_service(self) -> Result<(), ()>;
}

impl ValidateSingleForService for bexos_i2c_spi::ControllerConfig {
    fn validate_as_single_for_service(self) -> Result<(), ()> {
        Topology {
            controllers: alloc::vec![self],
        }
        .validate()
        .map_err(|_| ())
    }
