...
use alloc::vec::Vec;
use bexos_i2c_spi::{I2cBundle, I2cOp, Request, Status, SubmitResult, topology_proto};
use bexos_userspace::{
...
pub async fn main(channel: u64) -> ! {
    ...
    let mut state = if start.migration_target {
        ...
        bexos_userspace::log("i2cd: ready\n");
        state
    };
    ...
    loop {
        ...
        for controller in &mut state.controllers {
            ...
            while let Some(completion) = controller.next_completion() {
                wire::reply_i2c_transfer(Channel(completion.client_channel), &completion.outcome);
                source.changed(1);
            }
        }
        bexos_userspace_async::yield_once().await;
...
fn handle_client(
    ...
    match ordinal {
        ...
        2 => {
            let outcome = I2cDeviceTransferRequest::decode(request, &[])
                .ok()
                .and_then(|request| {
                    i2c_request_from_fidl(&request)
                        .ok()
            ...
            match outcome {
                Ok((deadline, bundle)) => {
                    match controller.submit(
                        ...
                        SubmitResult::Rejected(status) => wire::reply_i2c_transfer(
                ...
                Err(status) => wire::reply_i2c_transfer(
                    Channel(client.channel),
                    &bexos_i2c_spi::TransferOutcome {
                        status,
                        reads: Vec::new(),
        ...
        3 => {
            let response = I2cDeviceLockBusRequest::decode(request, &[]).ok().map_or(
...
fn reply_info(controller: &bexos_i2c_spi::BusController, client: &bexos_i2c_spi::ClientEndpoint) {
    let Some((_, peripheral)) = controller
        .config
        .peripherals
        .iter()
        .enumerate()
        .find(|(_, peripheral)| peripheral.node_id == client.peripheral_node_id)
    else {
        return;
    };
    let bexos_i2c_spi::PeripheralConfig::I2c(i2c) = &peripheral.config else {
        return;
    };
    wire::reply(
        Channel(client.channel),
        &I2cDeviceGetInfoResponse {
            status: i2c_spi_fidl::Status::Ok,
            info: I2cDeviceInfo {
                controller_node_id: controller.config.node_id,
                peripheral_node_id: peripheral.node_id,
                address: i2c.address.raw,
                address_kind: wire::i2c_address_kind(i2c.address.kind),
...
fn i2c_request_from_fidl(request: &I2cDeviceTransferRequest<'_>) -> Result<I2cBundle, Status> {
    ...
    for index in 0..request.operations.len() {
        ...
        operations.push(match op.kind {
            I2cOpKind::Write => I2cOp::Write(collect_bytes(&op.bytes)?),
            I2cOpKind::Read => {
                if op.read_length == 0 || !op.bytes.is_empty() {
                    return Err(Status::InvalidArgs);
                }
                I2cOp::Read(op.read_length as usize)
            }
        });
    }
    Ok(I2cBundle { operations })
}
