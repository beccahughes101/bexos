...
pub fn validate_mutation(service: &Service, mutation: &Mutation) -> Result<(), Error> {
    match mutation {
        Mutation::User { package, uid, fingerprint, .. } if package == PACKAGE_ID => {
            for p in service.packages.values().filter(|p| p.id == *package && p.schema.fingerprint() == *fingerprint) {
        ...
        Mutation::Operator { package, .. } => {
            ...
            if p.id == PACKAGE_ID { let (_, bytes) = service.effective(&p.key, 0)?; Settings::from_table(&bytes)?; }
        }
        _ => {}
    }
    Ok(())
