...
fn run() -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        ...
        let scroll = String::from_utf8_lossy(session.client.received_trace())
            ...
            .rev()
            .find_map(|line| {
                line.split_once("text=retaineda scroll=")
        ...
        wait(
            &mut session.client,
            &format!("counter=18 text=retaineda scroll={scroll} "),
        )?;
        ...
        wait(
            &mut session.client,
            &format!("counter=18 text=retainedab scroll={scroll} "),
        )?;
        ...
        session
            .client
            .lock_user(1001)
            .map_err(|e| format!("lock isolated user {e:?}"))?;
        ...
        session
            .client
            .delete_user(1001)
            .map_err(|e| format!("delete isolated user {e:?}"))?;
        session
            .client
            .create_user(1001, "bob", "Bob", "testpass")
        ...
        session
            .client
            .delete_user(1001)
            .map_err(|e| format!("clean isolated user {e:?}"))?;
        Ok(())
    })();
    if result.is_err() {
        eprintln!(
...
fn native(
    ...
    client
        .launch_app("bexos.test.locale_native", "native", 0, uid)
        .map_err(|e| format!("isolated native launch {e:?}"))?;
    wait(
        client,
        &format!("locale-native: number={number} plural=few readonly=true generation={generation}"),
    )
}
