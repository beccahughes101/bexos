...
pub fn stage(rt: &mut Runtime<PhysicalBackend>, plan: &LoadPlan) -> Result<()> {
    ...
    map_ranges(&mut rt.backend, kernel_root(), &ranges, true)?;
    let result = rt
        .processes
        .iter()
    ...
    if result.is_err() {
        let _ = map_ranges(&mut rt.backend, kernel_root(), &ranges, false);
...
pub fn abort(rt: &mut Runtime<PhysicalBackend>) {
    ...
    map_ranges(&mut rt.backend, kernel_root(), &ranges, false)
        .expect("restore idle kernel mappings");
    for process in rt.processes.iter().filter(|process| process.root != 0) {
...
pub fn reclaim_old_kernel(rt: &mut Runtime<PhysicalBackend>, old: KernelRange) {
    map_ranges(&mut rt.backend, kernel_root(), &[old], false)
        .expect("reclaim idle kernel mappings");
    for process in rt.processes.iter().filter(|process| process.root != 0) {
