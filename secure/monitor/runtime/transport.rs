...
unsafe fn boot_mailbox(operation: u32) -> &'static mut Mailbox {
    #[cfg(feature = "resident_nucleus")]
...
unsafe fn request(secure: bool, r: [u64; 8]) -> Result<[u64; 8], Status> {
    ...
    match Call::decode(r)? {
        ...
        Call::RootFetch { address } if secure => {
            ...
            let destination = unsafe {
                core::slice::from_raw_parts_mut(
                    (crate::BANK + address) as *mut u8,
                    MAX_SHARED_BYTES as usize,
                )
            };
            let message =
                unsafe { (&mut *core::ptr::addr_of_mut!(CONTROL_MAILBOX)).fetch(destination)? };
            unsafe {
        ...
        Call::RootComplete { ticket, result } if secure => {
            let address = unsafe { CONTROL_BUFFER };
            if address == 0 {
                return Err(Status::InvalidHandle);
            }
            let response = unsafe {
                core::slice::from_raw_parts(
                    (crate::BANK + address) as *const u8,
                    MAX_SHARED_BYTES as usize,
                )
            };
            let control = unsafe { &mut *core::ptr::addr_of_mut!(CONTROL_MAILBOX) };
        ...
        Call::Complete { ticket, result } if secure => {
        ...
        Call::IsRegistered { handle } if secure => {
            #[cfg(feature = "boot_ipc_probe")]
...
impl<F: FnMut()> bexos_trusty_boot::ql::Transport for Boot<F> {
    fn exchange(
        ...
        let ticket =
            unsafe { boot_mailbox(operation).submit(BOOT_HANDLE, operation, length, buffer) }
                .map_err(|_| Error::Transport)?;
        loop {
            match unsafe { boot_mailbox(operation).poll(BOOT_HANDLE, ticket, buffer) } {
                Ok(result) => {
            ...
            if self.now_ns().saturating_sub(started) >= self.timeout_ns {
                unsafe {
                    boot_mailbox(operation).revoke(BOOT_HANDLE);
                }
...
pub unsafe fn exit(secure: bool, vmcb: &mut Vmcb, regs: &mut Registers) {
    let r = [
        vmcb.rax(),
        regs.rbx,
        regs.rcx,
