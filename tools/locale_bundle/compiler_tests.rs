...
fn real_cldr_plural_resolution_and_regional_interpolation() {
    ...
    let ar="n = { $count ->\n [zero] صفر\n [one] واحد\n [two] اثنان\n [few] قليل\n [many] كثير\n *[other] آخر\n}\n";
    let bytes=compile("en-US",&[Source{locale:"en-US",path:"en",text:"n = baseline"},Source{locale:"ru",path:"ru",text:ru},Source{locale:"ar",path:"ar",text:ar}]).unwrap();
    for (language,count,expected) in [("ru",1,"один"),("ru",2,"несколько"),("ru",5,"много"),("ar",0,"صفر"),("ar",1,"واحد"),("ar",2,"اثنان"),("ar",3,"قليل"),("ar",11,"كثير"),("ar",100,"آخر")] {
        let formatter=Formatting::new(include_bytes!(env!("TEST_CLDR")),Settings{region:"de-DE".into(),..Settings::default()}).unwrap();
        let r=Resolver{catalog:Catalog::parse(&bytes).unwrap(),languages:vec![language.into()],formatter};
        assert_eq!(r.resolve("n",&vec![("count".into(),Value::number(count))]).unwrap(),expected);
        if language=="ru" {assert_eq!(r.resolve("value",&vec![("count".into(),Value::number(1234.5))]).unwrap(),"1.234,50");}
    }
}
